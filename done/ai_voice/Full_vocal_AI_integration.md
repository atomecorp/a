...
- 2026-08-09: the full-screen Atome long-press assistant now requires the MCP bridge, routes AI chains through `mcp.toolchains.execute`, reuses Home endpointing and anti-echo rules, supports captured-speech barge-in, and uses one explicitly active vault-backed provider. Browser STT silence endpoints, Home provider selection, MCP Atome create/modify, and the generated/compiled Tauri iOS STT target are validated. The Android target is generated and its ARM64 debug APK builds with API 26 minimum/API 36 target plus verified microphone/network permissions; physical-device voice acceptance remains open.
