...
- **Validé** : contrat wasm-bindgen identique (21 exports, tous les `run_/apply_/read_atome_bevy_*` présents) ; probe réelle `tests/probes/project_scene_canvas_regression_probe.probe.mjs` sur `:3001` → import image rendu WebGPU, `opaque_ratio=1`, `luma_range=67`, 1 canvas, **0 erreur console / 0 pageerror**. **A/B confirmé** : WASM original (19,7 Mo) et optimisé (10,4 Mo) produisent un résultat identique (même `luma_range=67`, même `drag_failed`) → le `drag_failed` est un geste synthétique headed pré-existant, indépendant du changement.
