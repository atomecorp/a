...
const OUT_DIR = path.resolve('tools/headless_output/molecule_webgpu_visual_probe');
...
const mediaPaths = {
    vampire: path.resolve(PROJECT_ROOT, 'temp_import_tests', 'Vampire.m4v'),
    fire: path.resolve(PROJECT_ROOT, 'temp_import_tests', "Jeezs's fire.m4v"),
    audio: path.resolve(PROJECT_ROOT, 'temp_import_tests', 'test.m4a')
};
...
const main = async () => {
    ...
    try {
        ...
        const bootstrap = await page.evaluate(async () => {
            const { ensureMoleculeEngine } = await import('/application/eVe/core/media_engine/index.js');
