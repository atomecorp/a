...
const outFile = path.join(outDir, 'mtrack_preview_dispatch_probe.json');
...
const openMtrack = async (page) => {
  ...
  const opened = await safeEval(page, async (input) => {
    ...
    return runtime.invokeById({
      ...
      presentation: 'ui',
      source: { type: 'headless_probe', layer: 'mtrack_preview_dispatch_probe' }
    });
...
const run = async () => {
  ...
  try {
    ...
    const result = await safeEval(page, async () => {
      ...
      if (!api?.debugBenchmarkPreviewDispatch) return { ok: false, error: 'debug_preview_dispatch_missing' };
      return api.debugBenchmarkPreviewDispatch({ track_count: 24, clips_per_track: 12, iterations: 10 });
    }, null, { ok: false, error: 'preview_dispatch_eval_failed' });
    report.ok = !!result?.ok;
