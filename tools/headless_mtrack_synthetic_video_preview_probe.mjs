...
const outFile = path.join(outDir, 'mtrack_synthetic_video_preview_probe.json');
...
const openMtrack = async (page) => {
  ...
  const opened = await safeEval(page, async (input) => {
    ...
    return runtime.invokeById({
      ...
      presentation: 'ui',
      source: { type: 'headless_probe', layer: 'mtrack_synthetic_video_preview_probe' }
    });
...
const run = async () => {
  ...
  try {
    ...
    const result = await safeEval(page, async () => {
      ...
      if (!api?.debugBenchmarkSyntheticVideoPreview) return { ok: false, error: 'debug_synthetic_video_preview_missing' };
      return api.debugBenchmarkSyntheticVideoPreview({
        iterations: 8,
        video_source: '/assets/videos/superman.mp4'
      });
    }, null, { ok: false, error: 'synthetic_video_preview_eval_failed' });
    report.ok = !!result?.ok;
