...
- Canonical user identity is an opaque immutable UUID v4 principal independent of phone. Fastify boot migration journals each legacy principal before changing active references, keeps events/snapshots immutable, and resolves historical actors only through protected legacy aliases. Phones are private verified credentials, never Atome particles, JWT claims, or public projections. Phone lookup requires an authenticated principal; phone changes/removal require explicit OTP purpose and revoke refresh sessions.
