...
use crate::{
    components::{AtomeShapeShadow, AtomeShapeShadowOverlay},
    render_math::{atome_rect_transform, depth_for_layer},
    selection_overlay::build_shadow_texture_rgba,
    types::{
...
pub fn rebuild_shape_shadow_overlay(world: &mut World, entity: Entity) -> Result<(), String> {
    ...
    let Some((image_width, image_height, rgba)) =
        build_shadow_texture_rgba(style, shadow_width, shadow_height)
    else {
        return Ok(());
    };
    let image = Image::new(
