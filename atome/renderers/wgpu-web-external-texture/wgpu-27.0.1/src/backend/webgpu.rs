...
impl dispatch::DeviceInterface for WebDevice {
    ...
    fn create_external_texture(
        ...
        let source = desc
            .source
            .as_ref()
            .expect("Web external texture creation requires a browser media source");
        let mapped_desc = webgpu_sys::GpuExternalTextureDescriptor::new(source);
        if let Some(label) = desc.label {
            mapped_desc.set_label(label);
        }
        WebExternalTexture {
            inner: self.inner.import_external_texture(&mapped_desc).unwrap(),
            ident: crate::cmp::Identifier::create(),
        }
        .into()
    }
...
impl dispatch::ExternalTextureInterface for WebExternalTexture {
    fn destroy(&self) {
        // Browser GPUExternalTexture lifetimes are frame-scoped and owned by the browser.
    }
}
