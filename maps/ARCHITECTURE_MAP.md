...
Tauri mobile status: `platforms/desktop-tauri/gen/apple` and `platforms/desktop-tauri/gen/android` are the generated mobile applications, and the vendored STT plugin supplies their native speech-recognition implementations. The iOS Rust target compiles successfully. The Android lane is owned by `scripts/android/apk.sh`, dispatched from `./run.sh apk` through `scripts/setup/service_commands.sh`: it resolves the JDK, the SDK, the NDK and the Rust targets idempotently, stages `platforms/desktop-tauri/gen/android-webroot`, and is the only caller of `tauri android build`; `--isolated-home <dir>` relocates the cargo, Gradle and Android user caches under `<dir>` for hermetic and CI builds. `platforms/desktop-tauri/tauri.android.conf.json` sets API 26 minimum, API 36 target, release resource bundles to an empty list (Android resources are not file paths), and points `frontendDist` at the staged web root. Reqwest uses Rustls rather than a host OpenSSL dependency. `platforms/desktop-tauri/src/android_assets.rs` materializes the embedded web root into the application data directory because `resource_dir()` returns the URI `asset://localhost/` on Android, and a bounded lightweight Tauri bootstrap waits for the native static server instead of duplicating the product frontend as the initial asset route. The 2026-08-09 claim that an Android ARM64 debug APK was built is not reproducible from the current tree: `gen/android` had never been committed and the Rustls change was absent from `Cargo.toml` until 2026-09-23. As of 2026-09-23 the Android lane has been exercised up to the toolchain report, the staged web root, the release keystore path and the `cargo check --package squirrel --lib` host compile of the modified library; no APK has been produced from the current tree yet, because the build host used for that run could not download the crate and Gradle dependencies. Physical-device voice acceptance, on-device startup, and the first real `./run.sh apk` artifact remain required.
