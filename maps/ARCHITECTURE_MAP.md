...
Current viewport reanchor contract (2026-08-11): `surface_runtime.js` is the shared Web/WebView/Tauri/iOS resize lifecycle owner and `surface_size_runtime.js` is its size authority. Initial and observer-only sizing remain host-based. On `window.resize`, `visualViewport.resize`, or `orientationchange`, the resolver compares the visual viewport, layout viewport, and fullscreen host with the preceding surface size and selects the freshly changed source or agreeing pair. This prevents either a stale positive host rectangle on iOS or a stale `visualViewport` retaining Tauri's initial window size. `project_layer_geometry_runtime.js` owns fixed `inset: 0` project/Dashboard shells without viewport-unit dimensions. Main-menu updates remain one-per-frame and panel reflow remains one settled update from stored placement intent; platform shells must not inject delayed DOM height writers or a parallel viewport owner.
...
- Calendar and Dashboard share `resolveWorkspaceDataProjectId()` from `dashboard_workspace_mode.js`. The neutral Dashboard scene is never a Calendar data project, workspace bootstrap rebinds the Dashboard once the canonical project exists, and Calendar cards bypass Dashboard item caching so every presentation derives from `CalendarAPI.listEvents({ projectId })`.
