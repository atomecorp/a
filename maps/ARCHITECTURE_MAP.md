...
- `scripts/export_dom_subtrees.mjs` owns offline extraction of matrix, project, timeline, media-host, and full-app DOM debug exports from a captured DOM/HTML file. It writes only under `temp/` so generated subtree reports remain transient diagnostics, not source-of-truth artifacts.
