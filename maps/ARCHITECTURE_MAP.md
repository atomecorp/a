...
- Session entry into an authenticated or anonymous workspace uses `eVe/intuition/tools/user_workspace_surface_runtime.js` to load the current project onto the canonical shared project canvas, reveal the existing main ribbon, and open the existing dashboard runtime only after the current project surface is available. This is lifecycle orchestration over the existing project/menu/dashboard APIs, not a separate dashboard renderer, canvas, DOM dashboard, or menu state owner.
...
- Project wallpaper/background rendering is also centralized in `eVe/domains/rendering/`. `eVe/user/background.js` owns user preference loading, protected media authorization, and consumption of `eve:profile-preferences-updated` for immediate background projection, but it must remain importable without starting the runtime; `eVe/eVe.js` explicitly calls `startUserBackgroundRuntime()` after `eve.user_background` imports, and a missing shell `#view` is an explicit startup error. Generated background parameters come from `eVe/intuition/tools/background_random_runtime.js` when generated backgrounds are used, generated pixels come from `user_background_pattern_renderer.js`, preference-to-surface dispatch comes from `user_surface_background_texture_runtime.js`, and `bevy_surface_background_runtime.js` forwards a non-Atome surface-background patch to Bevy. The explicit user wallpaper `download` action may fetch a remote image and must then route it through the existing background media upload/protected URL path before projection. The active project route uses an opaque Bevy canvas and must not depend on the removed `eve_background_visual_layer` DOM image layer.
