...
Current viewport reanchor contract (2026-08-11): `surface_runtime.js` is the shared Web/WebView/Tauri/iOS resize lifecycle owner and `surface_size_runtime.js` is its size authority. Initial and observer-only sizing remain host-based. Window, visual-viewport, orientation, and Tauri native-viewport signals carry their source into the resolver; a native Tauri size is published in logical CSS units by `platforms/desktop-tauri/src/viewport_runtime.rs` at startup and on host resize. One frame-coalesced update plus one trailing forced reconciliation keeps the canvas, Bevy logical surface, menu and panels in the same resize generation. Browser Winit events only reconcile its window to the canonical Atome surface config and cannot replace that config from an uncapped native DPR. `project_layer_geometry_runtime.js` owns fixed `inset: 0` shells, and platform code must not inject DOM height writers or another renderer/state owner.
