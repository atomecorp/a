...
- Project drag, resize, and text editing are scene intents on the active project path. `drag.move` and `resize.move` stream `gesture_frame` events for realtime sharing, while `drag.end`, `resize.end`, and `text.commit` persist through `set`/commit via `window.Atome.commit` or `window.Atome.commitBatch`; visible DOM text/edit hosts are not part of the active project route.
