...
- Dashboard `+` actions no longer create generic records. News, Store, Monitor, and Goals are explicit no-ops; Calendar and Contacts open their existing eVe panels; Projects enter `transition`, fade out and tear down the dashboard before delegating creation and activation to shared Matrix/Adole helpers, and concurrent project `+` clicks share one in-flight creation. Project item clicks follow the same transition rule: Dashboard and project must never be visible simultaneously, and `loadProjectAtomes`/project activation happens only after the explicit click path begins.
