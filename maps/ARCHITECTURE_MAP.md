...
- Session entry into an authenticated or anonymous workspace uses `eVe/intuition/tools/user_workspace_surface_runtime.js` to load the resolved current project onto the canonical shared project canvas, open the existing dashboard runtime for that explicit project, wait for dashboard records, and only then reveal the existing main ribbon. This is lifecycle orchestration over the existing project/menu/dashboard APIs, not a separate dashboard renderer, canvas, DOM dashboard, or menu state owner.
