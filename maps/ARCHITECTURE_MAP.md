...
- Phase 1 Bevy preparation is represented only by `eVe/domains/rendering/virtual_scene_contract.js`: a renderer-agnostic `AtomeRenderNode` tree/diff/dirty-flag contract derived from canonical Atome records. It is intentionally not a full Virtual DOM, not an ECS, and not a renderer. Bevy integration must consume this contract later instead of competing with it.
