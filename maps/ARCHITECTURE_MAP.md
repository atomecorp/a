...
- The dashboard reserves the measured toolbox height as an excluded bottom band. The toolbox remains the existing product toolbox; dashboard hit-testing stops above that reserved rectangle. Dashboard rendering may place only its dedicated low-layer bottom shadow gradient into the reserved band so the foreground dashboard casts depth onto the project/toolbox area without adding a DOM overlay, second canvas, or alternate renderer.
