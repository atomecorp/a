...
- Dashboard item listing is cached in `dashboard_runtime.js` by `projectId + categoryId`; generic record categories are batch-loaded by `dashboard_data_adapters.js` through a single canonical `state_current` read per grouped dashboard load. Calendar dashboard items come from `CalendarAPI.listEvents()` through the read-only `calendar_api.js` facade, not the panel-building `calendar.js` module, and only dated, non-deleted, unique events are projected; Contacts dashboard items come from the same `collectPeopleDirectory()` source used by the Contacts panel before the local contacts API fallback. Dashboard item normalization deduplicates by normalized item id before focused rendering. Opens and header activation render immediately from cached or empty lists, then hydrate visible categories in the background and rerender only when projection-relevant content changes. The cache is invalidated by explicit refresh, project-change lifecycle, logout, and explicit invalidation; header clicks do not force a synchronous reload.
