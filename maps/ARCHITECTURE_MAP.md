...
- Dashboard rendering keeps an internal disposable record cache and diffs records by id with a targeted structured comparison before calling `updateProjectSceneOverlay(...)`, which merges record removals when unavoidable, record updates, parked-record reuse, and scene effects into one Bevy projection render per logical dashboard render. Dashboard records are non-selectable decorative/runtime records; closing parks them invisible/non-interactive instead of despawning them so repeated opens and category switches reuse Bevy entities. Layout hit-testing in `dashboard_runtime.js` owns dashboard actions, and the toolbox-reserved canvas band is inert while the dashboard is open so project selection cannot target dashboard veil, shadow, or fill records. High-frequency wheel, pointermove, inertia, and transition ticks are coalesced by `dashboard_render_scheduler.js` onto one cancellable animation-frame render request; close cancels pending dashboard frames before parking records so stale header/category renders cannot repaint visible dashboard fragments. Open, close, creation, and final editor states still use immediate renders so probes can verify deterministic scene state.
