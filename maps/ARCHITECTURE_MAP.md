...
Choice palettes (2026-09-18): Choice state stays with the definition owner. A leaf palette choice closes the palette before invocation (direct click or held drag), a slider keeps it open and a nested palette opens its own level. A locked on/off tool takes its palette slot — same icon, same label, active paint — shows its own options in place of the palette's other choices, and is switched off from that slot, which never reopens the palette. Locked tool ids come from their owners as published state and are read by the main menu and the contextual rail; the Mystic keeps the previous resolution (no activeToolIds). Main/contextual renderers share resolveToolChoiceIcon/resolveToolPresentation, the View-mode and lock events only schedule a redraw and are detached on destruction. See API_MAP for selectedChildKey. No choice cache or new event bus is introduced.
