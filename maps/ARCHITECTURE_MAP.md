...
- Flower glass surfaces use the same compositor-owned workspace capture and `BackdropSurfaceMaterial` as the main menu and contextual rail: `backdrop_surface` samples the shared two-pass Gaussian target through a rounded SDF mask and renders each surface plus its content on the presentation layer, so the captured project remains below menu chrome rather than being recursively blurred. `workspace_blur.wgsl` is the attributed Bevy 0.19 separable Gaussian kernel, evaluated from physical fragment coordinates; Atome's `12 px` token maps to a Gaussian support radius after DPR conversion instead of a five-tap image-copy approximation. Shape shadows remain a separate cached Gaussian silhouette operation. The common backdrop shader derives the live surface sampling extent from the actual capture texture dimensions and shared downscale, eliminating copied resize state from resident menu materials. A transform or settled scene diff while a backdrop is visible schedules the existing bounded, entirely asynchronous presentation-prime sequence so capture, both blur passes and presentation publish the final frame after moved content without re-entering Winit. This keeps Flower, side rails and the bottom menu live without enabling continuous idle rendering. Dashboard header side and bottom shadows remain dedicated external gradient textures and do not consume `material.backdrop` or `material.shadow`.
