...
- iOS file propagation is event-driven. `FileSyncCoordinator.syncAll()` runs after initialization, explicit file mutations/imports/captures, explicit sync, or list freshness; no repeating directory-scanning timer exists, and the local HTTP server never synchronizes all roots for ordinary static GET requests.
