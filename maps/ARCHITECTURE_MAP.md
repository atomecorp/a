...
- Project-visible media Atomes on the cleaned canvas route may be opened in Molecule from canonical Atome state and `project_scene_runtime` records without requiring a visible Atome DOM host. Recording Atomes that already exist before final project projection must still be associated to the active project through the canonical commit path before scene rendering, otherwise refresh and project-list reconstruction can diverge. The Molecule timeline payload is derived from Atome properties and remains outside DOM attributes.
