...
5. eVe coalesces the latest frame into one ephemeral assistant visual record. The record mounts once in the panel band and receives one invisible direct-style patch before its absolute animation clock starts; subsequent frames patch independent ephemeral glow/core/shell reveal uniforms in the shared WebGPU compositor, so warmup and coalescence cannot reorder the choreography. When the first procedural SDF mounts, the compositor renders workspace layer 0 into a reusable full-resolution capture, runs ordered horizontal and vertical Gaussian passes on private layers into two more reusable targets, then presents the original workspace and assistant on layer 1. The assistant samples the final blur with bounded pixel-space spherical refraction only inside its shell, while BevyUI remains on presentation layer 1. All three offscreen cameras and technical quads deactivate after the last SDF is removed. Dashboard/project reconciliations preserve the assistant. The same frame loop drives the 420 ms reveal, 180 ms settle, and 320 ms disappearance. Voice begins only after reveal; teardown requests Kira stop immediately, retains interaction ownership until disappearance completes, then releases visual/session ownership deterministically. Any delayed native completion remains scoped to the original `close()` promise and cannot retain `active` or mutate a later session.
