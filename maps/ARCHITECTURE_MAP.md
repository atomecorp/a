...
- Session entry into an authenticated or anonymous workspace uses `eVe/intuition/tools/user_workspace_surface_runtime.js` to reveal the existing main ribbon and open the existing dashboard runtime only after a current project is available. This is lifecycle orchestration over the existing menu/dashboard APIs, not a separate dashboard renderer or menu state owner.
...
- Project wallpaper/background rendering is also centralized in `eVe/domains/rendering/`. `eVe/user/background.js` owns user preference loading, protected media authorization, and consumption of `eve:profile-preferences-updated` for immediate background projection; generated background parameters come from `eVe/intuition/tools/background_random_runtime.js` when generated backgrounds are used, generated pixels come from `user_background_pattern_renderer.js`, preference-to-surface dispatch comes from `user_surface_background_texture_runtime.js`, and `bevy_surface_background_runtime.js` forwards a non-Atome surface-background patch to Bevy. The explicit user wallpaper `download` action may fetch a remote image and must then route it through the existing background media upload/protected URL path before projection. The active project route uses an opaque Bevy canvas and must not depend on the removed `eve_background_visual_layer` DOM image layer.
