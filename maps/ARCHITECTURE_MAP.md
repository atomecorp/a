...
- Dashboard item listing is cached in `dashboard_runtime.js` by `projectId + categoryId`; generic record categories are batch-loaded by `dashboard_data_adapters.js` through a single canonical `state_current` read per grouped dashboard load. The cache is invalidated only by explicit refresh, project-change lifecycle, logout, or same-runtime item creation updates.
