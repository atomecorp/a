...
- Active project selection visuals are rendered inside the same project canvas through Bevy projection diffs. `project_scene_selection_state.js` derives selected ids from the existing selection runtime and `project_scene_runtime.js` marks disposable projection records; the browser Bevy adapter forwards `selected` in style patches; `atome/shared/render_visual_tokens.js` owns the cross-platform design values; and `atome/renderers/bevy-core/src/selection_overlay.rs` renders the visible dotted light-gray contour and uniform 12 px shadow as disposable Bevy entities. No per-Atome DOM outline, host class, selection property, or legacy WebGPU selection overlay is allowed on the cleaned project route.
