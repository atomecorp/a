...
- Bevy integration is represented in JavaScript by `eVe/domains/rendering/render_atom.js`, `eVe/domains/rendering/virtual_scene_contract.js`, `eVe/domains/rendering/bevy_projection_adapter.js`, `eVe/domains/rendering/bevy_media_texture_resolver.js`, `eVe/domains/rendering/bevy_web_renderer_runtime.js`, and `eVe/domains/rendering/bevy_native_renderer_runtime.js`; in shared open Atome Rust by `atome/renderers/bevy-core/`; in native Rust by the optional `platforms/desktop-tauri/src/bevy_backend/mod.rs` wrapper and `platforms/desktop-tauri/src/bevy_backend/bridge.rs`; and in browser/WASM Rust by `platforms/web/bevy-renderer/`. The JavaScript contract remains the renderer-agnostic `AtomeRenderNode` tree/diff/dirty-flag source derived from canonical Atome records, while common Bevy ECS components, render operations, texture handling, render math, and selection overlays live in Atome, not in eVe or in platform-specific crates. Browser mode maps projection into the generated WASM renderer; Tauri and iOS/AUv3 project surfaces dispatch the same disposable Bevy scene and render-op contract through native invoke commands and must not import `atome/src/wasm/squirrel_bevy_renderer.js` for the main canvas. RenderAtom content projection canonicalizes upload and recording media URLs before Bevy mapping, the Virtual Scene assigns dense render layers from actual paint order, the browser adapter maps that projection into explicit Bevy payload data with Rust `i32`-bounded layers, and the browser runtime starts the generated WASM renderer on an existing shared canvas with an `AtomeRenderScene` carrying `nodes` plus visual tokens from `atome/shared/render_visual_tokens.js`, then applies Virtual Scene diffs and surface-size patches through explicit Bevy WASM exports while filtering the known non-fatal WASM runner `unreachable` completion from page error reporting. Initial startup consumes immediately available durable poster/peak cache inputs, while uncached video and audio-waveform nodes remain present as visible Bevy entities and receive decoded textures later through serialized resource operations rather than through a fallback renderer or DOM projection; later spawn diffs use the same pending-media path when video/audio metadata is not immediately available. Surface-size patches cancel pending deferred-media timers and resume queued generation after yielding so resize remains responsive. Runtime selection ids are projected as a disposable `selected` flag and style diff, then shared Atome Rust rebuilds Bevy-owned overlay entities for the light-gray dotted contour and uniform 12 px shadow at the half-step between the selected entity layer and the next paint layer. The Rust Bevy surfaces map explicit projection data into Bevy ECS components and expose Bevy WebGPU renderer entry points for native and web targets. Browser Bevy now owns supported shape plus RGBA texture sprites for text, raster image, SVG, video frames, and audio waveforms; video textures are extracted after a representative seek and muted frame presentation, while waveform textures are derived from canonical peaks when present or from decoded audio source samples when peaks are absent. These surfaces are not a full Virtual DOM and never own canonical Atome state.
...
- iOS AiS companion app: AIS server, native SQLite iOS, WebGPU, native Kira, native Symphonia, and a native Bevy command boundary that must be backed by a linked Rust/Metal renderer before the iOS main project surface can render.
...
- A media record whose source exists but fails texture decoding is not allowed to reject the full initial Bevy scene; the node is skipped and tracked by the Bevy web runtime while valid nodes render.
