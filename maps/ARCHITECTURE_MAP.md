...
- Card of somebody else: read from `collectPeopleDirectory()`; a public-directory user is read-only and shows only the fields its owner shares; a local address-book contact stays editable and is saved through `Squirrel.contacts`. No settings, no session action. The current user's own row unfolds their identity, bio and profile; their settings stay in Home.
