...
- Bevy integration is represented in JavaScript by `eVe/domains/rendering/render_atom.js`, `eVe/domains/rendering/virtual_scene_contract.js`, `eVe/domains/rendering/bevy_projection_adapter.js`, `eVe/domains/rendering/bevy_pending_media_contract.js`, `eVe/domains/rendering/bevy_media_texture_resolver.js`, `eVe/domains/rendering/bevy_web_renderer_runtime.js`, `eVe/domains/rendering/bevy_native_renderer_runtime.js`, and `eVe/domains/rendering/project_scene_bevy_projection_runtime.js`; in shared open Atome Rust by `atome/renderers/bevy-core/`; in native Rust by the optional `platforms/desktop-tauri/src/bevy_backend/mod.rs` wrapper, `platforms/desktop-tauri/src/bevy_backend/bridge.rs`, and the iOS C ABI staticlib wrapper in `platforms/ios/bevy-renderer/`; and in browser/WASM Rust by `platforms/web/bevy-renderer/`. The JavaScript contract remains the renderer-agnostic `AtomeRenderNode` tree/diff/dirty-flag source derived from canonical Atome records, while common Bevy ECS components, render operations, texture handling, render math, and selection overlays live in Atome, not in eVe or in platform-specific crates. Browser mode maps projection into the generated WASM renderer; Tauri project surfaces use the visible Bevy/WebGPU canvas unless the host explicitly declares a presentable native renderer through `window.__ATOME_NATIVE_BEVY_PRESENTABLE__ === true`; iOS/AUv3 project surfaces dispatch the disposable Bevy scene and render-op contract through the native Swift bridge whenever `window.__ATOME_IOS_NATIVE_INVOKE` is available, while the WebView remains the application shell and host surface. Xcode builds and links the iOS Rust staticlib so Swift/Xcode diagnostics can prove `rust_linked=1` and `bevy_core_linked=1`; the staticlib build uses aborting panics, stripped debuginfo, and disabled forced unwind tables so Rust/Bevy metadata does not exceed ld compact-unwind encoding limits in the app or AUv3 targets. The current iOS command boundary reports `presentable=0`, `renderer_mode=linked_no_presenter`, and `ios_bevy_native_not_presentable` after successful Rust scene validation, so the missing Metal/Bevy presenter stays explicit and no Web/WASM fallback is selected. RenderAtom content projection canonicalizes upload and recording media URLs before Bevy mapping, the Virtual Scene assigns dense render layers from actual paint order, the browser adapter maps that projection into explicit Bevy payload data with Rust `i32`-bounded layers, and the browser runtime starts the generated WASM renderer on an existing shared canvas with an `AtomeRenderScene` carrying `nodes` plus visual tokens from `atome/shared/render_visual_tokens.js`, then applies Virtual Scene diffs and surface-size patches through explicit Bevy WASM exports while filtering the known non-fatal WASM runner `unreachable` completion from page error reporting. If a refreshed project view reaches an already-started canvas before the JavaScript project runtime has a previous scene, the browser runtime uses the canvas runtime's last virtual scene as the diff baseline and sends the missing spawn/update operations immediately. The Web Bevy export layer requests an initial redraw, preserves wake requests made before the winit event-loop proxy exists, wakes the Bevy winit event loop when JavaScript queues render operations, and accepts explicit `request_atome_bevy_redraw()` requests from the browser runtime after startup, render diffs, deferred media resource updates, and surface resize, so reactive rendering presents startup scene changes, newly created Atomes, mutations, texture resources, and surface-size changes without waiting for the next pointer or keyboard event. Project refresh loading must await the project scene projection before marking `loadProjectAtomes(...)` complete, so the recent-load cache cannot freeze a loaded-but-not-presented scene. Initial browser startup consumes immediately available durable poster/peak cache inputs; image nodes resolve their disposable RGBA texture before Bevy spawn, while uncached video and audio-waveform nodes remain present as transparent pending Bevy entities and receive decoded textures later through serialized resource operations rather than through a fallback renderer or DOM projection. Safari/WKWebView video texture and poster generation attach the temporary video element to the hidden in-flow decode pool in `eVe/domains/media/shared/video_decode_pool_runtime.js` before seek/play/readback, because detached or zero-sized videos can fail to rasterize a frame until later user interaction. Native startup and native diffs do not run the browser RGBA texture resolver; they send media sources, text, and waveform peaks to Swift/Rust so the native renderer owns platform media loading and Xcode receives native diagnostics even when a browser `img.decode()` would fail. Surface-size patches cancel pending deferred-media timers and resume queued generation after yielding so resize remains responsive. Runtime selection ids are projected as a disposable `selected` flag and style diff, then shared Atome Rust rebuilds Bevy-owned overlay entities for the light-gray dotted contour and progressively faded 12 px shadow blur at the half-step between the selected entity layer and the next paint layer. The Rust Bevy surfaces map explicit projection data into Bevy ECS components and expose Bevy WebGPU renderer entry points for native and web targets. Browser Bevy now owns supported shape plus RGBA texture sprites for text, raster image, SVG, video frames, and audio waveforms; video textures are extracted after a representative seek and muted frame presentation, while waveform textures are derived from canonical peaks when present or from decoded audio source samples when peaks are absent. These surfaces are not a full Virtual DOM and never own canonical Atome state.
...
- Project drag, resize, and text editing are scene intents on the active project path. `surface_runtime.js` owns client-to-logical coordinate conversion before hit-testing so browser/device-pixel canvas scaling does not offset drag, resize, caret, or selection targets, and it handles native `dblclick` separately from `pointerdown` so browser click-count differences cannot block text edit re-entry. `surface_text_pointer_runtime.js` detects text edit re-entry, routes active text pointer sessions, gives active text caret/selection priority over resize except on the bottom-right corner, and marks active text targets from `project_scene_text_edit_state.js` without using DOM state. `surface_pointer_runtime.js` owns homothetic resize target calculation for active project single-selection and multi-selection gestures, deriving one uniform scale from canonical scene bounds before emitting `resize.move` or `resize.end` props; inactive text resize emits a recalculated `text_style.font_size` for crisp Bevy redraws, while active text resize emits only geometry. `project_scene_gesture_runtime.js` coalesces `drag.move` and `resize.move` bursts to animation-frame cadence, updates only disposable project scene records for live feedback, and batches realtime `gesture_frame` events for sharing. `project_scene_text_runtime.js` owns active project text edit sessions, routes hidden keyboard input through `hidden_text_service_runtime.js`, projects live multi-line text/cursor/selection feedback into disposable Bevy scene records, applies selected-range `rich_text.spans` formatting for bold and color, maps canvas pointer coordinates back to text indices, and commits clean text, rich spans, and measured size through `text.commit`. `project_scene_invalidation_runtime.js` owns selection, video natural-size, and project-audio progress invalidation listeners, while `project_scene_direct_transform_runtime.js` owns direct Bevy transform patch construction for live gesture feedback. `drag.end`, `resize.end`, and `text.commit` persist through `set`/commit via `window.Atome.commit` or `window.Atome.commitBatch`; visible DOM text/edit hosts are not part of the active project route.
