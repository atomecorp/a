...
2026-09-25 Mystic centre shadow ownership: the Atom handle's box shadow is a BevyUI style patch — one `updateTreeMotion` op carrying `shadow` and nothing else — on the mounted centre node, never a shader uniform, a Rust record or a second surface, so the geometry and the opacity the cascade owns stay untouched. `createMysticShadowFade` (`eVe/intuition/ribbon/bevy_ui_mystic_motion.js`) owns the 330 ms ease-out alpha, its cancellation generation, its one-op `clear()` and the reduced-motion shortcut; `eVe/intuition/ribbon/bevy_ui_mystic_runtime.js` starts it only on the `opening → open` transition (cascade completion, a resize that promotes the menu, and after `animateReplacement` reopens), clears it before any closing cascade and in `resetState`, and re-applies the live alpha after a re-render while the menu is open. The centre geometry is a disc by construction (`resolveMysticCenterMetrics` in `eVe/intuition/mystic/mystic_layout.js`, `size / 2`), the shadow value is read from the skin (`mystic_skin.js` → `MYSTIC_CENTER_SHADOW`), and the native renderer derives the shadow's radius from the node's `border_radius`, so the shadow follows the circle. The tiles, the SDF shader and the uniforms are unchanged. Evidence: eVe/documentations/MYSTIC_CENTER_DISC_SHADOW_VALIDATION_2026-09-25.md.

2026-09-25 Active-tool ownership repair: selected-object definitions cannot replace a live tool target or capture source. Global project transport and selected-media playback remain separate canonical owners. The rail projection reuses the existing Bevy tree and recording pulse, keeps pinned stop controls available during ACL loading, and suppresses them on Dashboard suspension. Activation order is observed before render coalescing. No DOM state authority or alternative renderer added. Owner: `eVe/intuition/runtime/eve_intuition/atome_contextual_edit_projection_runtime.js`.

2026-09-23 A terminal renderer failure names its cause on screen instead of leaving a mute blank page. The product UI is the Bevy/WebGPU canvas, so when that canvas cannot start every other surface is missing too: `eVe/intuition/runtime/eve_intuition/boot_failure_surface.js` is the single owner of the last paintable state, a plain-DOM, non-renderer surface deliberately outside the design system for the same reason the Rust `TAURI_LOCAL_HTTP_FAILURE_SCRIPT` is. The boot keeps its existing lifecycle authority: `isTerminalWorkspaceBootFailure()` decides that the attempts are over, `boot_runtime.js` records the reason and stops the retries through `settleTerminalWorkspaceBoot()`, and only then may the surface paint — and only for a renderer failure it can prove (`bevy_renderer_wasm_panic:`, `bevy_renderer_start_failed_terminal:`), never for `remote_account_not_provisioned`, whose login surface still carries the fixing action, nor for any transient failure the boot still retries. The surface is idempotent, paints once, never replaces a live surface, and owns no state beyond the failure text it displays. Evidence: `tests/eve/boot_failure_surface_contract.test.mjs`; the module is inside the bundled critical graph. The real Android/emulator run stays To verify.
...
2026-09-22 Dashboard rail two-tier tools: each rail header splits into an upper two thirds band (icon + rubrique label, the category filter) and a lower third band (`Nouveau`, the creation surface). `dashboard_layout.js#dashboardHeaderBands` owns the split for both `createDashboardLayout` (`header_filter_rect` / `header_new_rect`, emphasis following `filteredCategoryId`) and `hitTestDashboardLayout` (`kind: 'header'` with `zone: 'filter' | 'new'`), and `dashboard_header_records.js` derives the icon/label bounds, the `header_separator_*` line and the `header_new_*` label from it. `dashboard_environment.js#itemsForRender` cascades the filtered category from its own lane and empties the others; `dashboard_bevy_ui_runtime.js` keeps `state.filteredCategoryId` exclusive with the creation `state.activeCategoryId` and toggles the filter with a targeted category reload, without a transition module or a second scroll engine. Evidence: eVe/documentations/FRAMEWORK_STATE.md.
...
Undo resolution tracks active source events within each transaction. Selection-scoped commands carry only their Atome IDs through the existing WebSocket/vault history route; project-scoped commands carry every remaining target. Redo retains the same target subset. Mystic pointer ownership terminates an armed DOM or WebGPU manipulation session immediately, without changing the canonical selection.

Lasso retains rectangle/project/selection IDs in runtime context, not DOM attributes. Its waiting rectangle hands activation to the standard Mystic owner; Copy/Paste consume that captured selection and capture uses the existing Bevy preview compositor instead of cloning DOM. Multi-selection Mystic uses the same explicit selection payload. Structured preview multi-selection/pinch parity and physical runtime acceptance remain to verify; this entry is not full cross-runtime acceptance.
...
Selection treatment propagation (2026-09-16): the existing selection-style
mutation owner resolves a selected structural Molecule from canonical state,
loads its project records once, and expands `parent_id` ancestry recursively to
live non-Molecule descendants. Nested Molecules are traversal nodes rather than
paint targets, preserving their transparent structural role. Couleur, Font and
inactive Draw opacity reuse that resolver; Size retains the existing recursive
transform planner. No DOM ancestry, renderer state, public API or second
mutation path participates.
...
canonical preference in parallel with project loading, then applies it once
through `restoreProjectViewMode` after the authoritative scene load. The load is
marked as prepared so its internal restore cannot race that owner. An explicit
choice made after prefetch always wins, including when the choice is already
Natural in memory but must replace a durable Matrix value. `project_data.js`
owns one global latest-wins activation generation;
...
canonical Atome Mystic context, preserving the existing Couleur/Font tools and
...
Tauri mobile status: `platforms/desktop-tauri/gen/apple` and `platforms/desktop-tauri/gen/android` are the generated mobile applications, and the vendored STT plugin supplies their native speech-recognition implementations. The iOS Rust target compiles successfully. The Android lane is owned by `scripts/android/apk.sh`, dispatched from `./run.sh apk` through `scripts/setup/service_commands.sh`: it resolves the JDK, the SDK, the NDK and the Rust targets idempotently, stages `platforms/desktop-tauri/gen/android-webroot`, and is the only caller of `tauri android build`; `--isolated-home <dir>` relocates the cargo, Gradle and Android user caches under `<dir>` for hermetic and CI builds. `platforms/desktop-tauri/tauri.android.conf.json` sets API 29 minimum, API 36 target, release resource bundles to an empty list (Android resources are not file paths), and points `frontendDist` at the staged web root; API 29 is the floor because the NDK sysroot only ships `libamidi.so`, linked by `midir`'s Android backend, from API 29 onward, and `gen/android/app/build.gradle.kts` carries the same number. Reqwest uses Rustls rather than a host OpenSSL dependency. `platforms/desktop-tauri/src/android_assets.rs` materializes the embedded web root into the application data directory because `resource_dir()` returns the URI `asset://localhost/` on Android, and a bounded lightweight Tauri bootstrap waits for the native static server instead of duplicating the product frontend as the initial asset route. The 2026-08-09 claim that an Android ARM64 debug APK was built is not reproducible from the current tree: `gen/android` had never been committed and the Rustls change was absent from `Cargo.toml` until 2026-09-23. As of 2026-09-23 the Android lane compiles and links end to end from the current tree: `cargo check --package squirrel --lib --target aarch64-linux-android` and the `cargo build --lib` driven by `tauri android build` both finish clean, and `libsquirrel_lib.so` reaches `platforms/desktop-tauri/gen/android/app/src/main/jniLibs/arm64-v8a`. Two defects were repaired to reach that point: the `src/server` dependencies (`rusqlite`, `bcrypt`, `uuid`, `chrono`, `jsonwebtoken`, `rand`, `sha2`, `hex`, `reqwest`, `zip`, `tempfile`, `dotenvy`) had been declared under the macOS target table, which produced 223 unresolved-crate errors for the Android target, and the API 26 floor predated the `libamidi.so` requirement. The first real `./run.sh apk` on a host able to bind local sockets assembled the debug APK on 2026-09-23: `platforms/desktop-tauri/gen/android/app/build/outputs/apk/universal/debug/app-universal-debug.apk`, 504 912 620 bytes, sha256 `8dee010baa3979368d67342b53d4048b47c54b098992cabfbdadc76a7e667306`, with `aapt2` reporting `com.squirrel.desktop.debug`, minSdk 29, targetSdk 36, `native-code 'arm64-v8a'`, the `INTERNET` and `RECORD_AUDIO` permissions and `debuggable=true`, and `apksigner` confirming the local Android-debug signature. AGP nests every artifact under its Gradle flavor, so artifact discovery reads the CLI's own `Finished N APK at:` report, which the CLI writes to stderr while Gradle diagnostics go to stdout, and therefore captures both streams into `temp/apk-build.log` and keeps `temp/apk-build.log` and `temp/apk-build-marker` under the git-ignored `temp/` for its filesystem fallbacks; the lane previously searched `outputs/apk/debug` and failed after a successful build. The lane removes this configuration's previous APK or AAB before invoking the CLI, because AGP re-packages into an existing output without truncating it: the repackaged debug APK measured 994 683 526 bytes carrying 489 765 844 dead bytes, against 504 916 716 bytes with none once the stale file was gone, and the release lane produced 229 264 109 bytes signed by the keystore it created under `~/.atome/android/`. Three consecutive real `./run.sh apk` runs now finish with `[apk] Done`, and the CLI's `Finished N APK at:` report reaches `temp/apk-build.log`, which captures both streams because that report is written to stderr. Visible identity and app icon now have one owner per platform, and the technical identifiers deliberately keep their historical names. `platforms/desktop-tauri/tauri.conf.json` carries `productName` and the main window title `atome`, and `platforms/desktop-tauri/Cargo.toml` declares the `[[bin]]` target `atome` and the `default-run` that marks it as the main binary, so the macOS bundle is `atome.app` with `CFBundleName`, `CFBundleDisplayName` and `CFBundleExecutable` all `atome` and the development runtime is `target/debug/atome`; `bundle.resources` maps every tree under one `project/` root (`project/atome`, `project/eVe`, `project/node_modules/rubberband-wasm/dist`, `project/version.txt`) because `tauri-build` copies that map into the profile directory on every build, where a destination key equal to the binary name collides with the binary itself: macOS reads the application name from the executable whenever the app runs outside its bundle, which is what `tauri dev` does; the Android launcher label lives in `platforms/desktop-tauri/gen/android/app/src/main/res/values/strings.xml` (`app_name` and `main_activity_title`, both `atome`) because `tauri android init` never rewrites an already generated project. One artwork source drives every icon: `atome/src/assets/images/logos/atome.svg`. Two masters are derived from it and each platform consumes the one its shape requires. The mobile master is opaque and alpha-free, because Apple rejects a transparent application icon: the Tauri CLI expands it into the Android `mipmap-*` launcher set and a `mipmap-anydpi-v26` adaptive icon over the white `@color/ic_launcher_background`, and into the iOS set `platforms/ios/atome-auv3/application/Assets.xcassets/AppIcon.appiconset/` (the CLI also writes its own `platforms/desktop-tauri/icons/ios/` copy, which the Xcode project does not consume). The desktop master is produced by `temp/build_macos_icon.mjs`, which insets the artwork in a transparent rounded square whose geometry is measured on Apple's own application icons at a 1024 px canvas: body 856, margin 84, corner radius 188. macOS does not mask application icons, so the shape must be part of the artwork; the previous opaque full-bleed square rendered as a hard-edged white tile in the Dock. `tauri icon` expands that master into `platforms/desktop-tauri/icons/` (`.icns`, `.ico` and the PNG sizes). Application id `com.squirrel.desktop`, Cargo package and library `squirrel` / `squirrel_lib`, the `System.loadLibrary` call in `platforms/desktop-tauri/gen/android/app/src/main/java/com/squirrel/desktop/generated/Rust.kt`, the `~/.atome/android/squirrel-release.jks` keystore, the Android theme `Theme.squirrel`, the `squirrel` service name and the `squirrel/Data`, `squirrel/Uploads` and `squirrel/project` data directories are intentionally unchanged: renaming them would orphan the installed application, its signing key and its local database. On-device startup and physical-device voice acceptance remain required. The same lane owns the optional local emulator behind `--emulator`, which is the only path that produces Android runtime evidence on this host: it installs `platform-tools`, the emulator and one system image into a project-local SDK root under the git-ignored `temp/` tree, keeps the AVD, its emulator state and the logcat captures under that tree with nothing versioned and nothing global touched, boots the AVD with 4 GB of RAM, reuses the artifact the same run built and verified, and records logcat from before `am start` so a startup failure is captured. Two undocumented requirements are encoded there: `avdmanager` ignores `ANDROID_HOME` and `ANDROID_SDK_ROOT` and derives its SDK root from the grandparent of the `com.android.sdkmanager.toolsdir` property, which the lane overrides through `AVDMANAGER_OPTS`, and the emulator rejects a candidate SDK root that has no `platform-tools` subdirectory. That lane reproduced the on-device startup failure locally: `java.lang.UnsatisfiedLinkError: dlopen failed: cannot locate symbol "__cxa_pure_virtual"` is raised by `System.loadLibrary("squirrel_lib")` because the native library statically carries Oboe's C++ objects while leaving 32 C++ runtime symbols undefined, declares no `DT_NEEDED` entry for `libc++_shared.so` and the APK ships no `libc++_shared.so`, so the Android application cannot start until that runtime is linked and packaged. The Android application is therefore not functional yet, and correcting that linkage is a decision that still has to be taken before on-device startup and physical-device voice acceptance can be verified.
...
- Panel Lab introduces exactly one new primitive component type at a time after
  ...
  actions and standard/palette menu tools consume the same contract; Mystic
  overrides only the standard radius with its preference-controlled tile roundness. Its label is a vertically centered right-side
...
- The Calendar header is a disposable local-time projection, not Calendar business state. `dashboard_header_records.js` formats its time/date on each render, and the Dashboard lifecycle owns exactly one next-minute timeout while active. Timer wakeups rebuild through the canonical BevyUI diff; close, suspension, destruction, and superseding opens release the timeout before any later projection can run.
- Dashboard item label editing is a runtime interaction over the existing project canvas. `dashboard_bevy_ui_runtime.js` owns long-press detection, editable field hit resolution, and prevents the release click from opening an item; `dashboard_item_text_fields.js` is the single source for project/contact title fields and calendar title/start-date fields; `dashboard_label_edit_runtime.js` owns hidden text capture, Enter/Escape ownership, and Bevy-projected draft/caret/selection state; `dashboard_label_persistence.js` commits labels only through Matrix project rename, CalendarAPI event title/start update, or local Contacts update. Active project rename also persists the current-project name used by reload bootstrap. Only local `eve_contacts_local` contacts arm label editing; non-local contacts remain click-openable and direct persistence calls reject explicitly. The mystic context resolver must allow Dashboard item hit zones to open the radial context menu by right-click or long press, while the toolbox-reserved band and non-item Dashboard chrome remain blocked. A Mystic Rename invokes that existing label editor; a Dashboard project Mystic Delete invokes `project_data.js` and refreshes only the `projects` category. If the mystic takes the pointer, Dashboard label-edit long press must yield to that mystic pointer ownership.
...
- Dashboard layout precomputes lane item starts and uses bounded lookup for visible item records. Lanes, headers, and cards retain their full canonical height and square/double-width geometry even when partially visible; separate visible intersections bound hit-testing, while disposable `properties.clip` metadata flows through the existing Virtual Scene and Bevy projection so the viewport crops records without rescaling them or adding DOM. Each root surface keeps the viewport clip for its standard exterior shadow, while its icon/media/text children are clipped to that cell's exact visible intersection; content therefore exits the screen with its owner and cannot overlap the next row or consume the stable lane gap. In Bevy core, the same clip crops both sprites and backdrop-glass `Mesh2d` geometry; backdrop UVs remain referenced to the full logical block and the resident mesh handle is mutated in place, preventing both stretch and per-frame asset accumulation. Visible card, media, backdrop, and label record ids are tied to stable category+item identities rather than visual slot numbers, so scroll and focused-category changes move existing items through transform/style diffs and only spawn/despawn records as they enter or leave the bounded visible window. The bottom reserved band uses the measured top of painted maintoolbox controls/surfaces when they are visible; the generic minimum band is used only when no toolbox surface is measurable.
- Initial dashboard open has no active category so each lane shows its own category color/data. Every rail header is a two-tier tool: the upper two thirds carry the icon and the rubrique label and filter the category, the lower third carries the `Nouveau` label and keeps the creation behaviour. Filtering is instant — no spread transition and no `focus_spread_*` record exist — and the filtered category's unique items cascade from their own visual lane across the other lanes through stable category+item records, so missing category data hydrates into the same item identities instead of forcing an empty intermediate projection. The filtered entête emphasis replaces the old uniform focused backdrop, and a second short press on any upper band, the filtered one included, restores the base multi-colour overview. Filtered item hit-testing preserves the item category as the action owner even when the item is displayed on another visual lane, and horizontal scrolling remains the layout's per-lane scroll.
- Dashboard no longer owns `+` creation actions from focused entêtes. There is no full-height plus strip, no plus hit-test route, and no no-op plus branch. A short press on the lower third of a header opens the in-place creation grid (the upper two thirds filter the category instead) (see the creation-mode entry); a creation opens the new project through the Dashboard item path, and calendar/contact cells open only registered BevyUI panel surfaces and must preserve the workspace main menu. Project item clicks still follow the same transition rule: Dashboard and project must never be visible simultaneously, and `loadProjectAtomes`/project activation happens only after the explicit project-item click path begins. Project activation owns the recovery of active project layer visibility, `eve_surface_project` pointer interactivity, workspace `project` mode, and menu visibility through the existing project/render/Mystic routing, not through Dashboard DOM cleanup.
...
- `workspace.startup_view` is a persisted user preference, not a boot heuristic: `eVe/domains/dashboard/workspace_surface_preference.js` owns both sides of it, `dashboard_workspace_mode.js` records the surface on every real Dashboard/project change, and `project_bootstrap.js` reads it to decide whether a valid saved project resumes or the Dashboard is presented. The value round-trips through the canonical profile API (`preferences.workspace`) and is mirrored to `window.__eveProfilePreferences`, so a restart, a reload, or another platform presents the surface the user actually left; no platform-specific store, second session record, or per-runtime default may reintroduce a fixed project-or-Dashboard route.
...
- Mystic, main-menu, contextual-rail, panel, liquid and assistant glass surfaces share one compositor-owned workspace capture and one quarter-resolution GPU mip pyramid. `workspace_backdrop.rs` renders project content into a single-mip copy source; `workspace_blur.rs` copies it into the pyramid and invokes Bevy 0.19's mip-generation pass after the capture view and before presentation. Each material converts its own public logical blur radius and DPR to an independent LOD and sampling footprint, so creation order cannot overwrite another surface's radius. The shaders derive screen UVs from `view.viewport` through Bevy's canonical `frag_coord_to_uv` and compensate the low-LOD mip sampling phase; neither texture dimensions nor a cached WebView size reconstruct presentation coordinates. A scene mutation, direct transform, animated resource frame, resize or scene transition requests one coalesced reactive redraw; capture, pyramid and glass are encoded in that generation, with no delayed redraw prime or continuous idle loop. Resize retargets the capture camera and every consumer to one new image generation before releasing the old resources. Shape shadows remain a separate cached Gaussian silhouette operation. Dashboard header side and bottom shadows remain dedicated external gradient textures and do not consume `material.backdrop` or `material.shadow`.
- Liquid Mystic/main-menu surfaces advertise their non-zero blur through `material.procedural.background_blur_px`; the renderer folds that field into the same active-backdrop decision as `material.backdrop`. Leaving Dashboard while liquid glass is resident consequently refreshes the shared capture and does not create a Dashboard-specific scene or shader path.
...
eVe is the closed product layer. It owns product UI, private tools, visual composition, panels, Matrix, ribbon, mystic, Finder-facing product workflows, Molecule/MTraX product behavior, branding, product stores, and closed runtime composition.
...
- Browser events such as left click, double click, drag, resize, keyboard routing, mystic menu routing, media transport, and MTRAX opening must recover only the `atome_id` from the DOM id and then consult the owning registry/correspondence table to decide behavior.
...
- eVe is closed for product experience, product UI, private tools, panel chrome, Matrix/ribbon/mystic/Finder workflows, product stores, branding, Molecule/MTraX behavior, and closed composition.
...
- Generic system UI controls such as Button, Slider, Input, and Console belong to Atome when the control contract is product-neutral; eVe may compose them for ribbon, projection, mystic, footer, toolbox, and panel surfaces but must not keep a parallel control source of truth.
...
- Product UI shell, panels, tools, matrix, menu, ribbon, mystic, projection, and Finder-facing workflows.
...
- Project media import intent is centralized in `eVe/intuition/runtime/project_media_import_runtime.js`; mystic menu import and capture import must consume that runtime and must let `project_drop.importFilesToProjectViaCreator` remain the canonical media upload/project creation path.
- Mystic contextual routing is owned by `eVe/intuition/mystic/`: project-canvas Atome targeting reads the active project scene through lazy hit testing at menu-open time, active selection compatibility is computed outside the DOM, and mixed-kind multi-selection intentionally exposes only the `info` tool until a complete compatible batch-tool strategy is defined. A completed Mystic long press owns bounded terminal-event guards: its native `pointercancel` is stopped before the captured BevyUI root can interpret it as `cancel`, and its derived `contextmenu` plus short compatibility-click window are consumed. Its pointer lock ends synchronously once that terminal event has crossed the earlier Bevy capture listener, so a subsequent press/release on a visible petal can always dispatch `activate`. Any subsequent primary `pointerdown`, including at the release point, is a new user gesture and uses the ordinary Mystic close path. The Mystic’s full-surface BevyUI root owns blank-space dismissal directly: `press` closes, while `release` stays passive unless the hit-test resolves a visible tool. During the opening animation, each petal’s center-origin hit is also dismissal rather than an invisible activation; only settled visible petals receive their normal activation handler. A genuine right-click is distinguished by its preceding secondary `pointerdown`, so it retains the close contract when Mystic is already open and otherwise opens Mystic on a valid target. Dashboard exposes its item/label arbitration through `readMysticTargetAtPoint`, while the global Mystic gesture runtime owns both the secondary click and the item hold across every painted card child; the bounded label hold remains reserved for rename. The Mystic lock is a single shared contract in `context_pointer_lock.js`: `surface_runtime.js` cancels an active scene drag, while `bevy_ui_runtime.js` discards the pending `release`/`activate` of the control that received the initial primary press. This prevents that underlying control from closing a Mystic opened by the same long press. Visible Mystic interaction is owned by `eVe/intuition/ribbon/bevy_ui_mystic_runtime.js` and `bevy_ui_mystic_model.js`; it uses the shared BevyUI tree, captured pointer sessions, and the `mystic` workspace layer. `bevy_ui_mystic_motion.js` is the only opening/closing choreography owner: one interruption-safe animation-frame loop projects all petal transforms and content opacity through `updateTreeMotion` with a shared straight radial progress. It mounts no temporary procedural record, so the tree contains only actionable petals. The four phases are `closed`, `opening`, `open`, and `closing`; reopening or closing samples the current frame instead of restarting or spawning another loop. `Back` is a submenu-only control, so root pointer movement over the center cannot trigger navigation or close the menu. The retired DOM Mystic renderer is not an alternate path.
...
- Tools that remain lit once started declare their residence exactly once, in `eVe/intuition/tools/core/active_tool_registry.js`: `rail` for the record/playback family the contextual rail already publishes, `menu` for the operative creation tools (text, drawing, code, page, placeholder, template). Their lit state and their lit presentation stay on the surface that owns the slot, so no second command system and no presentation copy exist. A rail-resident lit tool is displayed once: `buildAtomeContextualEditTree({ activeSlots })` removes it from the ordinary level and pins its case against the rail's bottom edge, above the Atome handle, non-scrolling, the ordinary stack pushed up by exactly the reserved height. A menu-resident lit tool keeps its ribbon slot (its locked palette child) and never enters the rail. The unique `createRecordingIndicatorPulse` is synchronized by the rail and by the ribbon on the slots that display a lit tool, and cleared at stop; a view mode that paints itself active is not a tool and never pulses.
...
- `bevy_ui_native_event_bridge.js` owns the bounded native BevyUI event queue/drain handoff. Browser and native UI events are dispatched to their existing JavaScript handlers in the same turn when their canonical renderer export has already queued them; polling remains only for renderer-originated later events. BevyUI text-entry drafts remain disposable UI state: each keystroke updates the existing text component/resource incrementally (`update_node_text` for native BevyUI, project-record text-resource diff for the visible overlay), while the caret continues through direct motion and canonical application state changes only on the existing input handlers.
- Project wallpaper/background rendering is also centralized in `eVe/domains/rendering/`. `eVe/user/background.js` owns user preference loading, protected media authorization, stale-profile protection for local background preference updates, and consumption of `eve:profile-preferences-updated` for immediate background projection, but it must remain importable without starting the runtime; `eVe/eVe.js` explicitly calls `startUserBackgroundRuntime()` after `eve.user_background` imports, and a missing shell `#view` is an explicit startup error. Protected background reads select their authorization token from the resolved media owner: local Axum URLs use the local token and absolute Fastify URLs use the cloud token. Generated background parameters are owned by the Background panel modules, generated pixels come from `user_background_pattern_renderer.js`, preference-to-surface dispatch comes from `user_surface_background_texture_runtime.js`, and `bevy_surface_background_runtime.js` forwards a non-Atome surface-background patch to Bevy. The Rust background owner renders image wallpapers on a viewport-sized quad and updates a centered source UV crop on every surface resize, implementing `cover` without changing the source aspect ratio. Applied-background identity is scoped to the canvas plus its current WASM renderer instance, so renderer replacement on a reused Browser, Tauri, or iOS surface reapplies the latest background while duplicate updates to the same renderer remain suppressed. `surface_background_defaults.js` and the CSS custom property `--eve-default-surface-background` define the shared fallback project background color for the HTML shell and Bevy color-only surface patch; that color is the very light grey `#f5f5f7` (RGB 245/245/247), and the default solid/no-pattern route must not manufacture a transparent texture. The user wallpaper is a Dashboard surface only: `user_surface_background_texture_runtime.js` reads the published `window.__eveWorkspaceMode.mode` and subscribes to `eve:workspace-mode-changed` through a local event constant, exactly like the sibling video decode source runtime, so the rendering layer never imports the Dashboard module graph; in `project` mode it publishes one colour-only patch (`signature: project-default-surface-background`, `mode: color`, `fit: cover`, the shared default RGBA) with no `texture` and no `sourceUrl`, and resets its image dedupe memory so returning to the Dashboard republishes the wallpaper instead of being skipped as a duplicate. `dashboard` and `transition` keep the unchanged image or generated payload, so the clean surface appears only once the project is really revealed. The project surface colour is canonical state, not renderer state: `user_surface_background_texture_runtime.js` subscribes to the shared `atome:changed` bus event on `core/event_bus.js` and reads the project record through the public read-only API `window.Atome.getStateCurrent()` instead of mirroring it into a second store, takes the first declared property among `background`, `backgroundColor` and `bg`, converts it with the shared `readSurfaceBackgroundRgba()` from `surface_background_defaults.js` (hex `#rgb`/`#rrggbb` or `rgb()`/`rgba()`, alpha clamped to 0..1, an unparsable value resolves to `null`), and republishes the project patch as `signature: project-color-surface-background:<projectId>:<r>:<g>:<b>:<a>` with `mode: color` and no texture and no source URL. The colour projection is gated on the published workspace mode `project` and on the project id still being current after the asynchronous read, so a late answer never repaints another workspace, and a project that declares no readable colour keeps the shared `project-default-surface-background` patch. The colour value is never written here: the `couleur` tool of the project contextual rail writes `{ background, backgroundColor, bg }` through `applyColorToSelection` and the canonical commit pipeline, and the rail itself is the project-background toggle owned by `project_layer_routing.js`. A profile owner without a stored `preferences.background` resolves to the bundled `assets/images/Background/eVe.png` through `defaultUserBackgroundParams()` for the live params and `window.eveBackground.defaults`; that resolution is painted from the params only and is never written to the profile, any stored background keeps priority, and the first wallpaper the user picks replaces the default through the existing Background panel save path. The explicit user wallpaper `download` action delegates remote image retrieval to the Fastify-owned `downloadRemoteWallpaper` upload/protected URL path before projection; local Axum/Tauri uploads must not own that remote-download route. The active project route uses an opaque Bevy canvas and must not depend on the removed `eve_background_visual_layer` DOM image layer.
...
- Fastify is the identity/session/routing/socket orchestrator. A supervised
  ...
  jail implementation. The IPC channel is also the worker lifetime authority:
  disconnecting it terminates the worker even if Fastify was killed without a
  graceful signal. Provider shutdown has a bounded grace period followed by a
  targeted force-stop, preventing orphan workers from exhausting the host
  process limit.
...
- eVe product UI, panels, tools, menu, Matrix, ribbon, mystic, Finder workflow: `eVe/intuition/`.
...
- The Dashboard Contacts creation cell is an interaction-only bridge into
  this existing surface: it calls the registered panel open API. It cannot load Contacts data, create a contact, or
...
- `SyncEngine` connects to the configured Fastify `/ws/sync` endpoint even when Tauri owns local persistence. `squirrel:directory-invalidated` is the single remote-directory refresh contract; active Contact, Dashboard Contacts, Finder Contacts, and Communication surfaces subscribe through `public_directory_events.js` and release their listeners with their surface lifecycle. The Dashboard Contacts category additionally follows the local contact population contract in `contact_change_events.js`, so a locally edited contact name or photo repaints its vignette without an application restart; Finder Contacts and Communication read the public directory only and stay on the directory contract.
...
Choice palettes (2026-09-24, superseding the 2026-09-18 icon borrowing): a palette slot keeps its OWN label and its OWN icon whatever is chosen inside it — the choice is never projected onto the slot. Choice state stays with the definition owner: a locked on/off tool still takes its palette's place in the level (it shows its own options instead of the palette's other choices and is switched off from that same slot, which never reopens the palette), and locked tool ids still come from their owners as published state read by the main menu and the contextual rail through resolveToolActiveChild. The state a palette used to borrow is now read inside the level: projectViewPlaybackModeDefinition marks the child in force with active: true and atome_contextual_edit_model paints it through interactionFeedback. Main/contextual renderers share resolveToolPresentation without activeToolIds, the lock channel only schedules a redraw and is detached on destruction, and the main menu no longer listens to PROJECT_VIEW_MODE_CHANGED_EVENT. No choice cache or new event bus is introduced.
...
### 2026-09-16 — Dashboard plein écran et propriétaires

Le Dashboard reste une projection jetable dans le tree BevyUI et le canvas partagé. Le projet peut rester résident, mais `surface_base` opaque et `surface_image` en `cover` le masquent totalement. Le propriétaire de visibilité suspend puis démonte la projection du menu principal, y compris sa surface liquide, afin qu’aucun rendu différé ne puisse la repeindre ; l’activation canonique d’un projet la remonte après la présentation effective. Il n’existe ni DOM Dashboard, ni second canvas, ni renderer, ni état métier parallèle.

La géométrie appartient à `dashboard_layout.js`, le cycle à `dashboard_bevy_ui_runtime.js` et `user_workspace_surface_runtime.js`, la visibilité du menu à `workspace_main_menu_visibility.js`, le fond à `bevy_surface_background_runtime.js`, et le verre au matériau commun. La météo est une projection externe jetable ; sa seule donnée durable est la localisation validée du profil. Calendar, Contact et Project gardent leurs propriétaires de création par appui long. Store n’est plus une rubrique Dashboard mais son outil et ses données restent intacts.

Le fond profil reste dans la couche de capture du workspace, tandis que les surfaces vitrées et leur contenu sont projetés dans la couche de présentation du compositeur. Cette séparation permet au flou canonique d’échantillonner le fond du Dashboard sans rééchantillonner récursivement ses propres cartes. `dashboardContentGlassStyle()` est le constructeur commun obligatoire pour toute cellule hors en-tête : le rectangle transparent teinté/flouté est toujours présent, puis le média éventuel est superposé avec l’opacité issue des mêmes tokens. Les builders ne possèdent donc ni alpha ni flou locaux.

The Mystic ground is one isolated branch of the shared procedural material, dispatched by `material.mystic.x > 2.5` ahead of the liquid (`> 1.5`) and assistant (`> 0.5`) branches, so deleting the branch and its dispatch line deletes the feature. `atome/renderers/bevy-core/src/assets/shaders/procedural_sdf.wgsl` owns the flip itself — a real perspective divide, not a squash — the workspace back face sampled from the same compositor backdrop and carried WHOLE by the plate that was cut from it, the menu plate laid flat in the cell the plate leaves behind (the hole, dosed by `mystic_count.y`), the shared system-glass front face, the family tint, the rim and the contact shadow occluded by the plate. `types_procedural.rs` mirrors the uniforms and `procedural_sdf.rs` carries them, both bounding them so no JS value reaches the shader unbounded. The JS side owns geometry only: `mystic_layout.js` decides where a tile stands and when it turns — the solidary one-pixel table, the packed columns and rows, and the one wave plan every tile reads — `mystic_menu_items.js` owns the canonical desktop set and the rung each key keeps, `mystic_tokens.js` what a tile looks like, and `intuition_mystic_menu_renderer.js` packs the material through `menu_surface_record_runtime.js`, the single record owner it shares with the liquid ground. The tree keeps the icons, the labels, the hit test and the accessibility: the ground is a design, never a second mount path.

### 2026-09-20 — Mystic access, execution placeholders and narrow placement

Access projections remain derived from canonical Atome records; backend authorization still owns writes. Root/sidebar/lasso use the same JSON resolver. Async opening and rail reads reject obsolete contexts. The existing scene dispatcher and tool gateway enforce project modes; deferred drop choices recheck the mode before calling their mutation owner. GPU capture images and cameras are released on closure, and the shared motion sampler performs no per-tile UI updates.
...
- One card, two places: the Home panel (`eve_bevy_panel_home`) is the current user's card plus their settings; the Contact panel lists people as accordions and unfolds each person's card in place. Both are built by the same Home view builders and the same editing core (`bevy_panel_home_profile_editor.js`); the Contact list keeps no card code of its own.
- Card of somebody else: read from `collectPeopleDirectory()`; a public-directory user is read-only and shows only the fields its owner shares; a local address-book contact stays editable and is saved through `Squirrel.contacts`. No settings, no session action. The current user's own row mounts the Home card itself, so it is strictly identical to the Home panel, settings included.
- Sharing is enforced on the server: `server/directoryPublicService.js` projects `eve_profile.sharing.fields` (default name, first name, nickname, photo) into `directory_public_profiles.card_json`; per-field privacy rules still narrow what a requester reads.
- The liquid-glass menu rendering, the render-style choice and the legacy/modern taxonomy choice are removed; menus render flat, the main menu keeps the legacy composition, context menus stay driven by `context_menus.json` and the expertise level.

# Lit tools: single residence and pinned rail zone — 2026-09-24

The tools that REMAIN LIT once started now have one owner instead of one story per surface. `eVe/intuition/tools/core/active_tool_registry.js` declares the two facts a surface used to re-decide — residence (`rail` | `menu`) and the invocation path used both to read the lit state and to stop the tool — for the record/playback family the contextual rail publishes and for the operative creation tools (text, drawing, code, page, placeholder, template). The lit presentation stays where it belongs: on the slot that displays the tool, so the rail keeps switching Play to Stop and the ribbon keeps its own labels and icons through `shared/tool_presentation.js`. `tool_runtime_circle_tool_state.stopOperativeCreationTools` is now a thin alias, and the table it duplicated is deleted.

The contextual rail no longer scatters a lit case. `buildAtomeContextualEditTree({ activeSlots })` filters the moved entries out of the ordinary level (`withoutMovedTools`, roots only) and reserves `min(n × itemSize, availableRailHeight)` against `layout.bottom`, so the pinned zone is glued above the Atome handle on the dominant-handed edge, never scrolls with the rail viewport, and pushes the ordinary stack up by exactly its height — a Tetris reflow, not an overlay. The first activated case stays the closest to Atome; `atome_contextual_edit_runtime.js` owns that activation order and falls back to the catalogue order after a cold render. Each pinned case keeps the level node id (`atome_contextual_tool_<key>`, `…_icon`), so it inherits the handler, the tool identity and the pulse of the level node it stands for.

Presentation of a lit case is one owner on each surface, and so is the pulse: the unique `createRecordingIndicatorPulse` is synchronized by the rail (`litToolIconIds()`, covering rail-resident keys and the locked palette child that stands for a lit ribbon tool) and by the ribbon (`resolveBevyMainMenuLitToolIconIds` through its own `litToolPulse`), and cleared at stop. A view mode that paints itself active — fullscreen, playback mode, cumulative selection — is not a tool: it never pulses and never enters the pinned zone. A menu-resident tool therefore shows no rail case at all.

Evidence and limits: `tests/eve/contextual_rail_active_zone.test.mjs` plus the existing `contextual_rail_scroll`, `bevy_ui_main_menu_contract` and main-menu motion suites, and `npm run check:syntax` (2 208 files). The visible pulse, the one-press stop and the returning stack still require real-runtime pixel acceptance in the Web 3001 / Tauri lane; Tauri and iOS are not claimed here. A pinned case keeps the options its tool declares: a long press fans them out from the pinned case itself — never from the level the tool left — and the case carries the shared palette accent while they are open.

# Pinned rail case without a rail — 2026-09-24

The pinned zone is no longer a property of a mounted rail. A tool that stays lit declares its residence in `eVe/intuition/tools/core/active_tool_registry.js`, and `readActiveRailToolEntries()` now reads the FACTS of the rail family wherever they live: a record through the action owner or the capture owner it delegates to, a playback through the selected-project media registry and the project transport. `atome_contextual_edit_runtime.js` therefore resolves the lit slots before deciding whether a rail can be mounted, and when nothing is selected — or no object is editable — it mounts the reserved zone alone (`definitions: []`, no scroll viewport, the rail's exterior shadow kept). Nothing is duplicated: the ordinary rail path and the pinned-only path build the same cases with the same ids and stop through the same canonical invocation; the gateway remains the owner of the authorization decision, and the "selection required" guard is told the tool was already lit, so it never turns a stop into a blocked activation.

Evidence: `tests/eve/contextual_rail_active_zone.test.mjs` (16/16) plus `contextual_rail_scroll`, `bevy_ui_main_menu_contract`, `bevy_ui_main_menu_animation_lifecycle`, `bevy_ui_main_menu_palette_motion`, `atome_edit_footer_delete_cold_start`, `capture_recording_lifecycle`, `selected_project_media_playback_runtime`, the manifest guard and `npm run check:syntax`. Two suites unrelated to these files (`project_view_level_rail`, `project_view_list_transport_ui`) failed before and after this change; their sources were last touched by the ongoing Visual/List work, not here. The visible pulse and the one-press stop still require real-runtime pixel acceptance in Web 3001 / Tauri.

# Armed creation tools: taxonomy residence, rail options, generator armed state — 2026-09-25

The 2026-09-24 rail declared WHO is pinned but never WHERE each creation tool lives, so Page, Placeholder, Code and Generator kept their old ribbon behaviour. The residence is now data, declared once in `eVe/intuition/menu/context_menus.json`: `vocabulary.tools` lists the seven creation keys, and `menus.sidebar.tools.<tool>` carries `{ inherits, pinned: true, options }` — `inherits` is an existing kind used as the validation anchor, `pinned` is what `active_tool_registry.isCreationToolPinned()` reads, and `options` is the tool's own option set at every expertise level (inherited `size` / `couleur` / `font` removed with `null`). All seven tools are pinned; only Page and Placeholder declare options, Code keeps its panel, Template has no ribbon entry and invents no surface.

`buildArmedToolRailDefinitions()` therefore serves every pinned tool, and a tool whose content declares a list EXPOSES that list to the rail instead of declaring a second one: `readCreationToolContentKey()` (published by the registry) hands the content slot to `contentOptionKeys()`, which flattens a palette child into its own children — the Generator's registry projection (`generator_run_*`) becomes one flat level of cases in registry order, with the run generator lit. Looks still come from the menu content (`catalog[key]`), order from `order`, and each value from its single owner: `readPageFormat()` for the six formats, `readChoice()`/`readLimits()` for the Placeholder kinds and its two sliders, `eveGeneratorApi.readChoice()` for the Generator. `invokeArmedToolOption()` writes back to those same owners: `setActive(true, format)`, `setActive(true, kind)`, the registered slider actions `ui.placeholder.duration.apply` / `ui.placeholder.max_chars.apply`, and `runCase(generatorId)`.

Page stops being a ribbon palette: `page_create` in `main_menu_create_content_runtime.js` is a plain `type: 'tool'` with `latch: true`, no `children` and no `invoke_on_expand`, so the press arms the tool and closes the menu like Text and Drawing. Its six `page_format_*` entries stay in the catalogue (the rail reads label and icon there) but are no longer listed as palette children. Placeholder keeps its choices in place — the choice arms through `setActive(true, kind)` — and the Generator, which had no armed state at all, now owns one in `eVe/intuition/tools/generator/runtime.js` (`isActive` / `setActive` / `readChoice` / `runGeneratorCase` published through `window.eveGeneratorApi`), arms on a run, stays armed so generators follow each other, publishes `eve:tool-state-changed` like Page and Code, and stops the other operative creation tools through `stopOperativeCreationTools({ keepToolId })`. No second creation path: generation still goes through `eveToolBase.createAtome`, and `window.eveGeneratorApi` is the only new public API.

Evidence: `tests/eve/armed_tool_rail_definitions.test.mjs` (5/5, characterized as failing on the pre-change baseline), `tests/eve/context_menus_characterisation.test.mjs`, `tests/eve/create_tools_modes_contract.test.mjs`, `tests/eve/contextual_rail_active_zone.test.mjs` (pinned case for Code/Page/Placeholder/Generator, stop through the owner, formats in the rail level with only the current one lit, a case arming its format), `temp/tool_taxonomy_probe.mjs` (44 checks) and `npm run check:syntax` / `npm run check:m0`. Pixel acceptance in the Web 3001 / Tauri lane (`temp/armed_tool_rail_real_app.probe.mjs`, `temp/creation_tools_defects_real_app.probe.mjs`, `temp/tool_manager_real_app.probe.mjs`) stays To verify; iOS is out of scope.
