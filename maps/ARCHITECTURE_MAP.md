...
- Bevy integration is represented in JavaScript by `eVe/domains/rendering/render_atom.js`, `eVe/domains/rendering/virtual_scene_contract.js`, `eVe/domains/rendering/bevy_projection_adapter.js`, `eVe/domains/rendering/bevy_media_texture_resolver.js`, and `eVe/domains/rendering/bevy_web_renderer_runtime.js`; in native Rust by the optional `platforms/desktop-tauri/src/bevy_backend/mod.rs` contract; and in browser/WASM Rust by `platforms/web/bevy-renderer/`. The JavaScript contract remains the renderer-agnostic `AtomeRenderNode` tree/diff/dirty-flag source derived from canonical Atome records. RenderAtom content projection canonicalizes upload and recording media URLs before Bevy mapping, the browser adapter maps that projection into explicit Bevy payload data with Rust `i32`-bounded layers, and the browser runtime starts the generated WASM renderer on an existing shared canvas, then applies Virtual Scene diffs and surface-size patches through explicit Bevy WASM exports while filtering the known non-fatal WASM runner `unreachable` completion from page error reporting. Initial startup consumes immediately available durable poster/peak cache inputs, while uncached video and audio-waveform nodes remain present as visible Bevy entities and receive decoded textures later through serialized resource operations rather than through a fallback renderer or DOM projection; later spawn diffs use the same pending-media path when video/audio metadata is not immediately available. Surface-size patches cancel pending deferred-media timers and resume queued generation after yielding so resize remains responsive. The Rust Bevy surfaces map explicit projection data into Bevy ECS components and expose Bevy WebGPU renderer entry points for native and web targets. Browser Bevy now owns supported shape plus RGBA texture sprites for text, raster image, SVG, video frames, and audio waveforms; video textures are extracted after a representative seek and muted frame presentation, while waveform textures are derived from canonical peaks when present or from decoded audio source samples when peaks are absent. These surfaces are not a full Virtual DOM and never own canonical Atome state.
...
- Active project Atome visual rendering enters `eVe/domains/rendering/project_scene_runtime.js`. `tool_genesis.js` may keep bounded project shell creation, but project Atome records must update the project scene and render through the Bevy web runtime instead of the removed `renderProjectAtTime()` / `project_scene_webgpu_adapter.js` legacy path or one visible `eve-atome_*` host with per-Atome interaction bindings.
...
- Active project selection visuals are rendered inside the same project canvas through Bevy projection diffs. `project_scene_runtime.js` derives selected ids from the existing selection runtime and marks disposable projection records; no per-Atome DOM outline, host class, selection property, or legacy WebGPU selection overlay is allowed on the cleaned project route.
...
- Active project resize gestures are resolved centrally in `surface_runtime.js` from scene hit-test bounds and logical canvas edge handles, with final resize geometry computed homothetically by `surface_pointer_runtime.js`. Legacy per-host resize listeners remain only for non-project DOM-rendered UI until those owners are separately migrated.
...
- Tauri desktop: local Axum, WebGPU, native Kira, native Symphonia, and optional feature-gated Bevy backend preparation that must consume canonical projection data without replacing the shared WebGPU compositor until a validated migration explicitly promotes it.
