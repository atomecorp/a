...
- Dashboard rendering keeps an internal disposable record cache and diffs records by id before calling `updateProjectSceneRecords(...)`. High-frequency wheel, pointermove, inertia, and transition ticks are coalesced by `dashboard_render_scheduler.js` onto one animation-frame render request; open, close, creation, and final editor states still use immediate renders so probes can verify deterministic scene state.
