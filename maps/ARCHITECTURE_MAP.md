...
- Main ribbon content ownership stays in `eVe/intuition/eVeIntuition.js`; visible menu membership is product UI composition, while `tool_runtime.js` owns the executable tool ids. The `view` palette exposes list/table/natural view-mode intentions without creating DOM-owned view state.
