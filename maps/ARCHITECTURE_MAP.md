...
- Dashboard no longer owns `+` creation actions from focused entêtes. There is no full-height plus strip, no plus hit-test route, no no-op/panel/project plus action branch, and no project creation path from the Dashboard header surface. Project item clicks still follow the same transition rule: Dashboard and project must never be visible simultaneously, and `loadProjectAtomes`/project activation happens only after the explicit project-item click path begins.
