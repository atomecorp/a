...
- Phase 1 Bevy preparation is represented in JavaScript by `eVe/domains/rendering/virtual_scene_contract.js` and in native Rust by the optional `platforms/desktop-tauri/src/bevy_backend/mod.rs` contract. The JavaScript contract remains the renderer-agnostic `AtomeRenderNode` tree/diff/dirty-flag source derived from canonical Atome records. The Rust Bevy module is feature-gated, compile-time-only preparation that maps explicit projection data into Bevy ECS components and defines a low-power desktop update/redraw policy; it is intentionally not a full Virtual DOM, not the active renderer, and not a canonical state owner.
...
- Active project media rendering is bridged by `eVe/domains/rendering/project_scene_webgpu_adapter.js`, which reuses `MoleculeWebGpuRenderer` as the WebGPU texture renderer behind the shared project compositor for image, SVG, video, and audio-waveform clip projections. It must remain a disposable render adapter and must not become a DOM media mount path, canvas-per-Atome renderer, or fallback rendering system.
...
- Project drag, resize, and text editing are scene intents on the active project path. `project_scene_gesture_runtime.js` coalesces `drag.move` and `resize.move` bursts to animation-frame cadence, updates only disposable project scene records for live feedback, and batches realtime `gesture_frame` events for sharing. `drag.end`, `resize.end`, and `text.commit` persist through `set`/commit via `window.Atome.commit` or `window.Atome.commitBatch`; visible DOM text/edit hosts are not part of the active project route.
