...
- The dashboard reserves the active BevyUI main-menu height as an excluded bottom band, falling back to measured legacy DOM toolbox height only when the BevyUI menu is inactive or absent. Dashboard hit-testing stops above that reserved rectangle. Dashboard rendering must not place a visual mask in the reserved band; the foreground menu remains the unchanged BevyUI main menu. This must not add a DOM overlay, CSS canvas filter, second canvas, or alternate renderer.
