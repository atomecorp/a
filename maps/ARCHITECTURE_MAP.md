...
- Active project Atome visual rendering enters `eVe/domains/rendering/project_scene_runtime.js`. `tool_genesis.js` may keep bounded project shell creation, but project Atome records must update the project scene and render through the Bevy web runtime instead of `renderProjectAtTime()` or one visible `eve-atome_*` host with per-Atome interaction bindings.
- `eVe/domains/rendering/project_scene_webgpu_adapter.js` is legacy active-renderer debt for the Bevy migration. It may remain only while no active project Atome route calls it as a fallback. Reusing it to cover browser video or SVG after Bevy projection starts is forbidden because that would restore a parallel renderer.
- Active project selection visuals are rendered inside the same project canvas. `project_scene_runtime.js` derives selected ids from the existing selection runtime, marks disposable scene atoms, `webgpu_compositor.js` forwards that state to active clips, and `project_scene_selection_overlay.js` supplies the light outline bitmap consumed by `project_scene_webgpu_adapter.js` as an additional WebGPU layer; no per-Atome DOM outline, host class, or selection property is allowed on the cleaned project route.
- Project drag, resize, and text editing are scene intents on the active project path. `surface_runtime.js` owns client-to-logical coordinate conversion before hit-testing so browser/device-pixel canvas scaling does not offset drag or resize targets. `project_scene_gesture_runtime.js` coalesces `drag.move` and `resize.move` bursts to animation-frame cadence, updates only disposable project scene records for live feedback, and batches realtime `gesture_frame` events for sharing. `drag.end`, `resize.end`, and `text.commit` persist through `set`/commit via `window.Atome.commit` or `window.Atome.commitBatch`; visible DOM text/edit hosts are not part of the active project route.
