...
- A media record whose immediately required texture input fails decoding is not allowed to reject the full initial Bevy scene; the node is skipped and tracked by the Bevy web runtime while valid nodes render. Uncached source-backed media enters the deferred queue with transparent pending material, then receives its texture through the same Bevy resource update path.
