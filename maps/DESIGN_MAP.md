...
- `eVe/domains/rendering/surface_runtime.js` owns bounded project and matrix WebGPU canvas surface projection. These canvas surfaces must attach to the current rendering host, expose host-sized CSS dimensions, keep DPR buffer dimensions separate from logical scene dimensions, and may expose short `data-role` values for auditability because they are rendering-zone surfaces, not final Atome DOM subtrees.
...
- Active project selected-state styling is a canvas-rendered WebGPU overlay layer: a light grey hairline with a subtle shadow generated by `project_scene_selection_overlay.js` from transient selection runtime state and consumed by `project_scene_webgpu_adapter.js`. It must not be implemented as per-Atome DOM outline, inline style, class mutation, or DOM overlay.
...
- Project resize affordance on the active route is a logical canvas hit region derived from scene bounds. It must not create visible per-Atome resize handles or host-level resize classes.
