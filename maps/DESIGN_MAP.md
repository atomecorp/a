...
- Dashboard card media stays canvas-rendered. Contacts with `metadata.user_face` project a single Bevy image record above the card color and below the name text, clipped to the card content rect with cover semantics. Projects with `metadata.project_preview_source` project a single Bevy image record above the card color and below the name text, clipped to the card content rect with contain semantics plus renderer-provided natural dimensions so the full miniature remains visible and the card color fills any margin. Project media comes from a renderer-owned Bevy/WebGPU PNG capture produced in the reusable hidden preview iframe/canvas, not from a live embedded project in the Dashboard block; Dashboard first paint must not block on cold preview capture and hydrates previews afterward. The capture surface must stay paintable for WebGPU while non-interactive and behind the app, wait for deferred Bevy resources, and reject transparent or nearly empty PNGs instead of visually accepting a blank card. These visuals must not use CSS backgrounds, DOM cards, per-item canvases, Matrix DOM clones, DOM screenshots, icons as substitutes, or renderer fallbacks.
