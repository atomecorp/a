...
- Dashboard visual QA samples real Playwright screenshots for the mockup category colors, visible entête text/icon pixels, card contrast, active `+` symbol, stable projection scale across left/right reopening, zero Dashboard records/nodes after close, absence of orphan dashboard pixels after refresh/launch, and verifies that the reserved toolbox band remains outside dashboard rendering.
...
- Dashboard open and close fade through Dashboard Bevy record opacity, never through CSS opacity on the canvas or the neutral workspace host. Opening projects the Dashboard records into the neutral `__eve_dashboard_workspace__` scene, claims the shared project canvas foreground for that neutral scene, starts from record opacity `0`, and fades records to their resolved visual opacity without loading a user project. Closing fades those same records to opacity `0`; for the neutral workspace they remain as transparent disposable Bevy scene records so the next open can animate immediately with style diffs instead of waiting for a full respawn, then the host is hidden only after the records are transparent. Non-neutral Dashboard overlays still remove their `__eve_dashboard_` records on close. No DOM overlay, HTML card, CSS fade, canvas opacity mutation, host opacity mutation, or second renderer participates in the visual effect.
