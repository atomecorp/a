...
- Active project audio waveform playback progress is canvas-rendered through the same Bevy project projection. The visual is a thin near-white vertical playhead, approximately 2 logical pixels wide, with a subtle translucent glow and a narrow dark translucent contrast backing so it remains readable on bright waveform peaks. It is positioned over the waveform from left to right by runtime playback progress. It is valid only for Audio Atomes represented on the project canvas and must not appear in Molecule, sequencer, timeline, API-only playback, DOM overlays, per-Atome canvas surfaces, or waveform DOM children.
