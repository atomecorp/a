...
- Owns the dashboard v1 visual token contract for Bevy projection records and scene effects: colors, radius default, project veil, violet reserved-band fill, dormant backdrop blur radius/downsample/tint, bottom shadow gradient, header side-shadow style, layers, fixed block sizing, and fullscreen item detail padding. The block unit is resolved by `resolveDashboardBlockUnitSize()` from the canonical main-toolbox square size (`TOOL_UI_METRICS.toolboxSquareSizePx`) multiplied by `metrics.blockUnitMultiplier` (default `2`); lane height, card unit width, entête width, and active plus width derive from that unit rather than viewport height. Fullscreen item detail opens immediately on disposable Bevy records; it does not animate a white expansion panel.
