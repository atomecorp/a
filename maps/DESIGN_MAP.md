...
- Owns the dashboard v1 visual token contract for Bevy projection records and scene effects: colors, radius default, project veil, violet reserved-band fill, dormant backdrop blur radius/downsample/tint, bottom shadow gradient, header side-shadow style, layers, dimensions, header/plus/card metrics, fullscreen editor padding, and the `300ms` fullscreen transition duration.
