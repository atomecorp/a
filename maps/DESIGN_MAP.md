...
- Dashboard header/icon/label/media texture sharpness and reuse are owned by the Bevy media texture resolver plus `bevy_media_texture_cache.js`, while final canvas sharpness is owned by the browser Bevy DPR-aware surface contract. Header SVG icons, project previews, and contact faces use the shared media `texture_scale` contract when their logical display size is small; repeated dashboard opens or rubrique changes must reuse unchanged text, SVG, and media raster payloads by content/rich-text edit state/style/dimensions/DPR/texture scale instead of adding per-card canvases, DOM text, pre-scaled Dashboard geometry, or a dashboard-only texture system.
