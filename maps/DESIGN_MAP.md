...
- On the password step, clicking the persistent Atome logo with an empty password returns to the phone step in the credential surface, restores phone focus and the mirrored caret, and does not submit credentials; clicking it with a non-empty password keeps the existing password validation path.
