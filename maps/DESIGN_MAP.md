...
- Owns the dashboard v1 visual token contract for Bevy projection records and scene effects: colors, radius default, project veil, violet reserved-band fill, dormant backdrop blur radius/downsample/tint, bottom shadow gradient, header side-shadow style, layers, dimensions, header/plus/card metrics, dashboard open/close fade duration (`transitions.dashboardFadeMs`, default `500` ms), and fullscreen item detail padding. Fullscreen item detail opens immediately on disposable Bevy records; it does not animate a white expansion panel.
...
- Dashboard visual QA samples real Playwright screenshots for the mockup category colors, visible entête text/icon pixels, card contrast, active `+` symbol, stable projection scale across left/right reopening, parked-record invisibility on close, absence of orphan dashboard pixels after refresh/launch, and verifies that the reserved toolbox band remains outside dashboard rendering.
