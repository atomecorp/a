...
- Every visible instance of that backdrop material is live: the common shader derives its sampling space from the current capture texture rather than a copied per-menu WebView size, and moving or resizing project content underneath Flower, the contextual side rail, or the bottom menu refreshes the complete capture/blur/presentation chain. A stale captured object or white band is a blocking visual defect; this contract is met with bounded asynchronous redraw primes only while backdrop materials exist, not with continuous idle rendering.
...
Le Dashboard est une composition WebGPU plein écran sur le fond du profil, cadré en `cover` au-dessus d’une base opaque qui interdit tout flash du projet résident. Toutes les cellules hors en-tête — météo, news, événement, projet, contact et moniteur — reposent systématiquement sur un verre dépoli canonique, transparent et teinté par la couleur sémantique. Cette règle reste vraie avec un média : l’image est dessinée à opacité contrôlée au-dessus du verre, puis reçoit son bandeau de titre translucide. `dashboard_skin.js#contentGlass` possède l’alpha de teinte, le flou, le remplissage et l’opacité média ; aucun builder ne duplique ces valeurs. Les anciennes bandes jointives, le voile projet, les ombres latérale/basse et le focus coloré global sont retirés.
