...
- The authenticated main menu is read from the handedness edge: the interactive Atome tool is first and opens or closes the Dashboard, followed by `home`, `find`, `capture`, `time`, `communicate`, `mode`, and `view` from `toolbox.children`, with the manual legacy-menu item on the opposite edge. Right-handed placement reverses the row visually so Atome remains nearest the right edge, while active resize observation keeps that edge-aligned row glued to the bottom of the WebGPU surface. At very narrow widths the 60 px blocks do not shrink; the right-handed row may extend off the left side through a negative logical `x` so the Atome block stays flush to the right edge, while left-handed layout stays at `x = 0`. Each 60 px block renders one uniformly sized, centered canonical SVG icon using the same hydrated opaque `lightgray` (`#d3d3d3`) tint above an opaque localized label lowered within the BevyUI column; menu block backgrounds are opaque so Dashboard reserved-band colors cannot bleed into tool colors. The prehydrated RGBA texture is preserved through the Virtual Scene and must not be replaced by re-decoding the original SVG source. BevyUI overlay records use the disposable `__eve_bevy_ui_` prefix, are excluded from project preview captures, and must not disturb Dashboard media records or existing scene effects. Tool items reuse the same normalized definitions and invocation handlers as the ribbon, with Atome routed directly to the existing Dashboard toggle.
