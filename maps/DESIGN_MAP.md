...
- The Bevy dashboard follows the R&D canvas mockup structure: joined lanes that keep their own category colors, square/double-width cells based on lane height, flat internal inset cards for real items only, white bottom-left item labels rendered through a single higher-density `text_style` texture with a lighter stroke plus diffuse shadow, one lateral entête shadow, one bottom shadow, and one high-density texture/icon record per header. It opens with no focused category; after an entête click, inactive headers dim by direct fill color, the focused header/rail/card set reflects only that active category, one active full-height `+` strip appears, and the active category's deduplicated item list is distributed once across the existing colored lanes without visual repetition or global lane recoloring. Clicking the active entête again returns to the unfocused dashboard overview. The Projects `+` strip leaves the dashboard completely before showing the newly created active project desktop.
