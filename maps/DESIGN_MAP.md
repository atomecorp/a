...
- `eVe/domains/rendering/surface_runtime.js` owns bounded project and matrix render-surface projection. Browser mode binds these surfaces to the Bevy WASM WebGPU canvas; Tauri and iOS/AUv3 route the main project surface through `bevy_native_renderer_runtime.js` and native Bevy commands instead of loading the browser/WASM Bevy module. Tauri access to those native commands is authorized by the `bevy-native-renderer` capability set. These surfaces must attach to the current rendering host, expose host-sized CSS dimensions, keep DPR buffer dimensions separate from logical scene dimensions, and may expose short `data-role` values for auditability because they are rendering-zone surfaces, not final Atome DOM subtrees.
- Project wallpaper/background display is canvas-rendered by Bevy, not by a DOM layer. User preferences still originate in `eVe/user/background.js`, generated pixels are produced offscreen by `eVe/domains/rendering/user_background_pattern_renderer.js`, and `eVe/domains/rendering/bevy_surface_background_runtime.js` feeds a non-selectable Bevy surface-background entity behind every Atome. The legacy `eve_background_visual_layer` DOM image layer must not be recreated on the active project route.
