...
- Project wallpaper/background display is canvas-rendered by Bevy, not by a DOM layer. User preferences still originate in `eVe/user/background.js`, generated random background parameters are owned by `eVe/intuition/tools/background_random_runtime.js` when generated backgrounds are used, generated pixels are produced offscreen by `eVe/domains/rendering/user_background_pattern_renderer.js`, and `eVe/domains/rendering/bevy_surface_background_runtime.js` feeds a non-selectable Bevy surface-background entity behind every Atome. The explicit wallpaper `download` button may fetch a remote image, but final display still uses the uploaded/protected media URL through the same Bevy background route. The legacy `eve_background_visual_layer` DOM image layer must not be recreated on the active project route.
