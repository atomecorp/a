...
- Dashboard header/icon/label texture sharpness and reuse are owned by the Bevy media texture resolver plus `bevy_media_texture_cache.js`; repeated dashboard opens or rubrique changes must reuse unchanged text and SVG raster payloads by content/rich-text edit state/style/dimensions/DPR instead of adding per-card canvases, DOM text, or a dashboard-only texture system.
