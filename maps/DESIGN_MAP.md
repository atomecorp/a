...
- Dashboard header icons use the existing per-rubrique base size multiplied by `1.5` and sit `5 px` lower. Calendar keeps its icon but omits the redundant rubrique label; its centered time occupies the `55–75%` vertical zone with `max(16 px, 19% of header width)`, while its date occupies `80–93%` with `max(10 px, 10% of header width)`, preserving a `5%` gap. All positions derive from the full canonical header rectangle even when the viewport crops it. Header and card roots consume the exact shared `EVE_COMMON_SKIN_TOKENS.bevy.systemSurface.shadow`; icons, media, labels, and text never receive another shadow.
