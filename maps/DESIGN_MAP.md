...
- The initial surface fades in directly over the body and publishes `eve:login-choice-visible` once visible so boot warmups can start behind the overlay. Workspace/dashboard/toolbox visuals may be prepared behind this login surface, but the login surface must not fade out until the workspace path has the project canvas, dashboard runtime, and main ribbon ready to reveal.
