...
- The Bevy dashboard follows the R&D canvas mockup structure: joined lanes that keep their own category colors in the unfocused overview, square/double-width cells based on lane height, flat internal inset cards for real items only, white bottom-left item labels rendered through a single higher-density `text_style` texture with a lighter stroke plus diffuse shadow, one lateral entête shadow, one bottom shadow, and one high-density texture/icon record per header. It opens with no focused category; after an entête click, the active category color floods the dashboard background, table, lanes, header fills, active `+` rail, and item cards while inactive header text/icons remain visibly dimmed. The active category's deduplicated item list is still distributed once across the existing visual lanes without data duplication. Clicking the active entête again returns to the unfocused multi-color dashboard overview. The Projects `+` strip leaves the dashboard completely before showing the newly created active project desktop.
