...
- Dashboard card label editing stays canvas-rendered. Long press reuses the hidden text service for keyboard, IME, caret, and native selection state, while `dashboard_item_text_fields.js` defines the editable text rectangles and `dashboard_records.js` projects the draft label plus `rich_text.editing.selection` into the matching Bevy text record slot so selection and caret render through the shared text texture path. Calendar cards display the event name and start date as two separate editable text records. No dashboard DOM input, item card, visible text node, CSS editor, or per-item overlay is allowed.
