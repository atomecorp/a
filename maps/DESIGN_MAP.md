...
- The credential surface is a full-screen 1/7-5/7-1/7 composition for phone, OTP, and password. The upper band owns the current instruction or OTP error, the central band owns native input capture plus mirrored value display, and the persistent Atome logo moves to the bottom band as the validation control. The native input is visually hidden so password bullets are rendered only by the clean mirrored text, and successful password submit clears native focus/selection before reveal. In test/demo OTP mode the upper band displays `O.T.P: {code}` instead of the production SMS instruction.
