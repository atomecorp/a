...
- The authenticated main ribbon currently exposes `home`, `find`, `capture`, `time`, `communicate`, `mode`, and `view` in that order; `capture` reveals the product capture palette including browser media import, `time` reveals the calendar panel child, `mode` reveals `perform`, `edit`, and `consume` mode tools, and `view` reveals the `list`, `table`, and `natural` view-mode tools through the existing palette child projection contract.
