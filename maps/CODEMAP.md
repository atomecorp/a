...
- `eVe/domains/dashboard/dashboard_bevy_ui_runtime.js` owns the dashboard-only render diff and animation-frame coalescing helper. It compares disposable Bevy records by id, emits only changed records plus removed ids, and must stay an internal dashboard performance helper rather than a second project renderer or scene owner.
