...
- `EVE_TOOL_SKIN_TOKENS.bevyMenu` is the canonical BevyUI menu material: 60 px tool size, state colors, text/icon colors, shadow, backdrop, shape variants, contextual chrome, and semantic palette-accent geometry. `bevy_ui_menu_surface.js` is its sole BevyUI adapter and complete standard-tool constructor; main menu and contextual edit share its transparent shell, material background, content, and relative layer contract, while Flower consumes the same material with its radial structure. All three decorate their full trees as presentation content so no menu pixel can feed the shared backdrop capture.
