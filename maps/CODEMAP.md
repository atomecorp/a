...
- `eVe/domains/dashboard/dashboard_environment.js` owns dashboard shell/environment reads: current project resolution, painted toolbox surface discovery, toolbox reserved height, handedness preference, environment signature, and active-category item projection for render layout. It measures visible toolbox controls/surfaces before transparent wrapper containers so the dashboard bottom aligns to the first painted maintoolbox pixel. It does not own durable project state or DOM rendering.
