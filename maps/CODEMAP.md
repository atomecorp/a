...
- `eVe/domains/rendering/surface_runtime.js` owns centralized project-surface pointer routing plus render-surface host attachment and logical size tracking. It observes the active host, `#view`, window, visual viewport, orientation and Tauri native-viewport events; work is frame-coalesced and ends with one forced settled reconciliation so later panel/WebKit consumers cannot leave Bevy at the preceding size. `reconcileRenderSurfaceSize()` is the only explicit re-entry and still dispatches the canonical `surface.resize` scene intent. `surface_hit_target_runtime.js` owns pure client-to-logical coordinate conversion and hit acceptance; no visible per-Atome resize DOM or alternate renderer path is introduced.
