...
- `project_view_mode_state.js` owns latest user intent without per-project FIFO and applies Natural through the same canonical surface transition as List/Matrix, so an older structured tree cannot survive a project restore. `project_view_records.js` owns canonical paged project reads, preserves `meta.project_id` membership, and excludes canonical deletion tombstones; `project_view_reorder_runtime.js` owns durable `hierarchy_order` writes and uses one canonical batch when available. The canonical shared drag session owns pointer preservation and drag cleanup; List renders a source fade plus insertion gap, while Matrix consumes the same press/drag/release lifecycle. `project_view_surface_runtime.js` retains a pending pointer-node preservation request across queued renders. List and Matrix rebuild their selected projection from canonical selection on load. `project_view_surface_context_runtime.js` owns the contextual rail and `project_view_surface_events.js` owns canonical change/size subscriptions.
- `project_data.js` owns a single global latest-wins workspace-activation generation. It prepares the persisted per-project representation before `setCurrent`, applies it through `project_view_mode_state.js` immediately afterward, and then consumes the authoritative `loadProjectAtomes` projection without rendering the scene again. The activation load carries internal `viewModePrepared: true`; `tool_genesis_project_load_runtime.js` skips only its otherwise-late redundant restoration in that case and retains the existing prefetch/final-restore behavior for other foreground loads.
