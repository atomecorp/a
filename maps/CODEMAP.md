...
- `eVe/intuition/tools/user_workspace_surface_runtime.js` owns post-session workspace surface activation for both authenticated and anonymous sessions. It reveals the existing main ribbon through `window.new_menu_v2.reveal()` and opens the existing dashboard runtime through `window.eveDashboardRuntime.open()` after the current project is available; it does not create a second dashboard entry point or duplicate menu state.
