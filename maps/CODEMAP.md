...
- `eVe/domains/dashboard/dashboard_environment.js` owns dashboard shell/environment reads: current project resolution, painted toolbox surface discovery, toolbox reserved height, handedness preference, compact environment snapshots/signatures, and active-category item projection for render layout. In focused category mode it distributes the active category's deduplicated items across the existing lanes without copying the same item into each lane; `createDashboardItemsForRenderMemo()` memoizes that derived list by data serial, active category, and source category references so hot render frames do not recompute it. It measures visible toolbox controls/surfaces before transparent wrapper containers so the dashboard bottom aligns to the first painted maintoolbox pixel. It does not own durable project state or DOM rendering.
- `eVe/domains/dashboard/dashboard_environment_watcher.js` owns the dashboard-only live environment observer while the dashboard is open. It watches the project surface, surface parent, toolbox candidates, window resize, Intuition handedness state, and profile preference updates, then coalesces environment rerenders only when the environment signature changes. Its last snapshot is reused by `dashboard_layout_resolution.js` for render layout so pointer/wheel/fade frames do not repeat toolbox DOM measurement. Category-visibility preference changes are handled by `dashboard_runtime.js` through the same profile-preference event so category lists are recomputed before render.
