...
- `eVe/domains/rendering/project_scene_runtime.js` also owns project-scene text edit, resize, and generic scene-intent commits. Text editing uses the centralized hidden text bridge and emits `text.commit`; resize emits `resize.end`; both commit canonically before updating disposable scene records.
