...
- Dashboard visuals are projected as disposable logical-coordinate records into the existing project Bevy surface through `project_scene_runtime.js`; there is no dashboard DOM renderer, no per-item DOM node, no dashboard-side scale transform, and no second visible canvas. The open dashboard now adds one low-layer project veil record and one vertical `bottom_shadow` texture-gradient record so the underlying project reads quieter and the dashboard casts a slight shadow into the reserved project/toolbox band while closing removes the records through the existing dashboard teardown diff.
