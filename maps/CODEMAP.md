...
- `eVe/domains/dashboard/dashboard_records.js` owns image-backed Dashboard card projection. Card shape fill is a slight `+3%` same-hue lift from the lane entête color so cards stay visually readable over the lane body while preserving the rubrique color family. Contact cards may emit one Bevy image record from normalized `metadata.user_face` between the card shape and Bevy text, using cover clipping inside the card content rect. Project cards may emit one Bevy image record from renderer-owned `metadata.project_preview_source`, using contain semantics plus renderer-provided `project_preview_width/project_preview_height` so the full project miniature stays visible and the existing card color remains visible in margins. `eVe/domains/rendering/project_preview_runtime.js` resolves those project sources from persisted preview fields, the current project scene records, or `loadProjectAtomes(...)` followed by the current Bevy scene state; `bevy_project_preview_capture_adapter.js` keeps the reusable capture iframe/canvas paintable behind the app with a cold-runtime timeout, and `bevy_project_preview_capture_frame.js` waits for deferred Bevy resources, rejects skipped/pending resources, and refuses transparent or nearly empty PNG captures. These image records are disposable Dashboard projection fields, not canonical Atome properties, DOM backgrounds, per-item canvases, Matrix DOM clones, or symbolic fallbacks.
...
- `tests/probes/dashboard_bevy_runtime_probe.test.mjs` owns the real Playwright dashboard acceptance path: guest workspace, boot-open-aware dashboard readiness, real main Atome handle click when needed, canvas header and `+` clicks, panel/project plus actions, project item activation, fullscreen detail projection, toolbox reservation, handedness mirroring, dashboard teardown, screenshot checks for entêtes/card contrast/no dashboard paint inside the toolbox band, and browser-error checks covering console, page errors, request failures, and HTTP response failures. Its focused helpers live under `tests/probes/dashboard_bevy_runtime/`: `runtime_support.mjs` owns Playwright waits/click helpers, `visual_support.mjs` owns shared screenshot color assertions, and `project_preview_support.mjs` owns the canonical preview fixture plus Dashboard project-card media/pixel assertions.
