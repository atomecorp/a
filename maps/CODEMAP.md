...
- `eVe/domains/rendering/surface_size_runtime.js` owns render-surface size derivation from host CSS dimensions, `devicePixelRatio`, coherent `ResizeObserver` device-pixel box data, and optional GPU texture-dimension clamping. For the active project surface, it resolves logical size from the project layer, `#view`, live `visualViewport`, then `window`, and never lets an old canvas backing size become the fallback authority during WebView resize transitions. Device-pixel boxes that contradict the page DPR are ignored so the Bevy backing-store cannot diverge from logical scene coordinates. It returns viewport/render-target measurements only and must not mutate Atome geometry.
