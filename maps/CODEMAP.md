...
- `eVe/intuition/runtime/eve_intuition/boot_runtime.js` owns eVeIntuition boot-time menu exposure, auth sync, panel surface registration, Flower/footer/vector/draw installation, deferred non-critical module warmup, UI tool registration, footer API exposure, and debug runtime installation. Workspace/tool warmups start only after an active workspace is known or after `eve:login-choice-visible`, so disconnected boot does not reveal toolbox content before the login accueil. `eVeIntuition.js` must not re-inline boot/warmup/register code.
