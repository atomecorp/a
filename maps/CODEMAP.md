...
- `eVe/domains/dashboard/` owns the closed eVe dashboard runtime: JSON default loading, category/item normalization, pure layout, visual tokens, transition interpolation, R&D mockup-style Bevy record projection, data adapters, and Atom/Home toggle lifecycle.
...
- Dashboard visuals are projected as disposable records into the existing project Bevy surface through `project_scene_runtime.js`; there is no dashboard DOM renderer, no per-item DOM node, and no second visible canvas.
...
- `tests/probes/dashboard_bevy_runtime_probe.test.mjs` owns the real Playwright dashboard acceptance path: guest workspace, real Atom/Home click, canvas header and `+` clicks, generic record persistence, fullscreen editor final rect after transition, toolbox reservation, handedness mirroring, and dashboard teardown.
...
- `eVe/intuition/runtime/eve_intuition/main_menu_content_runtime.js` routes the authenticated Atom/Home tool to the dashboard open/close runtime. The existing Home panel remains available for the unauthenticated login sequence.
...
- `eVe/intuition/runtime/eve_intuition/atome_edit_footer_lifecycle_runtime.js` owns Atome-edit footer ownership, `createIntuitionXFooterRuntime` lifecycle, generated style installation, focused fullscreen layout, double-click host resolution, show/hide, workspace reset listeners, and project-timeline availability sync.
