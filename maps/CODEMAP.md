...
- `eVe/domains/dashboard/dashboard_actions.js` owns dashboard item click actions and reasserts the workspace main menu after project-card activation. `dashboard_header_long_press_runtime.js` owns header-only long-press detection, and `dashboard_header_creation_actions.js` creates Projects and Calendar events through their existing APIs while Contacts only toggles the existing Bevy panel through `openPanelSurface` / `closePanelSurface` and `isBevyPanelSurfaceOpen`. It never loads the Contacts API or mutates the directory. Focused entêtes still expose no `+` creation strip or plus rail. In focused category mode, item hit-testing keeps the real item category separate from the visual lane category. Project item clicks enter workspace `transition`, fade out and tear down the dashboard, then activate/load that project as the active Bevy desktop instead of opening a dashboard fullscreen detail.
