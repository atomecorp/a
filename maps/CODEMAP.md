...
- `eVe/intuition/tools/user_workspace_surface_runtime.js` owns post-session workspace surface activation for both authenticated and anonymous sessions. It first asks the existing project loader to attach the resolved current project to the shared `eve_surface_project` canvas, then stabilizes the existing main ribbon through `window.new_menu_v2.showFully()` before opening the dashboard runtime with that explicit `projectId` and waiting for the dashboard Bevy projection to report `ok`; it does not create a second dashboard entry point, duplicate menu state, dashboard DOM, or an alternate canvas.
...
- `eVe/domains/rendering/project_scene_runtime.js` owns active project scene projection state keyed by project id. It accepts canonical project Atome records, delegates project-record normalization/filtering to `project_scene_record_projection.js`, delegates client-coordinate hit-test and lasso rectangle collection to `project_scene_hit_testing.js`, builds the disposable Virtual Scene projection, delegates Bevy native/web renderer execution to `project_scene_bevy_projection_runtime.js`, updates the bounded project render surface, and returns projection handles instead of per-Atome DOM hosts. `renderProjectScene(..., { preserveEphemeralRecords: true })` preserves only explicit ephemeral Bevy overlay records classified in `project_scene_record_projection.js` (`__eve_dashboard_`, `mol:lane:`, `mol:clip:`, `mol:kf:`, `mol:playhead`) during late project reconciliation. `eVe/intuition/runtime/tool_genesis.js` must await this projection before marking `loadProjectAtomes(...)` complete or refreshing the recent-load cache, so a project refresh cannot cache "loaded" Atomes before Bevy has accepted the scene.
...
- `eVe/intuition/runtime/tool_genesis_project_load_runtime.js` owns `tool_genesis.js` project Atome loading: local/remote/shared override selection, owner filtering, in-flight de-duplication, recent-load cache completion, load/render perf events, and Bevy scene dispatch through `renderProjectScene()`. `staleFirst` starts local and remote reads in parallel, renders fast local data when available, skips the duplicate final render when that fast local result is still final, and passes ephemeral-record preservation into project renders so dashboard/Molecule overlays survive reconciliation. It must not create per-Atome DOM hosts for active project rendering.
