...
- `eVe/domains/dashboard/dashboard_environment_watcher.js` owns the dashboard-only live environment observer while the dashboard is open. It watches the project surface, surface parent, toolbox candidates, window resize, Intuition handedness state, and profile preference updates, then coalesces environment rerenders only when the environment signature changes. Its last snapshot is reused by `dashboard_bevy_ui_runtime.js` for render layout so pointer/wheel/fade frames do not repeat toolbox DOM measurement. Category-visibility preference changes are handled by `dashboard_bevy_ui_runtime.js` through the same profile-preference event so category lists are recomputed before render.
...
- `eVe/intuition/ribbon/bevy_ui_main_menu_model.js` owns the BevyUI main-menu item order, item-size contract (`resolveDashboardBlockUnitSize() / 2`), handedness-aware bottom layout, Atome edge order, horizontal overflow metrics, per-block icon+label tree shape, shared icon sizing, single `lightgray` SVG icon tint, label vertical placement, and disposable BevyUI tree shape. Compact layouts keep fixed-size blocks and expose `scrollLeftPx` / `maxScrollLeft` instead of shrinking tools or using DOM scrolling; right-handed overflow defaults to the terminal scroll position so Atome starts pinned to the right edge, while left-handed overflow starts at `0`. `bevy_ui_main_menu_scroll_runtime.js` owns runtime-only wheel/drag horizontal scrolling, pixel/line/page wheel-delta normalization, handedness-aware vertical-wheel mapping, snapping, and activation suppression after scroll. `bevy_ui_main_menu_runtime.js` owns the `window.new_menu_v2` facade implementation, BevyUI mount/update/unmount calls, scroll-render animation-frame coalescing, active surface/window resize observation, tool latched/external-open visual state, Atome dashboard toggle, and manual legacy-menu toggle. `main_menu_bridge_runtime.js` composes the private legacy ribbon and the BevyUI menu facade without allowing automatic DOM ribbon fallback.
- `eVe/domains/rendering/bevy_ui_runtime.js` owns the internal JavaScript BevyUI facade `window.eveBevyUiRuntime`: it normalizes Squirrel-owned UI trees, stores the interaction tree in CSS pixels, binds the shared canvas with `touch-action: none` so mobile pointer drags remain on the BevyUI route, versions and queues overlay renders per tree so stale async texture hydration cannot overwrite newer UI records, projects the visible product path through non-selectable WebGPU project overlay records, and must not create DOM nodes per component. Native BevyUI WASM op submission is explicitly opt-in so overlay-backed product surfaces do not emit unused Bevy UI events or double-render over their WebGPU overlay. Focused helpers keep the rendering concerns closed: `bevy_ui_hit_test_runtime.js` owns pure Bevy UI node geometry hit-testing for the JS event router; `bevy_ui_image_runtime.js` hydrates `icon` / `image` sources and label textures into RGBA payloads through the shared Bevy media texture resolver, using the exported `DEFERRED_TEXTURE_BATCH_SIZE` from `bevy_media_resource_runtime.js` so ImageNode previews and deferred Bevy media resources share the same permanent per-frame upload budget while preserving tree order; `bevy_ui_render_scale.js` keeps the submitted UI tree in logical BevyUI units; `bevy_ui_project_overlay_runtime.js` projects the same disposable BevyUI tree as stable non-selectable WebGPU records under the centralized `__eve_bevy_ui_` prefix, forwards canonical `overlayRecord` payloads exactly with prefixed ids, applies large overlay trees in 20-record batches across animation frames so static dashboard mount stays under the T3.5 frame budget, and reconciles the fixed BevyUI main-menu tree atomically by prefix so its icon/label set cannot become a stable partial batch; `bevy_ui_overlay_reconciliation.js` owns overlay geometry signatures, post-clear bookkeeping, observable projection errors, and one queued reconciliation from an empty overlay baseline after a failed or interrupted remount; `project_scene_record_projection.js` classifies that prefix as ephemeral; `project_preview_runtime.js` excludes it from project preview captures; `project_scene_runtime.updateProjectSceneOverlay(...)` preserves existing scene effects when no explicit `effects` array is supplied; `virtual_scene_contract.js` preserves prehydrated overlay `bevyTexture` payloads and resource signatures through the Virtual Scene; `bevy_media_resource_runtime.js` owns deferred media texture resolution, queue ordering, and the shared `DEFERRED_TEXTURE_BATCH_SIZE` budget, and must not re-resolve nodes that already carry a Bevy texture; `bevy_ui_value_runtime.js` owns scalar/color/id normalization. `atome/renderers/bevy-core/src/ui/mod.rs` owns the corresponding native BevyUI ECS tree, image-node texture spawning, diagnostics, and event queue, while `ui/types.rs` owns the serializable UI op/node contracts, `ui/tests.rs` owns the focused Rust UI tests, and `platforms/web/bevy-renderer/src/exports.rs` exposes the opt-in native exports `apply_atome_bevy_ui_ops`, `read_atome_bevy_ui_diagnostics`, and `drain_atome_bevy_ui_events`.
- `eVe/intuition/runtime/eve_intuition/main_menu_content_runtime.js` owns the authenticated main ribbon content declaration. The visible main sequence is `home`, `find`, `capture`, `time`, `communicate`, `mode`, and `view`; `ai` remains an inline IA prompt route but is not exposed in the visible toolbox, `capture` opens the product capture palette including the browser media import action, `time` exposes `calendar` instead of the removed visible `timeline` child, `mode` is a palette whose children are `perform`, `mode_edit`, and `mode_consume`, and `view` is a palette whose children are `view_list`, `view_table`, and `view_natural`, while `matrix`, `activity`, and `undo` remain catalog/runtime tools but are no longer main-ribbon children.
