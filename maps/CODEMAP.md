...
- `eVe/intuition/tools/user_workspace_surface_runtime.js` owns post-session workspace surface activation for both authenticated and anonymous sessions. It resolves the current project from the explicit id, `window.__currentProject`, `AdoleAPI.projects.getCurrentId()`, or `eveToolBase.getCurrentProjectId()`, reuses an already attached canonical project surface before calling the project loader, otherwise asks the existing loader to attach the resolved current project to the shared `eve_surface_project` canvas, then stabilizes the existing main ribbon through `window.new_menu_v2.showFully()` before opening the dashboard runtime with that explicit `projectId` and waiting for the dashboard Bevy projection to report `ok`. Reopening a workspace whose dashboard entities are already warmed for the same `projectId` must not repeat dashboard warmup; this owner does not create a second dashboard entry point, duplicate menu state, dashboard DOM, or an alternate canvas.
