...
- `eVe/intuition/runtime/bevy_panel/bevy_panel_runtime.js` must use the installed canonical BevyUI canvas runtime (never create a competing pointer owner) and reassert the authenticated/workspace main-menu invariant after mounting a BevyUI panel surface, using the shared workspace menu helper instead of panel-local HTML chrome.
