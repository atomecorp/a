...
- Imported project media Atomes must carry the current project id in both the canonical commit envelope and media Atome properties (`project_id`/`projectId`) so scene membership, persistence reloads, diagnostics, and Molecule opening do not depend on DOM parent inference.
