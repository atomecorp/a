...
- `platforms/web/bevy-renderer/` owns the browser/WASM Bevy WebGPU renderer surface. It consumes virtual-scene projection data as explicit JSON/wasm-bindgen input, binds Bevy to a caller-owned canvas selector, and exports `run_atome_bevy_renderer(...)` plus explicit diff/resource update exports without implementing a DOM renderer, canvas-per-Atome path, or fallback renderer. Supported browser projections now include Bevy sprite shapes and Bevy RGBA texture sprites for text/raster images/SVG/video frames/audio waveforms.
