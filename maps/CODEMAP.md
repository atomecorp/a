...
- `eVe/domains/media/selected_project_media_playback_runtime.js` owns selected active-project media transport for the cleaned canvas route. It resolves selected media records from `project_scene_runtime`, sends audio and extracted video-audio through the existing `Squirrel.av.audio`/Kira facade, and projects live video frames back through disposable project-scene record updates for Bevy/WebGPU without creating visible per-Atome media DOM hosts or a second media engine.
