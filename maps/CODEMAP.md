...
- `eVe/domains/dashboard/dashboard_actions.js` owns dashboard item click actions and reasserts the workspace main menu after project-card activation. A short header press toggles the Dashboard creation mode (`dashboard_creation_runtime.js`); calendar/contact cells only open the registered editor through `openPanelSurface` and never load the Contacts API or mutate the directory. Focused entêtes still expose no `+` creation strip or plus rail. In focused category mode, item hit-testing keeps the real item category separate from the visual lane category. Project item clicks enter workspace `transition`, fade out and tear down the dashboard, then activate/load that project as the active Bevy desktop instead of opening a dashboard fullscreen detail. The Dashboard rail shows the visible category headers only: the former Settings header, which opened Home through that surface session owner [ensureToolModule(home) then window.open_home_panel] rather than calling openPanelSurface(home) directly, is removed, so the main-toolbar Home tool is the single entry point of the Home panel and the Dashboard never opens it.
...
- `dashboard_header_records.js` owns the `1.5×` header-icon geometry, the `+5 px` vertical offset, and the Calendar time/date-only header projection. `dashboard_bevy_ui_runtime.js` owns its single minute-aligned timer and lifecycle cleanup. `tests/eve/dashboard_records.test.mjs` guards geometry, Calendar label removal, timestamp-derived time/date, exclusive shared-shadow ownership, and one timer across open/close/destroy; `shape_shadow_overlay_tests.rs` guards repeated style rebuilds without live-overlay accumulation.
...
- `eVe/domains/user/profile_api.js` owns `updateUserProfileIdentity`: complete-profile load, latest confirmed event overlay, identity-only merge, and canonical persistence.
- `bevy_panel_contact_runtime.js` delegates current-user saves to that owner. `bevy_panel_home_actions.js` and `bevy_panel_home_runtime.js` consume the same confirmed update without a second server read.
