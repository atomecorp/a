...
- `todo/ui_bevy/bevy_panel_migration_guide.md` is the Priority 1 execution
  ledger for the mandatory component approval loop and its bounded passive-batch
  exception, product-panel coverage,
...
- `atome/src/squirrel/components/segmented_control_contract.js` owns renderer-neutral normalization for a segmented control: two or more unique options and one known selected value. `bevy_panel_segmented_control.js` composes it from the supported native `segmented_control` root and native button primitives because the active WASM vocabulary has no specialized segmented-widget renderer. `bevy_panel_lab_segmented_control_runtime.js` owns only reset-on-close `list`/`table`/`natural` presentation state and closed `panel_lab.segmented.*` intents; it creates no DOM control, MCP call, durable state, or Atome mutation.
