...
- `eVe/domains/rendering/surface_size_runtime.js` owns render-surface size derivation from host CSS dimensions, `devicePixelRatio`, coherent `ResizeObserver` device-pixel box data, and optional GPU texture-dimension clamping. For structural active-project resizes, it reconciles visual viewport, layout viewport, and project-host dimensions against the preceding surface size: one changed source wins, two agreeing changed sources win, and an unchanged source cannot retain an old WebView/Tauri size. Initial and observer-only embedded sizing remains host-owned. Device-pixel boxes that contradict the page DPR are ignored so the Bevy backing-store cannot diverge from logical scene coordinates. It returns viewport/render-target measurements only and must not mutate Atome geometry.
