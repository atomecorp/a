...
- `surface_interaction_runtime.js` owns recursive open-Molecule target resolution; it exposes a raw descendant when any ancestor remains in contextual edition and otherwise resolves the closed structural root. Flower consumes the same resolver.
...
- `eVe/shared/lasso_context_zone_runtime.js` retains only the waiting rectangle lifetime and fixed context. The official Flower context owner and shared Bevy Flower consume Text, Capture, Molecule, Shape, Copy, Paste and Delete descriptors; the temporary menu support module and its buttons are removed. Capture reuses `eVe/shared/lasso_context_capture.js` and the shared Bevy project-preview adapter, including crop origin.
...
- `eVe/intuition/flower/context_pointer_lock.js` announces pointer takeover when Flower opens. The existing DOM drag and shared WebGPU surface owners consume that signal to release their sessions immediately while selection remains canonical. `copy.js` accepts Flower's captured selection, while `paste.js` delegates Atome cloning and internal-reference remapping to the existing duplicate owner.
...
- `atome/src/squirrel/components/toggleable_contract.js` owns renderer-neutral
  ...
  30 px surface used by icon actions; `bevy_panel_lab_choice_runtime.js` owns
  only ephemeral checked/selected/focus/pressed state and closed
  `panel_lab.choice.*` intents. The validated
...
- `eVe/domains/dashboard/dashboard_actions.js` owns dashboard item click actions and reasserts the workspace main menu after project-card activation. A short header press toggles the Dashboard creation mode (`dashboard_creation_runtime.js`); calendar/contact cells only open the registered editor through `openPanelSurface` and never load the Contacts API or mutate the directory. Focused entêtes still expose no `+` creation strip or plus rail. In focused category mode, item hit-testing keeps the real item category separate from the visual lane category. Project item clicks enter workspace `transition`, fade out and tear down the dashboard, then activate/load that project as the active Bevy desktop instead of opening a dashboard fullscreen detail.
...
- `eVe/intuition/tools/capture_recording_feedback_runtime.js` owns bounded Bevy tool feedback and post-stop reveal delegation for `capture.js`. It receives the invoking Bevy presentation (`flower` or `main_menu`) and projects one session only to that surface. During a main-menu capture, `bevy_ui_main_menu_recording_visual_runtime.js` keeps the Capture palette open and `bevy_ui_main_menu_model.js` preserves the complete menu while replacing the active tool content with the recording preview. Flower keeps its specialized stationary-tool presentation. The main-menu recording runtime keeps the matching session/sequence, deduplicates unchanged 64-pair audio envelopes, and caps their projection at 30 Hz; `bevy_ui_main_menu_recording_visual_model.js` replaces the complete icon/label content during active audio/video phases and maps the scope through the fixed -60 dB to 0 dB display scale. These owners register live sources through their renderer owners but create no audio-scope canvas, frozen media clone, camera element, visible media node, or second renderer.
...
- `eVe/intuition/runtime/eve_intuition/media_reader_tool_runtime.js` owns eVeIntuition registration and orchestration for `ui.media.reader`, `ui.animation.reader`, `ui.play`, `ui.pause`, and `ui.stop`. `boot_runtime.js` imports it once after the shared eVe audio/Kira deferred modules and before Flower/contextual owners, then injects the existing selection and latch dependencies. It delegates selected media transport to `selected_project_media_playback_runtime.js`, delegates project automation lookup to `project_scene_runtime.js`, and is the only non-decode runtime allowed to call `setBevyVideoDecodePlayback(...)`.
...
- `capture://...` is an internal capture-session identifier, never a project media source or BevyUI URL. Flower and main-menu recording previews both consume only the recorder-pushed RGBA texture, rendered as the existing texture-backed image record; the persisted recording file resumes the normal media route only after durable stop success.
...
`eVe/domains/dashboard/dashboard_actions.js` owns the closed item-routing table: Projects delegate unchanged to `project_data.js#activateProjectWorkspace`, Calendar delegates by event id to its existing panel, Contacts delegate by stable identity to the existing Contact panel, and unsupported categories return inert without projecting a generic detail surface. `dashboard_item_flower_menu.js` owns Dashboard Flower target resolution and the existing project-only delete/duplicate/copy/paste actions. Neither owner may create a second state, panel, renderer, or persistence path.
...
### 2026-09-18 — Intuition Mystic menu (third surface style)

`eVe/intuition/mystic/mystic_layout.js` owns the whole tile geometry: the twenty-four-rung ladder, the ring those rungs need, the mirror that leans a star into the free room, the per-tile flip timeline and the clamp margin a caller reserves before opening. `mystic_menu_items.js` routes menu entries onto that ladder — a fixed cross (`north`, `east`, `south`, `west`), four family diagonals, then the free rungs in context order — and reports the overflow beyond twenty-four instead of dropping an entry. `mystic_tokens.js` owns the tile design values and the per-rung families, `mystic_motion.js` samples the flip from the menu's own elapsed time, and `intuition_mystic_menu_renderer.js` packs the material and owns no record.

`eVe/domains/rendering/menu_surface_record_runtime.js` is now the single scene-record owner both menu grounds mount through; `eVe/intuition/ribbon/bevy_ui_flower_surface.js` is the one place that decides which design paints and pushes the frame; `bevy_ui_flower_recording_visual.js` carries the capture feedback nodes the Flower runtime used to build itself. `bevy_ui_flower_model.js` keeps ONE geometry for the tree, the tiles and the hit test; `bevy_ui_flower_runtime.js` keeps the pointer session, the press-outside dismissal and the 1.5 s centre hold that closes the menu and hands over to the assistant. Regression owner: `tests/eve/bevy_ui_mystic_menu.test.mjs`.

### 2026-09-19 — Intuition Mystic menu: canonical content, the table, the flip law

This section supersedes the Mystic geometry and content description above; the ownership it names is unchanged. `eVe/intuition/mystic/mystic_menu_items.js` now owns `MYSTIC_DESKTOP_ITEMS`, the ONLY list of keys the root menu shows — `home`, `capture`, `dashboard`, `communicate`, `create`, `calendar`, `info`, `find` — each carrying the rung it owns, so `help` disappears from the table and the Flower's contextual crop (project `paste/audio/video/photo/import`, per-kind sets) is no longer reused by this style. `eVe/intuition/runtime/eve_intuition/flower_context_items_runtime.js` stays the ONE item producer: its Mystic branch builds those eight entries through the same `buildFlowerContextToolItem` handlers, and two contexts keep the corolla's own content — a text field and an explicit surface item. The rung travels on the item, the family colour still follows the rung, and no content key is invented.

`eVe/intuition/mystic/mystic_layout.js` no longer mirrors anything: `resolveMirror`, `mirrors`, `starLean` and `blockBounds` are replaced by `resolveAxisPlan`, a pure cascade per axis. The Atom's cell is clamped only by its own half cell — it stays glued to the edge the menu was opened on — and the canonical columns are handed out from it towards the side that has room (`MYSTIC_AXIS_PLAN_ORDER`), a column with no room switching sides and pushing the following ones one cell further out. `computeMysticLayout` therefore returns `extents` (the cells really used on each side) and `centred` (the axis the surface is too small to hold, where the block falls back to centring), and `MYSTIC_TILE_GAP_PX` is `1`: the cells are solidary. The flip law is the demo's and lives here too — `planMysticTiles` gives every tile its `rank`, `delayMs` (a QUARTER of the previous turn: `150` ms after a `600` ms turn, `200` ms after an `800` ms one, summed along the wave and never a constant), `durationMs` (`800` ms, `600` ms for the tile nearest the centre) and the wave's real `totalMs`; `eVe/intuition/mystic/mystic_motion.js` reads that one plan, so `done` is the last tile and no shared constant drives the block (`RIBBON_TOKENS.flowerMotion` stays the corolla's clock). `MYSTIC_HOLE_DOSE` replaces `MYSTIC_BACK_ALPHA` and `mystic_count.y` is now the dose of the hole the turning plate leaves behind; the shader comment, `atome/renderers/bevy-core/src/types_procedural.rs` and `eVe/domains/rendering/bevy_projection_adapter.js` carry the same value (`0.6` px of edge softness, under the one-pixel seam). Regression owner: `tests/eve/bevy_ui_mystic_menu.test.mjs`.
### 2026-09-19 — Mystic: the configurable seam and the skin that owns the look

`eVe/elements/skin/mystic_skin.js` is the new owner of what a Mystic element looks like. `EVE_MYSTIC_SKIN_TOKENS` is registered as `mystic` in `eVe/elements/skin/index.js` and takes the same per-element overrides as the panel, button, tool and dashboard skins (`resolveMysticSkin(overrides)`, `resolveEveSkinTokens('mystic', overrides)`). It derives the plate from `EVE_COMMON_SKIN_TOKENS.bevy.systemSurface.background`, the shadow from that surface's own contact shadow, the mark from the icon base and the family tints from `EVE_SEMANTIC_COLOR_FAMILIES`, and adds only the tokens no other surface has: `rim`, `hole.dose`, `edge.{softnessPx,maxSeamShare}`, `family.mix`, `center.{plate,tint,icon}` and `radius.roundnessDefault`.

`eVe/intuition/mystic/mystic_tokens.js` keeps no literal at all: `MYSTIC_BORDER_WIDTH_PX`, `MYSTIC_RIM_ALPHA`, `MYSTIC_HOLE_DOSE`, `MYSTIC_PLATE_FOND`, `MYSTIC_CENTER_PLATE`, `MYSTIC_CENTER_TINT`, `MYSTIC_CENTER_ICON`, `MYSTIC_FAMILY_MIX` and `MYSTIC_EDGE_SOFTNESS_PX` are reads of that skin. `resolveMysticEdgeSoftness(gap, skin)` caps a plate's anti-aliasing at half the seam the layout really handed out, and `intuition_mystic_menu_renderer.js` derives `mystic_style[3]` from the seam the image carries (`resolveFrameSeam`), so a wider seam never turns into a blurrier plate.

`MYSTIC_TILE_GAP_PX` (`1`) is now the FLOOR of a preference rather than the whole answer. `normalizeMysticTileGap` is the one clamp and `MYSTIC_TILE_GAP_MAX_PX` is `12`; `preferences.visual.mysticTileGap` carries the live value, the Home panel offers the presets `1 / 2 / 4` (`home_mystic_gap`, `bevy_panel_home_actions.js` passes `tileGap: visual.mysticTileGap`), and the tree, the block and the uniforms read that single normaliser. Regression owners: `tests/eve/bevy_ui_mystic_menu.test.mjs` (a seam that can never close, the floor, the preference end to end, the derived softness, the shared skin and its overrides) and `tests/eve/bevy_panel_home_contract.test.mjs`.
### 2026-09-19 — Mystic: the opening length is a preference, and the Home field edits it

The wave's LENGTH is now a preference and its SHAPE is not. `eVe/intuition/tools/user_visual_preferences_model.js` owns `MYSTIC_OPENING_MS_DEFAULT` (`2000`), `MYSTIC_OPENING_MS_MIN` (`300`), `MYSTIC_OPENING_MS_MAX` (`6000`) and `normalizeMysticOpeningMs(value)`, the only clamp: anything that is not a number falls back to the law's own length, anything under the floor is raised to it, anything over the ceiling is brought back under it. `preferences.visual.mysticOpeningMs` carries the live value, `eVe/intuition/liquid/intuition_liquid_preference.js` normalises it, exposes `readIntuitionMysticOpeningMs()`, compares it so a re-published profile does not restart the wave, and bridges the profile event to `visual.mysticOpeningMs`.

`planMysticTiles({ ..., openingMs })` in `eVe/intuition/mystic/mystic_layout.js` is the ONLY place the request is applied, and it applies it as ONE factor: every `delayMs` and every `durationMs` of the law is multiplied by `askedMs / naturalTotalMs`, so `600` ms turns for the tiles nearest the centre, `800` ms turns for the outer ones and the quarter stagger between them all survive untouched. `reducedMotion` is never rescaled, and a missing, zero, negative or non-numeric request keeps the natural length. `mystic_motion.js` forwards the request to that single plan and `bevy_ui_flower_runtime.js` reads the preference on both of its `sample()` calls, so opening and closing stay one clock.

The Home panel edits that value with the product's own input box, `createNumericFieldRuntime({ id: 'home_mystic_timing', intentPrefix: 'home.mystic_timing', min: 0.3, max: 6, step: 0.1, initialValue: 2, unit: 's' })` in `bevy_panel_home_runtime.js`, mounted by `bevy_panel_home_view.js` directly after the `home_mystic_gap` radio so the Mystic block reads roundness, seam, then opening time. The field speaks seconds and stores milliseconds; only the END phase of a gesture persists, `bevy_panel_home_actions.js` passes `openingMs: visual.mysticOpeningMs` to the same `applyHomePreferences`, `syncMysticTiming()` re-reads the stored value on load and on every profile update, and `home.mystic_timing.` is in the restricted guest intents. Regression owners: `tests/eve/bevy_ui_mystic_menu.test.mjs` (the one factor, the invariance of the ratios, the bounds, the reduced-motion exception) and `tests/eve/bevy_panel_home_contract.test.mjs` (the node ordering, the field, the committed intent).
