...
- `eVe/intuition/runtime/bevy_panel/` owns the shared BevyUI panel shell. `bevy_panel_runtime.js` owns mounted panel lifecycle plus ephemeral drag/resize geometry; `bevy_panel_tree.js` owns its tree shape; `bevy_panel_tokens.js` derives all panel visuals from `EVE_PANEL_SKIN_TOKENS.bevyPanel`. The temporary development/test-only `panel_lab` surface validates this shell from a trailing main-ribbon tool and must be removed with its dedicated registration/tests after component migration completes.
