...
`eVe/domains/dashboard/dashboard_actions.js` owns only project-card intent and Dashboard teardown. It delegates the whole transition to `project_data.js#activateProjectWorkspace`, whose single path owns current-project publication, per-project view restoration, WebGPU loading/foregrounding, and main-menu readiness; the Dashboard action must not issue a second menu-visibility pass after activation.
