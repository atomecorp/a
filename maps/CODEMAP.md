...
- `eVe/intuition/tools/capture_recording_feedback_runtime.js` owns bounded Bevy tool feedback and post-stop reveal delegation for `capture.js`. It registers live sources through their renderer owners but creates no audio-scope canvas, frozen media clone, camera element, visible media node, or second renderer.
...
- `platforms/desktop-tauri/src/audio_engine/metering_scope.rs` publishes a lock-free fixed 64-bin scope snapshot from the existing CPAL recorder pass; `atome/src/application/audio_runtime/record_audio_scope_transport.js` polls `audio_get_scope` at 34 ms only while the matching Tauri session is active and dispatches ephemeral `native_audio_scope` frames. The snapshot is visualization data, not a second recorder or persisted state.
...
- `tests/eve/capture_recording_bevy_feedback.test.mjs` guards audio scope, live video, photo flash, session cleanup, and native BGRA frames inside the Bevy tools. `tests/probes/audio_recording_bevy_capture_probe.test.mjs` exercises browser audio through the registered `ui.capture.audio` runtime boundary and verifies its durable Atome, shared project canvas projection, file, and decode result without a fullscreen or document-pointer capture API.
