...
- `eVe/elements/skin/index.js` is the canonical skin entry point for Dashboard, tools/toolboxes, and panel chrome tokens. It owns `EVE_SKIN_TOKENS`, `resolveEveSkinTokens(domain, overrides)`, and `applyEveSkinVars(target, domainOrTokens)`. Domain modules may keep compatibility exports, but repeated color, opacity, shadow, backdrop, radius, size, spacing, and alignment defaults should be promoted into `eVe/elements/skin/` rather than duplicated locally.
