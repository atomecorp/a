...
- `assistant_runtime.js` exposes the singleton API, traces `voice.assistant.toggle`, owns voice/visual transitions and keeps its surface interceptor above Dashboard BevyUI only while active. It uses the canonical BevyUI hit-test and `BEVY_MAIN_MENU_TREE_ID` to yield only pointer sequences that begin on the main toolbox; the pointer id remains yielded through release, including the release that completes the hold which opened the assistant. `bevy_ui_main_menu_hold_runtime.js` retains the one-shot Atome activation suppression for a bounded 420 ms after release so the asynchronous BevyUI activation cannot turn a hold into an additional Dashboard click. `main_menu_bridge_runtime.js` bootstraps the singleton assistant on demand before invoking its sole `toggle()`, so a long hold cannot depend on unrelated boot timing. Dashboard and project contacts remain intercepted. Opening, touch response and farewell are distinct localized operations; user-close animation and farewell start together. Visual teardown alone returns the runtime to closed and the native farewell finishes on its captured voice session without blocking a later opening. Native Tauri STT cancellation settles from a successful bridge stop even without a later `idle` event.
...
- `eVe/intuition/ribbon/menu.js` remains the legacy DOM ribbon bootstrap/coordinator for surfaces that have not yet been migrated, but it is not instantiated by the bottom main-menu bridge. `menu_model.js` owns ribbon content cloning/patching, tool-entry normalization, handle/icon resolution, tool sizing/radius, color interpolation, transform composition, and pressed/tool visual-state helpers; its clone path recursively clones plain data while preserving function-valued action handlers instead of using browser structured cloning on runtime menu content. Runtime ownership is split across focused modules: `menu_shell_runtime.js` creates the ribbon/layer DOM shell, `menu_delete_visual_runtime.js` owns delete target/backdrop visuals and dragged-node attraction, `menu_drag_delete_runtime.js` owns drag-delete event routing, `menu_layout_runtime.js` owns metrics/overflow/reveal stops, `menu_scroll_runtime.js` owns wheel/native scroll snap behavior, `menu_tool_render_runtime.js` owns tool/palette button rendering and tool press hover routing, `menu_visibility_runtime.js` owns open/close/dismiss visibility state, `menu_placement_runtime.js` owns placement/handedness, `menu_public_api_runtime.js` owns the legacy public facade and latch adapters, `menu_auth_dock_runtime.js` owns auth dock transition animation, `menu_external_open_runtime.js` owns external-open width/geometry resync, and `menu_quick_capture_runtime.js` owns the Home-handle quick-capture overlay, reveal/hover state, fullscreen handoff, and sweep completion. `menu.js` must not reintroduce those helpers inline.
- `eVe/intuition/ribbon/bevy_ui_main_menu_model.js` owns the BevyUI main-menu item order, canonical expandable-child projection, item-size contract (`resolveDashboardBlockUnitSize() / 2`), handedness-aware bottom layout, Atome edge order, horizontal overflow metrics, per-block icon+label tree shape, shared icon sizing, single `lightgray` SVG icon tint, label vertical placement, and disposable BevyUI tree shape. Compact layouts keep fixed-size blocks and expose `scrollLeftPx` / `maxScrollLeft` instead of shrinking tools or using DOM scrolling; right-handed overflow defaults to the terminal scroll position so Atome starts pinned to the right edge, while left-handed overflow starts at `0`. `bevy_ui_main_menu_scroll_runtime.js` owns runtime-only wheel/drag horizontal scrolling, pixel/line/page wheel-delta normalization, handedness-aware vertical-wheel mapping, snapping, and activation suppression after scroll. `bevy_ui_main_menu_hold_runtime.js` owns pointer-independent hold timing and cancellation. `bevy_ui_main_menu_runtime.js` owns the `window.new_menu_v2` / `window.new_menu` facade, BevyUI mount/update/unmount calls, mounted-tree reserved-height reporting, scroll-render animation-frame coalescing, active surface/window resize observation, canonical palette state, tool latched/external-open visual state, direct leaf invocation, dedicated hold actions, hold-to-palette, and Atome dashboard toggle. It dispatches menu geometry changes after effective BevyUI mount/unmount. `main_menu_bridge_runtime.js` composes only this BevyUI facade and does not create a DOM ribbon.
...
- `eVe/intuition/ribbon/bevy_ui_flower_model.js` owns the disposable Flower BevyUI tree shape, radial item placement, and visual node contract.
