...
Choice palettes (2026-09-18): tool_definition_ssot.js owns resolveToolChoiceChild/resolveToolActiveChild/resolveToolChoiceIcon; shared/tool_presentation.js is the single label+icon resolution (activeToolIds optional, absent for the Mystic); toolbox_runtime_model.buildMainEntries({ activeToolIds }) and atome_contextual_edit_model.js consume it. Both renderers close on a leaf choice and replace a palette slot with its locked child (bevy_ui_main_menu_model.activeToolIdsFromLatchedState, atome_contextual_edit_runtime.activeToolIds); the keep_open_on_child_click / keepPaletteOpenOnChildClick opt-outs are gone. main_menu_content_runtime.js binds View and Mode to their canonical owners; bevy_ui_main_menu_runtime.js owns the disposable refresh and lock listeners. contextual_selection_levels.test.mjs, bevy_ui_main_menu_contract.test.mjs and create_tools_modes_contract.test.mjs cover shared choice resolution, closure and slot replacement.
