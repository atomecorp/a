...
- Product-neutral rounded shape and shadow support flows through `corner_radius` and `material.shadow` from render records to the Bevy projection adapter and Atome Bevy core. Rounded shape masks are generated by `atome/renderers/bevy-core/src/texture.rs`; disposable shape shadows are generated by `atome/renderers/bevy-core/src/shape_shadow_overlay.rs` using the existing Bevy shadow texture path and are rebuilt both for initial plugin scene spawn and later render ops. Dashboard cards use this contract only for `10px` radius and remain flat without `material.shadow`; dashboard entêtes use one external `header_side_shadow` image/texture gradient through the existing Bevy image path so the single lateral shadow reaches the tracks without a full-width shadow surface, shape-shadow blur, or interior band over the entête column.
