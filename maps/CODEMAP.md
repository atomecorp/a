...
- `eVe/domains/rendering/project_scene_webgpu_adapter.js` is legacy active-renderer debt for the Bevy migration. New project Atome rendering must use the Bevy media texture route instead of this adapter.
- Project selection visuals on the cleaned route stay in `eVe/domains/rendering/`: `project_scene_runtime.js` reads existing selection runtime state and marks disposable render atoms, `webgpu_compositor.js` exposes clip selection, `project_scene_selection_overlay.js` owns the cached outline bitmap, and `project_scene_webgpu_adapter.js` adds that layer to the same WebGPU canvas.
