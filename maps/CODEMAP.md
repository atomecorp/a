...
- `eVe/intuition/ribbon/bevy_ui_main_menu_model.js` owns BevyUI main-menu order, expandable-child projection, fixed item sizing, handedness-aware bottom layout, explicit canvas positions used by motion and hit-testing, Atome edge order, overflow metrics, icon/label shape, semantic palette-group accent/backplate projection, and disposable tree structure. Accent geometry, family mapping, contrast, and shadow parameters belong to `RIBBON_TOKENS.mainMenuPaletteAccent`; colors resolve from the shared skin semantic families. `bevy_ui_main_menu_palette_motion.js` owns the pure historical 180 ms open, 70 ms recoil, and 120 ms settle sampling plus its single interruptible `updateTreeMotion` controller, affected-node filtering, and reduced-motion path. `bevy_ui_main_menu_scroll_runtime.js` owns wheel/drag scrolling and activation suppression; `bevy_ui_main_menu_hold_runtime.js` owns hold timing. `bevy_ui_main_menu_runtime.js` owns mount/update/unmount, reserved-height reporting, resize observation, palette and tool visual state, invocation, Atome Dashboard toggle, frame-coalesced cosmetic renders, and structural palette-render priority. The shared BevyUI runtime applies progressive image hydration only to fresh mounts; live main-menu updates reuse the texture cache without frame-budget waits. `bevy_ui_product_registry.js` owns lightweight internal per-window access and geometry readers; `bevy_ui_product_runtime.js` composes and registers the menu and Flower. No browser menu alias or DOM main-menu factory exists.
