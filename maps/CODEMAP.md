...
- `eVe/domains/rendering/project_scene_runtime.js` owns active project scene projection state keyed by project id. It accepts canonical project Atome records, normalizes them into `RenderAtom` entries for hit-testing only, builds the disposable Virtual Scene projection, sends initial nodes and diffs to browser Bevy, updates the bounded project render surface, and returns projection handles instead of per-Atome DOM hosts.
...
- Project selection visuals on the cleaned route stay in `eVe/domains/rendering/`: `project_scene_runtime.js` reads existing selection runtime state, marks disposable render atoms, and projects the selected state through Bevy diffs on the same project canvas. No per-Atome DOM outline, host class, selection overlay DOM, or legacy WebGPU selection bitmap is allowed on the active project route.
