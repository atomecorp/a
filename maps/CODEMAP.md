...
- `eVe/intuition/tools/workspace_main_menu_visibility.js` owns the post-auth/workspace invariant that the canonical `window.new_menu_v2` main menu is mounted and visible. `eVe/intuition/tools/user_workspace_surface_runtime.js` owns post-session Dashboard surface activation for authenticated and anonymous sessions and calls that helper after Dashboard open, Dashboard repair, and Dashboard-to-project restoration. It no longer resolves or loads the current user project for the default Dashboard path; it imports/uses `dashboard_bevy_ui_runtime.js`, runs data-only Dashboard warmup for `__eve_dashboard_workspace__` when needed, then ensures the full-viewport neutral `project_view___eve_dashboard_workspace__` host owns the shared `eve_surface_project` canvas before opening `window.eveDashboardBevyUiRuntime` on that neutral scene. `eVe/intuition/tools/dashboard_project_record_bootstrap.js` separately guarantees that an authenticated dashboard session has at least one durable project record without setting it current, activating it, or loading project Atomes. Its readiness contract requires an active `window.eveDashboardBevyUiRuntime` on `__eve_dashboard_workspace__` with mounted Bevy UI diagnostics. This owner does not create a second dashboard entry point, duplicate menu state, dashboard DOM, an alternate canvas, or a background project load.
