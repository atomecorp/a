...
- `assistant_runtime.js` exposes the singleton API, traces `voice.assistant.toggle`, owns voice/visual transitions and keeps its surface interceptor above Dashboard BevyUI only while active. Opening, touch response and farewell are distinct localized operations; user-close animation and farewell start together, and teardown releases interaction only after both finish.
...
- The procedural SDF record now spans the workspace while layout uniforms retain the centered bounded shell; its fixed four-point cut path drives curved GPU tearing and local-normal lobe separation without another surface.
