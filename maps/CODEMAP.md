...
- `eVe/domains/dashboard/dashboard_actions.js` owns dashboard item click actions and reasserts the workspace main menu after project-card activation. A short header press toggles the Dashboard creation mode (`dashboard_creation_runtime.js`); calendar/contact cells only open the registered editor through `openPanelSurface` and never load the Contacts API or mutate the directory. Focused entêtes still expose no `+` creation strip or plus rail. In focused category mode, item hit-testing keeps the real item category separate from the visual lane category. Project item clicks enter workspace `transition`, fade out and tear down the dashboard, then activate/load that project as the active Bevy desktop instead of opening a dashboard fullscreen detail. Its Settings header opens Home through that surface session owner [ensureToolModule(home) then window.open_home_panel] and never calls openPanelSurface(home) directly: the raw surface skipped guest/session resolution, so a Guest workspace loaded the profile as an account and the panel fell into its unavailable-profile state instead of the read-only Guest Home.
