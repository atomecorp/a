...
- `eVe/intuition/ribbon/bevy_ui_main_menu_model.js` owns the BevyUI main-menu item order, item-size contract (`resolveDashboardBlockUnitSize() / 2`), handedness-aware bottom layout, Dashboard-first edge order, Dashboard grid primitive, per-block icon+label tree shape, shared icon sizing, single `lightgray` SVG icon tint, label vertical placement, and disposable BevyUI tree shape. `bevy_ui_main_menu_runtime.js` owns the `window.new_menu_v2` facade implementation, BevyUI mount/update/unmount calls, active surface/window resize observation, tool latched/external-open visual state, Dashboard grid toggle, and manual legacy-menu toggle. `main_menu_bridge_runtime.js` composes the private legacy ribbon and the BevyUI menu facade without allowing automatic DOM ribbon fallback.
- `eVe/domains/rendering/bevy_ui_runtime.js` owns the internal JavaScript BevyUI facade `window.eveBevyUiRuntime`: it normalizes Squirrel-owned UI trees, stores the interaction tree in CSS pixels, projects the visible product path through non-selectable WebGPU project overlay records, and must not create DOM nodes per component. Native BevyUI WASM op submission is explicitly opt-in so the overlay-backed product menu does not emit unused Bevy UI events. Focused helpers keep the rendering concerns closed: `bevy_ui_image_runtime.js` hydrates `icon` / `image` sources and label textures into RGBA payloads through the shared Bevy media texture resolver, `bevy_ui_render_scale.js` keeps the submitted UI tree in logical BevyUI units, `bevy_ui_project_overlay_runtime.js` projects the same disposable BevyUI tree as non-selectable WebGPU project overlay records with minimal flex alignment for the visible product path, `virtual_scene_contract.js` preserves prehydrated overlay `bevyTexture` payloads and resource signatures through the Virtual Scene, `bevy_media_resource_runtime.js` must not re-resolve nodes that already carry a Bevy texture, and `bevy_ui_value_runtime.js` owns scalar/color/id normalization. `atome/renderers/bevy-core/src/ui/mod.rs` owns the corresponding native BevyUI ECS tree, image-node texture spawning, diagnostics, and event queue, while `ui/types.rs` owns the serializable UI op/node contracts, `ui/tests.rs` owns the focused Rust UI tests, and `platforms/web/bevy-renderer/src/exports.rs` exposes the opt-in native exports `apply_atome_bevy_ui_ops`, `read_atome_bevy_ui_diagnostics`, and `drain_atome_bevy_ui_events`.
