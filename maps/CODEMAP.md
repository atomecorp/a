...
- `bevy_ui_project_overlay_runtime.js` forces non-atomic overlay projection only when `previousIds` is empty. Mounted Dashboard updates therefore reuse `project_scene_direct_transform_runtime.js` for pure position changes during wheel, inertia, and snap frames; the main-menu tree remains prefix-atomic and forced.
...
- `assistant_runtime.js` exposes the singleton public API, traces `voice.assistant.toggle`, blocks project interaction, owns Escape/teardown, and coalesces TTS signal state into animation frames. Its single frame loop also owns the public `hidden`/`appearing`/`settling`/`visible`/`disappearing` transition state. It mounts and patches the invisible material before starting the absolute 420 ms reveal clock, so renderer warmup cannot consume the animation; voice starts only after that reveal. Visual ownership closes deterministically after the 320 ms disappearance, and a delayed native voice-stop promise cannot retain `active` or overwrite a later session. `index.js` only installs the assistant API; it must not allocate the ONNX engine while Bevy/WASM is starting. The local voice warmup is scheduled once by `boot_runtime.js`, twelve seconds after a workspace is confirmed active.
- `assistant_scene.js` mounts the assistant and its neutral 8% workspace dim record once through the existing project-scene overlay owner, then patches only opacity and procedural uniforms through the canonical direct-style route. Their panel-local order normalizes immediately above the Dashboard token range ending at 1420 and below panel/menu surfaces; both ephemeral records survive reconciliation until explicit teardown.
...
- `atome/renderers/bevy-core/src/procedural_sdf.rs` and `assets/shaders/procedural_sdf.wgsl` own the generic bounded assistant material quad. `workspace_backdrop.rs` owns the reusable full-resolution workspace capture, presentation quad, resize reuse, and first/last SDF activation lifecycle. `workspace_blur.rs` and `assets/shaders/workspace_blur.wgsl` own two ordered compositor passes on private render layers: a normalized horizontal Gaussian pass followed by a vertical pass. `AssistantOpticsSettings` centralizes the bounded optical defaults (24 logical-pixel blur, 24 logical-pixel refraction, 94% glass mix, 20% rim start, 10% halo) and converts the two radii through the active DPR before material creation. The SDF samples the original and final blurred targets, replaces the sharp backdrop only inside its shell mask, and applies continuous pixel-space refraction across the useful shell radius before its cyan/rose material. The web build emits classic and ESM renderer-version modules; `bevy_web_renderer_module_loader.js` appends that content version to both wrapper and WASM URLs so an old module instance or service-worker cache cannot silently retain a previous shader after reload. The iOS `AudioSchemeHandler` strips query components only for validated filesystem resolution and serves the original versioned custom-scheme request without a redirect, allowing WebKit to import the wrapper and fetch its WASM binary. `bevy_projection_adapter.js` and `bevy_renderer_adapter_registry.js` retain the strict procedural animation uniforms. No assistant canvas, visible DOM surface, private renderer, or blur fallback exists.
...
- `eVe/intuition/flower/menu.js` owns flower menu DOM orchestration; `menu_layout.js` owns radial geometry, `menu_items.js` owns item/icon normalization, `context_target.js` owns context target resolution including lazy project-canvas Atome hit testing, rejects points owned by mounted BevyUI system trees through `eveBevyUiRuntime.hitTestAtClientPoint(...)`, and rejects the whole project surface while the modal system assistant reports an active state, `context_selection.js` owns active-selection and mixed-kind menu strategy, and `context_pointer_lock.js` owns flower pointer locks.
