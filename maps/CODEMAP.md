...
- `eVe/intuition/ribbon/menu.js` and its focused helpers compose only the transitional Atome-edit footer ribbon. The visible main menu is BevyUI-only; its Atome item calls `toggleWorkspaceDashboardAndMainMenu({ source })`, while `tool.main.home` remains a normal `ui.home.panel` route.
...
- `eVe/intuition/ribbon/menu.js` is the footer-only DOM ribbon coordinator used by the Atome-edit footer. Its focused shell, layout, visibility, tool-render, delete, placement, and interaction helpers remain footer implementation details; they do not expose or construct the product main menu.
- `eVe/intuition/ribbon/bevy_ui_main_menu_model.js` owns BevyUI main-menu order, expandable-child projection, fixed item sizing, handedness-aware bottom layout, Atome edge order, overflow metrics, icon/label shape, and disposable tree structure. `bevy_ui_main_menu_scroll_runtime.js` owns wheel/drag scrolling and activation suppression; `bevy_ui_main_menu_hold_runtime.js` owns hold timing. `bevy_ui_main_menu_runtime.js` owns mount/update/unmount, reserved-height reporting, resize observation, palette and tool visual state, invocation, and Atome Dashboard toggle. `bevy_ui_product_registry.js` owns lightweight internal per-window access and geometry readers; `bevy_ui_product_runtime.js` composes and registers the menu and Flower. No browser menu alias or DOM main-menu factory exists.
