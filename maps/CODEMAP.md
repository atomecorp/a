...
2026-09-25 Mystic release, dwell and fling: `eVe/intuition/ribbon/bevy_ui_mystic_runtime.js` now owns one stationary-dwell timer (`MYSTIC_AUTO_OPEN_DWELL_MS` 1500 ms, `MYSTIC_AUTO_OPEN_DWELL_TOLERANCE_PX` 6 px, source `bevy_mystic_dwell`) for the four persistent tools, the palettes and the root return, in place of the deleted immediate hover opening; the release launches the tool OR opens the palette under the finger (`bevy_mystic_release`) but never the centre, never the root return and never a `disabled` item, and `MYSTIC_POST_REPLACEMENT_MOVE_PX` (12 px) of fresh movement is required after any geometry replacement; `activateDirectional({ direction, event, source })` resolves a cardinal fling to the configured cross slot (`bevy_mystic_fling`, root only, sources in `eVe/intuition/mystic/context.js`). Evidence: eVe/documentations/MYSTIC_RELEASE_DWELL_FLING_VALIDATION_2026-09-25.md.

2026-09-25 Active-tool rail repair: `eVe/intuition/runtime/eve_intuition/atome_contextual_edit_projection_runtime.js` owns the extracted disposable rail projection, serialized rendering, activation order and existing shared pulse. The model resolves labels/icons; `eVe/intuition/tools/core/active_tool_registry.js` supplies live facts and invocation targets. Runtime and handlers consume the same displayed definitions. Recording bootstrap retains the originating Molecule group until stop. Evidence: `tests/eve/contextual_rail_active_target.test.mjs`, `tests/eve/contextual_rail_transport_stop.test.mjs`, `tests/eve/contextual_rail_recording_owner.test.mjs`.

2026-09-23 Terminal renderer failures paint their cause instead of ending on a blank page: `eVe/intuition/runtime/eve_intuition/boot_failure_surface.js` is the single owner of what a dead display lane can still say — plain DOM, no renderer, no design system, because the lane that would draw it is the one that just failed (the same reason `TAURI_LOCAL_HTTP_FAILURE_SCRIPT` exists in `platforms/desktop-tauri/src/lib.rs`). `boot_runtime.js` keeps the ordering it already owned: the new `settleTerminalWorkspaceBoot(result)` is the one path that stops the retries for good, and it now calls `presentBootFailureSurface({ failure: result })` after recording `workspaceBootOpenTerminalReason`. The surface is idempotent by id (`eve_boot_failure`), paints only the two terminal renderer patterns (`bevy_renderer_wasm_panic:`, `bevy_renderer_start_failed_terminal:`), names the cause it can prove (`gpu_unavailable`, `present_timeout`, `renderer_failed`), carries the exact failure message a support log needs, offers one Retry that reloads the page, and stays silent on `remote_account_not_provisioned` and on every transient failure the boot still retries — the login surface carries the action that fixes an unprovisioned account, so painting over it would hide the cure behind the symptom. i18n: `eve.boot.failure.*` (title, gpu_unavailable, present_timeout, renderer_failed, data_intact) in both locales. It also writes one self-report line in the device log (`[boot-failure] surface painted: <cause key> <message>`) because the app's diagnostics owners belong to the dead lane; `scripts/android/apk.sh` greps that prefix and prints a `boot failure surface painted` verdict next to its white-screen verdicts. Evidence: `tests/eve/boot_failure_surface_contract.test.mjs` (painted surface, exact detail text, idempotence, injected retry, plus two real boot-runtime runs: terminal paints and stops retrying, transient retries and paints nothing); the module sits inside the bundled critical graph (`chunks/eve/critical-bootstrap.js` of `temp/ios_runtime_package_verify`). The Android/emulator acceptance of the real run stays To verify.

2026-09-23 Dashboard filter hides the fixed modules: `dashboard_news_modules.js#items(source, { filteredCategoryId })` omits the weather (`news`) and clock (`calendar`) module cards whenever a filter is open, and `dashboard_bevy_ui_runtime.js#readItemsForRender` — its only caller — passes `state.filteredCategoryId`, so no foreign pinned card survives in the cascaded lanes. The module runtime is never stopped (minute timer, 15-minute weather refresh, loaded city and readings), so restoring the base display re-projects both cards immediately without a new weather read, and the memoized items map stays untouched. Evidence: `tests/probes/dashboard_weather_runtime.test.mjs`, `tests/eve/dashboard_bevy_ui_runtime_contract.test.mjs`, `temp/rail_dashboard_check.mjs` (browser probe, not executed here).

2026-09-22 Dashboard rail two-tier tools: every rail header splits into an upper two thirds band (tool icon + rubrique label, the filter surface) and a lower third band (`Nouveau`, the creation surface). `dashboard_layout.js#dashboardHeaderBands` is the single geometry owner: `createDashboardLayout({ filteredCategoryId })` exposes `header_filter_rect` / `header_new_rect` per lane and `hitTestDashboardLayout` keeps `kind: 'header'` while adding `zone: 'filter' | 'new'`; `dashboard_header_records.js` derives the icon/label bounds, the `2 px` `header_separator_*` line (inset by `tokens.metrics.gap`, `headerSeparator` token) and the `header_new_*` label from that same helper, so no constant is duplicated. `dashboard_environment.js#itemsForRender(categories, filteredCategoryId, itemsByCategory)` owns the cascade projection (the filtered items cascade from their own lane, every other lane is emptied) and its memo key includes the filter id. `dashboard_bevy_ui_runtime.js` owns `state.filteredCategoryId`, mutually exclusive with the creation `state.activeCategoryId`: the lower band keeps `activateCategory` unchanged, the upper band toggles the filter through `toggleCategoryFilter` with a targeted category reload and `scrollByLane = {}`, and no transition module is reintroduced — vertical and horizontal scrolling stay the layout's. i18n: `eve.dashboard.header.new` (`Nouveau` / `New`). Evidence: eVe/documentations/FRAMEWORK_STATE.md.

2026-09-22 Contact vignette live refresh: `eVe/domains/user/contact_change_events.js` is the shared subscription contract for the contact population — it re-exports `eve:people-directory-updated` (dispatched by `atome/src/squirrel/contacts/bootstrap.js` after a local contact create/update/delete) and `eve:user-profile-updated` (dispatched by `eVe/domains/user/profile_api.js` after a profile identity commit). `eVe/domains/dashboard/dashboard_data_controller.js` consumes it through `connectDirectoryChanges()` beside the existing `public_directory_events.js` subscription, both feeding one coalesced `refreshContactsPopulation()` closure that drops the cached `contacts` items (even while the Dashboard is closed, like `connectProjectChanges()`) and reloads plus repaints only while the Dashboard is active, so a renamed contact or a replaced photo repaints its Dashboard vignette without an application restart and a reopened Dashboard never repaints the stale cache. The invalidation was documented in the maps since the Bevy UI Dashboard migration (`cdf9b5e6`) but had no runtime subscriber, because `domains/dashboard/dashboard_data_invalidation.js` was deleted in that commit. Evidence: `tests/probes/contact_change_invalidation_contract.probe.mjs`, `tests/probes/public_directory_invalidation_contract.probe.mjs`, `eVe/documentations/FRAMEWORK_STATE.md`.

2026-09-22 Dashboard rail Settings removal: dashboard_layout.js reserves one rail header per visible category only — the extra settings slot, its index shift and the layout.settings geometry are gone, and hitTestDashboardLayout no longer answers a settings kind. dashboard_records.js paints no __eve_dashboard_header_bg_settings record and dashboard_svg_sources.js carries no settings icon. dashboard_bevy_ui_runtime.js registers no settings hit, and dashboard_actions.js exposes activateItemAction and openProjectItem only: openSettings, its openHomePanel injection and the internal home-module bridge are removed, so the main-toolbar Home tool is the single entry point of the Home panel. Evidence: eVe/documentations/FRAMEWORK_STATE.md.
...
The implementation/acceptance report lists remaining gaps: eVe/documentations/MYSTIC_VALIDATION_2026-09-20.md.


### Nested crop and source-space media projection — 2026-09-17

- `surface_interaction_runtime.js` also serializes the asynchronous `select` + `atome.edit.enter` transition behind the next pointer, wheel, or native gesture. An immediate pinch/Ctrl-wheel after a video or audio double-click therefore enters `spatial_crop`/`temporal_crop` before generic resize can start.
- `atome_contextual_kind.js` reuses the media playback normalizer so persisted `video_recording` and `audio_recording` kinds enter the existing video/audio crop owners; photo/picture aliases converge on image. Inline SVG RenderAtoms enter the existing `svg_vector_edit_runtime.js` instead of being rejected by the media-only double-click filter.
- `atome_contextual_edit_runtime.js` keeps an exact edited descendant active when selection changes inside an edited Molecule; an ancestor group cannot reclaim an already active media crop session.
- `surface_interaction_runtime.js` owns recursive open-Molecule target resolution; it exposes a raw descendant when any ancestor remains in contextual edition and otherwise resolves the closed structural root. Mystic consumes the same resolver.
- `bevy_media_texture_resolver.js` keeps cropped images in natural source aspect and waveforms in full-peak source coordinates, reusing the bounded shared texture cache.
- `render_atom.js` and `bevy_renderer_adapter_registry.js` derive image/video and temporal waveform `uv_rect`; Bevy core `spawn.rs`/`resource_ops.rs` apply it and keep `AtomeSpriteSourceRect` synchronized through clipping.

### Contextual crop and footer removal — 2026-09-17

- Crop math and bounded property projection: `eVe/domains/rendering/media_crop_model.js`.
- Natural canvas gestures: `surface_interaction_runtime.js` and `surface_pinch_runtime.js`; Structured visual gestures: `project_view_visual_interaction_runtime.js`.
- Disposable preview and terminal commit: `project_scene_engine.js`.
- Image/video `source_rect` projection: `render_atom.js`, `bevy_projection_media_contract.js`, `bevy_renderer_adapter_registry.js`, and Bevy core `texture.rs`/`spawn.rs`/`resource_ops.rs`.
- Temporal waveform/playback: `project_view_list_preview_model.js`, `project_audio_playback_progress_runtime.js`, `selected_project_media_playback_runtime.js`, `media_playback_audio_executor.js`.
- Contextual UI now owns rail/vector nodes only: `atome_contextual_edit_model.js`, `atome_contextual_edit_runtime.js`, `atome_contextual_edit_handlers.js`; `atome_contextual_fullscreen_runtime.js` is deleted.
- `surface_interaction_runtime.js` preserves canonical `group`/`molecule` routing independently of projected `shape`; `media_crop_model.js` owns fit-derived image source windows and `surface_pinch_runtime.js` owns terminal pinch commit/cancel lifecycle.

### Canvas lasso Delete and iOS Molecule parentage — 2026-09-16

`eVe/intuition/runtime/eve_intuition/atome_contextual_rail_definition_invocation_runtime.js` distinguishes a structured List/Matrix rail (`record.structured_context`) from the Natural canvas rail. Structured Delete remains bound to its active row; canvas Delete forwards the complete canonical selection, including every invisible Molecule owner selected by a lasso. `eVe/intuition/tools/core/tool_runtime_molecule_structure.js` remains the whole-owner deletion authority. Native iOS event projection persists structural `parent_id`; derived `molecule_timeline` clips are not membership authority and must not keep an empty Molecule alive.

### Molecule-source composition and contextual Ungroup — 2026-09-15

`surface_text_pointer_runtime.js` records the number of gesture sources separately from `surface_pointer_runtime.js` expanding a single Molecule to its descendants for collective motion. `project_view_drop_feedback.js`, `surface_interaction_runtime.js`, and `project_scene_engine.js` carry that source count through the existing held composition palette and terminal `drag.end`, so one Molecule source can use all six choices without enabling composition for a genuine multi-selection. The result still delegates to `combineCanonicalMolecule`, which preserves the source Molecule envelope and nests it inside the new wrapper. Mystic continues to resolve closed descendants to their Molecule owner; its existing `ungroup` capability invokes `ungroupCanonicalMolecule`, with the French presentation labelled « Dégrouper ».

### Natural project selection closes the Atome rail — 2026-09-15

`eVe/core/atome_events/project_layer_routing.js` keeps its existing responsibility of publishing the project id as the canonical replacement selection. `eVe/intuition/runtime/eve_intuition/atome_contextual_edit_runtime.js` remains the sole consumer that owns Natural contextual lifetime: when the replacement selection does not resolve to an Atome in the active Natural scene, it removes stale selection-level context and deactivates the rail while preserving explicit edition sessions. No second selection listener or UI owner is introduced.

### Gesture, lasso and history owners — 2026-09-14

- `eVe/domains/rendering/surface_pointer_runtime.js` owns proportional target geometry and shared size limits. `eVe/domains/rendering/surface_pinch_runtime.js` owns multi-contact/wheel lifecycle only; surface, contextual footer and Size inputs reuse the shared calculation. Invisible edge hit-testing and object-footer resize grips are removed.
- `eVe/shared/lasso_context_zone_runtime.js` retains only the waiting rectangle lifetime and fixed context. The official Mystic context owner and shared Bevy Mystic consume Text, Capture, Molecule, Shape, Copy, Paste and Delete descriptors; the temporary menu support module and its buttons are removed. Capture reuses `eVe/shared/lasso_context_capture.js` and the shared Bevy project-preview adapter, including crop origin.
- `eVe/core/atome_commit_state.js` tracks pending mutation completion without another history. Scene mutations converge on one batch commit; `eVe/core/atome_timeline.js` serializes history actions after this barrier and resolves the remaining targets of partially undone transactions. `server/atomeHistoryCommands.js` applies optional selection IDs inside the source transaction and uses the canonical history classifier to exclude replay-only gesture frames from inversion; `server/atomePropertySecurity.js` preserves readable gesture completion and history source metadata.
- `eVe/intuition/mystic/context_pointer_lock.js` announces pointer takeover when Mystic opens. The existing DOM drag and shared WebGPU surface owners consume that signal to release their sessions immediately while selection remains canonical. `copy.js` accepts Mystic's captured selection, while `paste.js` delegates Atome cloning and internal-reference remapping to the existing duplicate owner.
- `eVe/intuition/runtime/eve_intuition/main_menu_edit_content.js` shares edit definitions between menu consumers; Redo delegates directly to existing `ui.redo` without a self-invoking touch callback.

OpenAI attachment ownership: assistant_media_session retains explicit files via asset_box_upload_transport.sendFileToServer and asset_box_atome_store.createUploadAtome, then records references through conversation_store. asset_box_file_upload.js owns the extracted file upload protocol (312 lines before lifecycle additions), reducing asset_box_upload_transport.js to 341 lines and exposing upload without loading the legacy uploads UI. Existing remote-wallpaper behavior remains in its original owner and passes its probe. conversation_store serializes saves and rejects account changes/deleted records. Finder closes normally after resuming a saved discussion.


OpenAI acceptance owners: tests/probes/openai_assistant_ui_acceptance.mjs reuses the canonical canvas harness for real held-pointer, text-focus and level-drag checks. assistant_scene.js anchors the procedural record to main_menu_tool_atome_icon_image; assistant_visual_contract.js converts top-origin scene coordinates to the existing bottom-origin material coordinates. auth_users.authenticatedUserSnapshot owns private self-authentication projection.
### OpenAI integration owners — 2026-09-10 (acceptance in progress)

- Native transport: Rust `platforms/desktop-tauri/src/server/provider_relay.rs` binds provider sockets to current remote account credentials; `platforms/desktop-tauri/src/server/ws_api.rs` owns extracted WS API dispatch. iOS `FastifySyncClient.swift` owns the equivalent relay invoked by `LocalHTTPServer.swift`. No primary OpenAI key is returned to native clients.
- Server: `providerHostedSession.js` owns connection-bound, one-use Remote MCP approvals. `providerCredentialVault.js` adapts the existing encrypted token vault; `wsAiProviderOperations.js` dispatches authenticated provider requests in the existing socket. `providerRealtimeSession.js` owns the separate lifetime of provider control sockets and remote hangup, not Atome mutations.
- Shared AI: `provider_broker.js` consumes the existing authenticated transport; `openai_stream.js` decodes SSE. `conversation_session.js` owns temporary public turns, bounded MCP sequences and pending decisions; `conversation_store.js` persists typed conversations through canonical Atome APIs. Finder consumes those records.
- Media/UI: `realtime_session.js` owns duplex capture/connection lifecycle. `tts_runtime.js` replaces `local_tts_runtime.js` with one PCM playback owner for local and OpenAI synthesis. `assistant_media_session.js` owns temporary references/results; `svg_draw_mask_runtime.js` composes existing Drawing geometry and shared project export. Assistant views consume main-menu inline controls and shared panel primitives.


### Dashboard, List and capture repairs — 2026-09-10

`eVe/domains/dashboard/dashboard_layout.js` and `dashboard_records.js` project the creation grid of the active header (`dashboard_creation_catalog.js` / `dashboard_creation_runtime.js` / `dashboard_creation_records.js`). `dashboard_creation_actions.js` creates through the existing project and News commands. News template vocabulary remains owned by `news_template_model.js`.
...
- project_view_drop_feedback composes the shared tool factory and popup placement for six explicit choices; it resolves the terminal release point as the sole valid choice. The original Natural/List/Matrix session continues moving the dragged source beneath the higher-depth projection. Beyond the 20 px outer-bound margin, the existing projection becomes empty and non-selectable but stays mounted until the pointer session ends; this avoids a transient surface resize from terminating a held drag when another stacked Atome is revealed. The same tree is refreshed for a later stationary target, while the already armed target prevents immediate rearming until it is left. Existing gesture sessions own the single final cleanup, and an outside release cannot reuse a previously hovered choice.
...
- Dashboard creation mode (2026-09-18): a SHORT press on the lower third of a creatable header (`projects`, `news`, `calendar`, `contacts`) replaces every card lane with a full-height grid of creation cells drawn with the card glass; a press on the lower band of ANY header (active or dimmed) closes it. `dashboard_creation_catalog.js` owns the steps (project families/goals, News shapes and the custom view/content steps, calendar/contact editor cells), `dashboard_creation_runtime.js` the path/toggle/busy state (`state.activeCategoryId` + `state.creation`), `dashboard_layout.js#layoutDashboardCreation` the grid (anchored to the header side, shrinks to fit), `dashboard_creation_records.js` the cells, and `dashboard_creation_actions.js` creates through the canonical project/News creators then opens through `actions.openProjectItem`, or opens the registered `calendar`/`contact` editor. Non-concerned headers are drawn at 0.35 opacity. The upper two thirds of the same header belong to the category filter (see the two-tier rail entry), so creation and filter stay exclusive. There is no header long press and no creation panel. `core_atome_types.js` declares project intent; no separate project store.
- `dashboard_news_modules.js`: disposable clock/current-weather readings and manual-location panel. Existing Finder geocoder owns debouncing/cancellation; shared Dashboard layout keeps two module cards outside scrolling News, and `items(source, { filteredCategoryId })` omits both while a category filter is open so a filtered display never shows a foreign pinned card. Dashboard shutdown closes the panel and stops its timer/requests.
...
`package_ios_runtime.mjs` compiles Spark, eVe, deferred features, optional
integrations and the preview frame in one split ESM graph. Stable entries retain
their scheme paths, while shared chunks ensure every browser input and the
view-mode, audio/video and BevyUI registry singletons are emitted exactly once.
...
`__ATOME_PACKAGED_MODULES__`, and copies only distributed non-JavaScript
assets. `AudioSchemeHandler.swift` resolves `atome://` directly from the one
`atome_runtime` root per target, streams large files, and owns the aggregate
request/byte/missing counters. `WebViewManager.swift`, its navigation/script
message extensions, and `application/ViewController.swift` own monotonic native
milestones, versioned ready messages, WebContent termination reporting, and the
native launch/error overlay. `project_bootstrap.js` remains the only current
project/session owner; `user_workspace_surface_runtime.js` selects saved-project
resume or Dashboard fallback; `boot_runtime.js` validates the presented WebGPU
workspace and Main Toolbar before publishing readiness.
`FeatureFlags.swift` keeps Panel Lab, Web Inspector, and text tracing behind
explicit Debug launch opt-ins. `AudioSchemeHandler.swift` maps the scheme root
to the real packaged entry, while `build_bevy_renderer.sh` links the optimized
iOS Rust archive in normal Debug builds and reserves the dev archive for
`ATOME_IOS_RUST_DEV=1`.
...
protects dedicated handlers; `atome_contextual_rail_selection_runtime.js` supplies
...
During saved-project boot, `tool_genesis_project_load_runtime.js` forwards its
already loaded record snapshot through `project_view_mode_state.js` to the
single structured surface owner. `project_view_records.js` normalizes that
snapshot for the first page and `project_view_surface_context_runtime.js` reads
the root playback rule from it, avoiding duplicate native state reads before
`bootPresentationReady` without retaining another cache or state authority.
...
UI preparation and motion (2026-08-27): `bevy_ui_mystic_motion.js` derives
...
Every user-facing project open (Matrix, Dashboard card, Finder row) activates
with `staleFirst: true`, so the switch projects the canonical local snapshot and
the load owner schedules its single authoritative refresh after presentation; an
explicit `staleFirst: false` or an empty local snapshot still reaches the
balanced route, which is what keeps an unreachable backend from holding the
project open.
...
multi-voice start/cleanup, and explicit preview-duration handling;
`molecule_support.js` resolves the existing extracted video-audio asset without
losing its canonical owner; and `molecule_session_clips.js` publishes per-clip
load failures instead of silently degrading the mix. Video pixels remain owned
by `bevy_video_decode_source_runtime.js`, whose retired decoder generations
ignore late load/end/error events, while every audible source (including video
audio) starts exactly once through Kira and the projected decoder stays muted.
`project_view_surface_runtime.js` forces a canonical Natural reconcile before
publishing the mode, retains the projected video source identity, and resets
shared navigation when the project changes. `project_view_records.js` owns that
reset and invalidates pending reads before replacing view state; List, Matrix,
Mix, Timeline, and Atome Detail reuse it. `project_view_insertion_target.js`
rejects a temporal insertion level inherited from another project.

`project_view_molecule_list_model.js` is the shared List/Matrix presentation
resolver: a Molecule expands to its direct `parent_id` Atomes only. Section,
...
`Atome.commitBatch`: a member hit resolves to the existing Molecule owner, each
new member receives that owner as `parent_id` plus one clip at `t=0`, and new
visual members append above the current maximum `z_index`/`order`. Internal
Plan changes route through the canonical scene intent and alter only persistent
visual depth. `tests/probes/molecule_ui_layered_media.mjs` is the persistent
headed Chromium acceptance owner for the six-member video/audio/text/image
composition, three simultaneous Kira voices, natural timing,
...
`project_view_playback_follow.js` owns armed selection follow. Drag geometry and
the shared 500 ms Natural/List/Matrix threshold lives in
`project_view_drop_intent_runtime.js`; exact ordering, pending transaction
confirmation, and batch persistence live in `project_view_order_runtime.js`.
`project_view_list_drag_runtime.js` and `project_view_matrix_drag_runtime.js`
translate Bevy gestures into those shared intents. The Matrix content and shared
selectable-list preview remain disposable projections; canonical fusion stays in
...
source from overlap hit-testing. A Natural schema-v2 Molecule enters the same
`project_view_molecule_info.js` contextual transport as List and Matrix.
...
Current assistant ownership (2026-08-10): `eVe/voice/assistant/` owns the closed full-screen Bevy assistant and the Atome-tool 520 ms toggle; `atome/src/squirrel/voice/assistant_session_controller.js` owns conversational turns, shared Home endpoint thresholds, a half-duplex TTS/acoustic-drain guard, residual anti-echo filtering, and strict MCP execution selection. Native STT is never opened while local TTS is active. `orchestrator_bridge.js` owns MCP transport, while `orchestrator_agent_toolchain.js` owns MCP AI toolchain result/confirmation normalization. `provider_client.js` owns active-provider/key resolution. Home's Bevy provider/model/key UI remains in `bevy_panel_home_{view,vault_runtime}.js`; it persists metadata in `profile.passkeys.keys` and secrets only in the token vault.
...
Current account/guest ownership (2026-07-30): `server/wsApiAuthProvisioning.js` owns the bounded, fingerprint-gated Fastify provision action; `server/wsApiGuestAdoption.js` and `server/wsApiGuestAdoptionFiles.js` own the remote guest-adoption journal and staged file completion. `server/wsApiIdentity.js` verifies that a JWT principal is a persisted Fastify user before remote events, state, snapshots, sync, sharing, or direct messages proceed. Browser guest identity is owned by `adole_api/session.js`, `auth_methods_session_account.js`, and IndexedDB-only `guest_workspace_store.js`; it is CSPRNG UUID v4 local state, not a user Atome or remote credential. `auth_workspace.js` keeps adoption explicit and sends the confirmed browser manifest through `/ws/api` before it clears the local guest space.

Current production identity deployment contract (2026-08-10): `scripts/server_secure_config.js` is the single deployment owner for JWT/cookie secrets and the persistent Fastify signing identity used by inter-runtime account provisioning. It validates configured RSA key pairs, fails closed on incomplete or mismatched paths, generates one stable identity under `/etc/squirrel/identity` when absent, records absolute key paths plus a stable server id/name in `/etc/squirrel/squirrel.env`, enforces key permissions, and backs the identity up before an update. `scripts/server_update.js` only orchestrates this owner; `server/serverIdentity.js` owns key creation, validation, and fingerprint derivation at runtime and for the manual generator.
...
| Project name, deletion, duplication and project clipboard | `eVe/intuition/matrix/core/project_data.js` (`updateProjectName`, `deleteProjectRecord`, `duplicateProjectRecord`, `copyProjectSnapshot`, `pasteProjectSnapshot`) |
...
- `todo/ui_bevy/bevy_panel_migration_guide.md` is the Priority 1 execution
  ledger for five migration packages, shared panel-style token groups,
  component-level contracts, package-level browser verification and
  product-owner approval, product-panel coverage, and final HTML-route
  retirement gates.
...
- `eVe/intuition/ribbon/bevy_ui_menu_surface.js` owns the shared Bevy footer geometry, common menu-tool content/node builders, plus the corner-resize grip and red-close constructors. Panel footer trees retain the shared grips. Atome-edit footers use the shared title/close surface without object resize grips.
- `eVe/intuition/shared/bevy_ui_tool_slider.js` owns the shared canvas-only
  ...
  tool invocation ownership. The code is retained for future tool-slider
  design review, not as a Panel Lab or generic panel component.
  `bevy_ui_pointer_runtime.js` exempts `tool_slider` and every node with an
  explicit `drag` handler from panel-scroll drag takeover, so its vertical
  gesture remains owned by the descendant interaction.
- `eVe/intuition/runtime/bevy_panel/bevy_panel_accordion.js` owns the validated
  ...
  `EVE_PANEL_SKIN_TOKENS.bevyPanel`.
- `atome/src/squirrel/components/select_contract.js` owns renderer-neutral
  ...
  a `select` header and generic `button` option rows. Product selection state belongs to each consuming surface. `bevy_ui_hit_test_runtime.js` traverses an
...
- `atome/src/squirrel/components/toggleable_contract.js` owns renderer-neutral
  ...
  `toggle` kinds around `buildBevyIconButtonSurfaceNode`, the same canonical
  30 px surface used by icon actions. The shared
...
- `bevy_ui_scroll_runtime.js` owns bounded, inertia-cancelling post-rebuild
  ...
  bridge; panel runtime invokes that bridge after a surface intent requests a
  reveal, never through a second scroll owner. `sourceTrees` retains the
  hydrated unscrolled input; every scroll frame derives the overlay and hit-test
  trees from that source without replacing it with a clipped viewport artifact.
...
- `eVe/intuition/tools/timeline.js` is now a compatibility wrapper only; the visible Timeline product-tool surface is `timelineSurface` in `bevy_panel_surfaces.js` and must not recreate `createEveDialog`, tool docks, or visible HTML controls. Timeline is the sixteenth and final product-panel migration; its existing Bevy route is not completion evidence and its controls must not be counted as generic Panel components.
- Calendar is lazy-registered by `eVe/intuition/tools/calendar.js`; `bevy_panel_calendar_runtime.js` owns its disposable editor mode, quick-create draft, and gesture state, while `bevy_panel_calendar_projection.js` owns time-slot snapping and multi-week month segments. Month/week/day double-clicks and draft/persisted drag-resize gestures remain BevyUI intentions; only validated saves and persisted-event gesture releases reach the existing Calendar service. The common registry carries no Calendar stub.
- `bevy_panel_tree.js` owns shared panel text, divider, composition-row, and passive-list-row builders above the opaque shell. `textNode` consumes canonical panel typography and `dividerNode` consumes canonical divider paint/margins. `listRowNode` configures the native BevyUI `row` primitive with existing panel control geometry, system surface, and localized text; it never adds an interaction handler, state owner, or DOM projection. `EVE_BUTTON_SKIN_TOKENS.bevyButton` is the single standard BevyUI button-style owner: common material/depth, radius, focus ring, pressed translation, disabled opacity, semantic tones, and icon-action geometry. `buildBevyIconButtonNode`, labeled panel action buttons, and standard/palette menu tools consume that contract. Mystic reuses the material but explicitly overrides the standard radius with its preference-controlled tile roundness; chips, segmented groups, close rings, and other composite controls retain only their required shape exceptions. Role-local rest/pressed/active shadow maps are removed. `bevy_ui_tree_normalization.js`, `bevy_ui_overlay_record_projection.js`, and `bevy_media_texture_resolver.js` carry the row label's normalized vertical alignment and +1 px offset to the text texture. The panel row places the localized label as a sibling on the right, never inside the button. `bevy_panel_surfaces.js` remains only the product-surface registration owner.
- `atome/src/squirrel/components/action_button_contract.js` owns the renderer-neutral labeled action-button presentation contract: localized label, `neutral`/`destructive` variant, and busy/disabled handler suppression. `bevy_panel_tree.js` consumes it through the shared native `button` builder, while `bevy_panel_lab_action_button_runtime.js` owns only the Lab's reset-on-close hover/focus/press snapshot and closed `panel_lab.action_button.*` intents. The validated state matrix has no DOM control, MCP call, durable state, or Atome mutation.
- `atome/src/squirrel/components/segmented_control_contract.js` owns the validated renderer-neutral segmented-control normalization: two or more unique options and one known selected value. `bevy_panel_segmented_control.js` composes it from the supported native `segmented_control` root and native button primitives because the active WASM vocabulary has no specialized segmented-widget renderer. `bevy_panel_lab_segmented_control_runtime.js` owns only reset-on-close `list`/`table`/`natural` presentation state and closed `panel_lab.segmented.*` intents; canonical foreground-scene records and hit-tests cover all three segments. It creates no DOM control, MCP call, durable state, or Atome mutation.
- `atome/src/squirrel/components/selectable_list_contract.js` owns the in-review renderer-neutral exclusive-list contract: two or more unique options and one required enabled selected value. `bevy_panel_selectable_list.js` composes the supported native `button` primitive because native `row` is non-interactive; it reuses Select presentation states and the shared check mark. `bevy_panel_lab_selectable_list_runtime.js` owns only reset-on-close `first`/`second` Lab presentation state and closed `panel_lab.selectable_list.*` intents. It creates no DOM control, MCP call, durable state, or Atome mutation.
- `atome/src/squirrel/components/multiline_input_contract.js` owns renderer-neutral newline-preserving local draft presentation. `bevy_panel_editable_text.js` supplies the shared editable tree, while `bevy_panel_multiline_input.js` composes the `358 × 80 px` panel field and `text_editing_layout.js` provides its opt-in wrapped WebGPU lines. `bevy_panel_lab_multiline_input_runtime.js` owns only close-reset draft, caret, and scroll presentation state and emits no MCP call, mutation, or DOM control.
...
- `bevy_panel_lab_text_input_runtime.js` owns only the Panel Lab field policy:
  ...
  close/reload. `bevy_panel_tree.js` hides the placeholder only while the empty
  field is focused. `text_editing_session.js`
  owns the shared submit/IME arbitration. `surface_runtime.js` and
  `surface_text_pointer_runtime.js` route a double-click on active project text
  to `text.selection.all`; `project_scene_text_runtime.js` delegates that
  selection to `text_bridge.js` without reopening or committing the session.
- `text_editing_session.js` also owns focused-editor document
  `selectionchange` synchronization, and it applies the shared touch-target rule
  of `hidden_text_service_runtime.js` (`setActiveTextEditorHitTest`): focus
  returns the target, a retained blur that does not reclaim the focus withdraws
  it, so a hidden editor kept mounted for scene text can no longer open the iOS
  keyboard on the first tool tap that reaches its band. Native selection-only movement, including
  the iOS keyboard trackpad, updates the disposable selection/caret snapshot
  and reuses the existing WebGPU text projection without creating input or a
  second editor.
- `tests/eve/bevy_panel_contract.test.mjs` guards the shared Bevy panel-shell contract: no tools dock or passive header, body is the single scroll area, and the footer owns title/close/drag/resize. Timeline's independent product-tool surface no longer builds an HTML dialog.
- `tests/eve/bevy_ui_layout_contract.test.mjs` owns the focused parity contract between BevyUI overlay layout and canvas hit testing for flex padding, gaps, native dividers, and control margins.
...
- `eVe/domains/dashboard/dashboard_preferences.js` owns only the `preferences.dashboard.weather_location` normalization; the Dashboard category filter of Home was removed on 2026-09-22.
...
- `eVe/domains/dashboard/dashboard_environment.js` owns dashboard environment reads and shared key normalization: current project resolution, registered BevyUI main-menu reserved height, handedness, compact environment snapshots/signatures, and the filtered-category item projection (the cascade that redistributes a filtered category from its own lane across the other lanes). It has no legacy toolbox DOM discovery or measurement fallback and owns no durable project state or DOM rendering.
...
- The visible main menu and Atome contextual-edit rail are BevyUI-only. `bevy_ui_menu_surface.js` owns their shared tool constructor and stable shadow/surface/shell/content layer roles: standard horizontal and vertical backgrounds share one plane, while moving palette children use the single lower emergence role instead of index-derived layers. The contextual rail is a transparent scroll layout and never owns a second material or shadow behind its tools. The main menu Atome item calls `toggleWorkspaceDashboardAndMainMenu({ source })`, while `tool.main.home` remains a normal `ui.home.panel` route. The former DOM ribbon/footer renderer and its private helpers are deleted.
...
- `project_scene_prefix_reconciliation.js` owns prefix-id collection and the pure desired/changed/removal plan with identical workspace normalization, while `project_scene_runtime.js` only applies that plan; direct-transform comparison is therefore reachable without growing the scene owner. `dashboard_layout.js` exposes visible `lanes` for interaction plus stable `projection_lanes` for every category; offscreen lanes stay outside the canvas until scrolled into view, and partially visible lanes retain full logical size while their clip changes. `dashboard_header_records.js` and `dashboard_card_records.js` keep the root viewport clip separate from the exact visible-cell clip applied to child icons/media/text, preserving both exterior shadows and inter-row isolation. `project_scene_direct_transform_runtime.js` accepts position, logical-size, and clip changes through the existing projection path.
- `eVe/intuition/tools/workspace_main_menu_visibility.js` owns the post-auth/workspace invariant that the internally registered BevyUI main menu is mounted, visible, and projected into the active or transition-target scene. `user_workspace_surface_runtime.js` opens the Dashboard above the resident project surface and closes it back to that same surface without loading the project again. None of these owners creates a browser menu alias, DOM dashboard, or alternate canvas.
- `eVe/intuition/tools/project_bootstrap.js` owns delayed post-auth project restoration and readiness. It resolves the authenticated user's persisted current project before another authorized project, creates one `untitled` project only when none exists, persists that selection, synchronizes the active-project flag, and resolves its exported readiness only after `loadProjectAtomes()` completes. Dashboard startup keeps this prepared project resident; it must not clear its host, move `eve_surface_project` back to `#view`, or delete `window.__currentProject`.
...
- `dashboard_header_records.js` owns the two-tier rail geometry: the upper band (icon + rubrique label, both bounded by the band height), the lower band (`eve.dashboard.header.new`), the `2 px` `header_separator_*` line and the full-frame `header_bg_*` glass, all derived from `dashboard_layout.js#dashboardHeaderBands`. `dashboard_bevy_ui_runtime.js` owns its single minute-aligned timer and lifecycle cleanup. `tests/eve/dashboard_header_zones.test.mjs` guards the band partition, the split-line hit zones, the separator and `Nouveau` records, the upper-band content bounds and the filter cascade; `tests/eve/dashboard_records.test.mjs` keeps its geometry guard but is red on pre-existing expectations (the Calendar time/date records and the removed label contract no longer exist in the records); `shape_shadow_overlay_tests.rs` guards repeated style rebuilds without live-overlay accumulation.
...
- Product UI, panel chrome, tool composition, Finder-facing workflows, Matrix, ribbon, mystic, closed voice surfaces, product persistence adapters, and Molecule/MTraX workflows belong to eVe.
...
- Intuition UI, panels, tools, menu, ribbon, mystic, matrix, and product interaction runtime.
...
- `eVe/intuition/tools/capture_recording_feedback_runtime.js` owns bounded Bevy tool feedback and post-stop reveal delegation for `capture.js`. It receives the invoking Bevy presentation (`mystic` or `main_menu`) and projects one session only to that surface. During a main-menu capture, `bevy_ui_main_menu_recording_visual_runtime.js` keeps the Capture palette open and `bevy_ui_main_menu_model.js` preserves the complete menu while replacing the active tool content with the recording preview. Mystic closes after starting capture; the existing recording owner retains the session and feedback. The main-menu recording runtime keeps the matching session/sequence, deduplicates unchanged 64-pair audio envelopes, and caps their projection at 30 Hz; `bevy_ui_main_menu_recording_visual_model.js` replaces the complete icon/label content during active audio/video phases and maps the scope through the fixed -60 dB to 0 dB display scale. These owners register live sources through their renderer owners but create no audio-scope canvas, frozen media clone, camera element, visible media node, or second renderer.
...
- `eVe/intuition/runtime/project_media_import_runtime.js` owns shared project media file selection for mystic-menu import, capture import, and the Home/Background wallpaper import, emits the narrow Xcode-visible import trace for iOS/AUv3 picker diagnosis, then delegates durable project media creation to `project_drop.importFilesToProjectViaCreator`. The picker order is fixed: native iOS/AUv3 document-picker bridge, then `showOpenFilePicker`, then one transient `<input type=file>` appended to `document.body` and removed on settle. `tools/background.js#openBackgroundImportDialog` calls requestProjectImportFiles({ accept: image/*, multiple: false }) and hands the file to `importBackgroundImageFile`, so no panel or dialog keeps its own hidden file input.
...
- `eVe/intuition/runtime/eve_intuition/basic_ui_tools_runtime.js` owns the lightweight bootstrap registrations for undo, Play latch, select, circle, text creation, matrix view, orientation, palette visibility, and item enable/disable. The post-presentation phase in `boot_runtime.js` replaces the provisional Play latch exactly once with `media_reader_tool_runtime.js`; no global loader is exposed. The former hidden `dummy` catalog registration was removed because it was diagnostic/demo surface, not product runtime.
...
- `eVe/intuition/ribbon/bevy_ui_product_runtime.js` routes both the Atome hold gesture and retained AI tool invocation to the same assistant runtime. The former DOM inline editor and direct AI action executor were removed.
- `eVe/intuition/runtime/eve_intuition/atome_contextual_rail_model_runtime.js` owns contextual-rail tool-key normalization, default tool sets by atome kind, persisted rail-tool read/write, project-playback tool availability, record-action mode/source normalization, and rail catalog tool definitions.
...
- `eVe/intuition/runtime/eve_intuition/atome_contextual_edit_model.js` also owns the rail's pinned zone: `buildAtomeContextualEditTree({ activeSlots })` removes the moved entries from the ordinary level (`withoutMovedTools`) and reserves `min(n × itemSize, availableRailHeight)` against `layout.bottom`, so the lit cases sit above the Atome handle, never scroll with the rail viewport, and push the ordinary stack up by exactly their height. The pinned case keeps the level node id `atome_contextual_tool_<safeId(key)>` (`…_icon`), hence the same handler, tool identity and pulse. `atome_contextual_edit_runtime.js` owns the activation order (`state.activeSlotOrder`) and the `litToolIconIds()` list synchronized on the unique `createRecordingIndicatorPulse`.
...
- `eVe/intuition/runtime/eve_intuition/atome_contextual_rail_definition_invocation_runtime.js` owns contextual-rail definition invocation for its runtime and explicit context calls, including selection-bound payload merge and slider dispatch.
...
- `eVe/intuition/tools/core/active_tool_registry.js` owns the residence of the tools that remain lit once started (`rail` | `menu`) and the invocation path used both to read and to stop them; `readActiveToolEntries({ keepToolId })` lists the lit menu-resident entries and `stopActiveToolEntries({ keepToolId })` stops them through the method each tool already publishes before releasing its latch. `tool_runtime_circle_tool_state.stopOperativeCreationTools` delegates to it; no other surface re-decides a lit tool's residence.
...
- Event layers such as click, double-click, selection, drag, resize, mystic menu routing, media transport, and MTRAX opening must resolve the nearest `eve-atome_*` host, recover the canonical Atome id, and then consult the Atome registry, runtime registry, or owning domain registry for behavior decisions.
...
- `eVe/domains/rendering/render_atom.js` owns the disposable RenderAtom content projection and must consume the shared media source canonicalization contract before media reaches Bevy, so bare uploaded or recorded filenames become explicit `/api/uploads/...` or `/api/recordings/...` sources with owner scope instead of browser-relative URLs. Canonical SVG Atomes stay editable `shape` records, while an SVG source/MIME or inline `svg_markup` projects as disposable `image` content for the shared Bevy SVG texture resolver; an ordinary labelled shape must remain `shape`. Inline canonical markup takes projection priority over a stale original source after vector edits. The record's canonical owner overrides a stale URL query owner only in this derived projection. Video RenderAtom content also normalizes the derived timeline fields `start`, `duration`, `trimIn`, `trimOut`, `offset`, `speed`, and `loop` from canonical Atome properties for renderer/decode projection; this remains disposable projection data and must not become a writable renderer state store.
...
- `eVe/domains/rendering/surface_size_runtime.js` owns render-surface size derivation from host CSS dimensions, `devicePixelRatio`, coherent `ResizeObserver` device-pixel box data, optional GPU texture clamping, and source-qualified window/visual/native viewport selection. `platforms/desktop-tauri/src/viewport_runtime.rs` publishes the host inner size in logical CSS units at navigation startup and every Tauri resize; it feeds this same size authority through `eve:native-viewport-resize` and creates no renderer or DOM layout owner. Device-pixel boxes that contradict the capped page DPR are ignored so the Bevy backing-store cannot diverge from logical scene coordinates.
...
- `eVe/domains/rendering/project_scene_spatial_index.js` owns the disposable uniform-grid lookup derived from `runtime.scene`; `project_scene_hit_testing.js` uses it to reduce canvas point and rectangle candidates before preserving the canonical hit-test/lasso predicates. `project_scene_incremental_update_runtime.js` applies a compatible single-record WebGPU diff for non-structural style, text, and resource changes, while project ordering, parentage, renderer readiness, background scenes, and queued frames still return to the canonical full projection.
...
- `eVe/domains/rendering/project_scene_runtime.js` owns active project scene orchestration, resize, drag, generic scene-intent commits, and synchronous disposable scene refresh when project-visible records are inserted before the async Bevy frame completes. It delegates live drag/resize frame coalescing to `eVe/domains/rendering/project_scene_gesture_runtime.js`, so move bursts update disposable scene records immediately while WebGPU rerenders and realtime `gesture_frame` commits are batched to animation frames. Text editing is delegated to `eVe/domains/rendering/project_scene_text_runtime.js`, active edit ownership is tracked outside the DOM by `eVe/domains/rendering/project_scene_text_edit_state.js`, and hidden keyboard capture is owned by `eVe/domains/rendering/hidden_text_service_runtime.js`. The hidden service keeps one transparent 1 px editor fixed at the visible origin with a 16 px native font to avoid iOS focus scrolling/zoom, removes `aria-hidden` only while that editor is focusable, and owns the transient pre-keyboard viewport lock. `renderProjectScene()` also publishes its caller in the native `scene.start.<atomes>.<proprietaire>` milestone (first frame outside this file, hash suffix stripped, 48 characters max), the only device-visible witness of a render loop. Direct Bevy transform patches are owned by `eVe/domains/rendering/project_scene_direct_transform_runtime.js`, which preserves canonical scale/rotation/origin during live pointer movement, direct opacity/style patches are owned by `eVe/domains/rendering/project_scene_direct_style_runtime.js`, and selection/video/audio invalidation listeners are owned by `eVe/domains/rendering/project_scene_invalidation_runtime.js`. `project_scene_text_runtime.js` consumes `InlineEditSession` for active text edit draft state, focus metadata, overlay anchor metadata, and deterministic `tx_id` propagation while keeping visible text on the Bevy canvas. Text commit and gesture end still persist `set` events through the canonical Atome commit boundary.
- `eVe/domains/rendering/surface_runtime.js` owns centralized project-surface pointer routing plus render-surface host attachment and logical size tracking. It observes the active host, `#view`, window, visual viewport, orientation and Tauri native-viewport events; work is frame-coalesced and ends with one source-neutral settled consensus so a late stale WebKit/native signal cannot leave Bevy at the preceding size. Only distinct stabilized sizes are published through `subscribeRenderSurfaceSize`; Dashboard, the main menu and structured views add no competing viewport listeners. `reconcileRenderSurfaceSize()` is the only explicit re-entry and still dispatches the canonical `surface.resize` scene intent when size changes. `surface_hit_target_runtime.js` owns pure client-to-logical coordinate conversion and hit acceptance; no visible per-Atome resize DOM or alternate renderer path is introduced. `SURFACE_RENDER_SIZE` keeps the size the engine was really told: `applySurfaceResize()` decides the `surface.resize` intent against it, not against the raw `SURFACE_SIZE` measurement that boot and the backing guard also write, so a silent measurement can no longer consume the intent and leave the canvas physically behind its CSS box.
...
- `eVe/core/atome_events/project_layer_runtime.js` owns project background and lasso gestures. `project_layer_tap_classifier.js` owns the pure 520 ms / 32 px background double-tap classification, allowing the bounded second press to outrank the smaller lasso activation threshold while an intentional drag beyond that bound remains a lasso. On the cleaned canvas route the runtime must ask `project_scene_runtime.js` for scene hit-tests before arming lasso, and lasso rectangle selection must use scene Atome ids when no per-Atome DOM hosts exist. `eVe/shared/lasso_context_zone_runtime.js` owns the disposable lasso action menu, opens it from a passive long press inside the runtime selection bounds, cancels that press on drag movement, and exposes the active lasso point blocker consumed by the mystic context runtime.
- `eVe/core/atome_events/project_layer_routing.js` arms the canonical provisional text editor during the accepted second background press and re-focuses that same editor from the consumed terminal click after WebKit's tap default action; it carries one owner through release and cancels it on pointer cancellation or lasso takeover. `tool_window_bridge_runtime.js` exposes only the closed `prepareProvisionalFocus` / `cancelProvisionalFocus` / `focusActiveTextEditor` bridge over `text_creation_session.js` and `hidden_text_service_runtime.js`; it is not a second editor or state owner.
...
- `eVe/intuition/runtime/tool_genesis_project_load_runtime.js` owns `tool_genesis.js` project Atome loading: local/remote/shared override selection, owner filtering, account-state filtering through `tool_genesis_account_record_filter.js`, in-flight de-duplication, recent-load cache completion, load/render perf events, and Bevy scene dispatch through `renderProjectScene()`. A `staleFirst` load awaits the canonical local snapshot itself: the 90 ms `ATOME_RENDER_FAST_PATH_BUDGET_MS` race belongs to the balanced route only, because losing that race used to downgrade a stale-first boot into a balanced load that awaited the authoritative read, which is how an unreachable server held the native launch surface. When that local snapshot is renderable, the load projects and resolves it without starting a remote request; the same owner schedules one forced authoritative refresh only after `eve:boot-presentation-ready`. That refresh carries `viewModePrepared: true`, so one boot transaction performs one preference prefetch/restore even though data receives a later authoritative reconciliation. Balanced and empty-local loads retain remote/shared override reconciliation. The runtime preserves only ephemeral records allowed by `project_scene_runtime.js`: Molecule overlays always, dashboard overlays only while the dashboard runtime is active for that same project. Closed dashboard records must never survive project load reconciliation, refresh, launch, or project switch through this path. It must not create per-Atome DOM hosts for active project rendering.
...
- `eVe/intuition/runtime/tool_genesis_group_runtime.js` owns only non-authoritative group preview composition for legacy non-project hosts. Canonical Molecule membership and mutations are owned by `tool_runtime_atome_mutation.js` through direct `parent_id` batch commits; Tool Genesis must not expose or reconstruct a membership, grouping, refresh, or fallback route.
...
- `eVe/intuition/runtime/group_visual_runtime.js` and `group_visual_preview_runtime.js` own only disposable non-project preview paint. They retain no membership data, member visibility state, group command, or synchronization fallback. Project-visible Molecules remain `RenderAtom` entries in `project_scene_runtime.js`; direct `parent_id` is the only membership authority.
...
- The renderer guard suite requires the same one-frame coalesced redraw path for standard glass, liquid glass and scenes without glass; delayed backdrop-specific retries and active-glass scanning are absent.
...
- `tests/strangler_v2/palette_ssot_guard.test.mjs` guards application examples from mutating product menu content through retired aliases; `tests/eve/bevy_ui_main_menu_contract.test.mjs` scans production JavaScript for browser menu/Mystic state regressions.
...
- `Squirrel.av.audio.unlockPlayback()` is the ephemeral browser user-gesture capability: `backend.kira.js` prewarms the Kira/WASM module at boot and initializes its existing output only when a real play action invokes this capability. It is not a second engine or an Atome mutation.
...
- ADOLE auth, session, projects, storage, sharing, activities, atomes, and runtime modules. `server/auth_identity.js` owns private phone credentials, protected legacy-principal aliases, and the transactional restart-safe active-reference migration; `server/auth_users.js` owns user-Atome creation around that identity boundary; `server/auth_crypto.js` owns CSPRNG UUID v4 opaque principal creation; `server/auth_otp.js` owns OTP generation, TTL storage, delivery policy, throttling, and short-lived connection-scoped enrollment proof. `server/auth_sessions.js` owns refresh-session revocation after credential changes. No caller may derive, expose, or project a principal from a phone.
...
- `eVe/intuition/tools/project_bootstrap.js` prepares the authenticated or anonymous current project for both startup views. It resolves only user-authorized persisted/current projects, may recover from a stale empty current project through the single renderable user-owned `project_id` in `state_current`, and never reads a project choice from the DOM. It creates a project only when no authorized project exists and keeps the bounded retry if authoritative relisting is delayed. Its readiness contract completes only after the selected project load has rendered; `boot_runtime.js` and authenticated workspace entry await that contract before opening the Dashboard above the resident shared project surface.
...
- `service.js` owns the public `createVoiceService` factory and wires capture/STT/TTS/orchestrator/processing/proactive APIs. `service_stt_runtime.js` owns session-safe STT preparation/start/stop/cancel composition, while `service_input_meter.js` and `voice_input_meter.js` own the ephemeral microphone RMS lifecycle and frame fan-out. Tauri STT prepares the configured Vosk model only on the first explicit listening action; neither voice-service construction nor vendored plugin setup may eagerly load it during WebView boot. `model_catalog.rs` and `model_download.rs` own selection and resumable preparation, while `desktop_audio.rs` converts every native input rate to exactly 16 kHz. The native runtime preserves ranked alternatives/confidences for product-hint selection, owns exactly one microphone stream, and drops that stream plus its processor on stop or timeout. It sends RMS directly from the open recognizer stream through `stt_api.js`; its quiet-speech calibration is a bounded dBFS envelope, while browser mode uses its permission-bound meter. `telemetry.js` owns the centralized, secret-redacting `[voice-trace]` records for microphone, STT, planner/provider, MCP execution, reply, and latency stages; `platforms/desktop-tauri/src/dev_logging.rs` forwards only this target at info level by default in debug builds, while production keeps it disabled unless the existing native logging gate is explicitly enabled. Ad-hoc console diagnostics are not a parallel logging path. `eVe/domains/user/profile_api.js` installs the existing product profile loader into the explicit `__eveLoadUserProfile` bridge consumed by the open provider client, and `eVe/user/background.js` prepares the existing device-scoped Home vault through that bridge before returning the profile. Neither path duplicates profile or secret ownership. `assistant_session_controller.js` owns the DOM-free assistant turn lifecycle and preserves a failed execution's exact error code after speaking its localized actionable reply. `tts_runtime.js` owns local ONNX synthesis playback through the existing Kira authority, while `local_tts_worker.js`, `french_phoneme_encoder.js`, `tts_pcm_analysis.js`, and `tts_frame_bus.js` own worker inference, deterministic French phoneme ids, real PCM analysis, and ephemeral frame publication. Browser speech synthesis may select only a voice compatible with the requested locale; otherwise it leaves voice selection to the system while preserving the requested language. `service_support.js`, `service_speech.js`, `service_providers.js`, `service_browser_stt.js`, `service_tauri_stt.js`, and `service_tts_runtime.js` retain their existing generic responsibilities.
- `assistant_session_controller.js` serializes local TTS and native STT. It waits for TTS completion plus a bounded 320 ms acoustic drain before opening exactly one `user_turn` capture, emits permanent bounded `voice.duplex.guard` and `voice.endpointing.decision` diagnostics, and releases the capture on every exit. The removed assistant `barge_in` VAD/capture path must not be restored without a native acoustic-echo-cancellation owner.
- `project_scene_collector.js` owns the compact canonical project/user/selection snapshot and a bounded inventory of 64 visible user Atomes. `project_scene_targeting.js` grounds relative moves by explicit id, explicit selection, exact type/text, normalized match, or one bounded fuzzy visible-label match and rejects absent, negated, or ambiguous targets before tool execution. `service_speech.js` reuses that inventory as French lexical hints, requires word-boundary hint matches, preserves ordering across native Vosk confidence scales, and applies only bounded command-context corrections.
...
- `tool_router.js`
- `ai_planner.js` owns provider selection, prompt construction, response normalization, quota reporting, and target grounding. `ai_planner_deterministic.js` resolves explicit pixel-relative movement to the existing `ui.move/move.relative` Runtime V2 plan before provider lookup; it never executes or mutates directly. `ai_planner_catalog.js` ranks tools against the utterance and forwards only bounded identifiers, descriptions, actions, and required-first input schemas; `ai_planner_diagnostics.js` produces bounded provider request/response summaries without raw nested catalog or response dumps. Scene, identity, memory, and history sections are each serialized once.
...
- `assistant_scene.js` mounts one undimmed assistant record through the existing project-scene overlay owner for the current foreground WebGPU scene, then patches procedural uniforms through the canonical direct-style route while the surface is stable and reprojections the full record only when its surface dimensions change. When foreground ownership changes, it mounts in the new scene and removes the assistant prefix from every stale occupied scene; teardown clears the prefix from all occupied scenes. Its panel-local order normalizes immediately above the Dashboard token range ending at 1420 and below panel/menu surfaces.
- `assistant_visual_contract.js` owns bounded sizing, stable cyan/rose colors, breathing/pulse limits, five deterministic vowel morphs, 45/90 ms morph interpolation targets, the listening-only B internal vein and C organic contour modulation driven by ephemeral microphone RMS, and independent glow/core/shell reveal curves across 0–140, 70–330, and 150–420 ms followed by the continuous 180/320 ms settle/disappear curves.
- Assistant gestures belong to the shared main-menu hold/pointer runtimes: 520 ms hold, explicit listening tap and inward level selection. The obsolete central overlay interaction and ejection classifiers were removed; generic procedural renderer uniforms remain shared rendering contracts.
- `atome/renderers/bevy-core/src/shadow_texture.rs` owns the cached-quality shadow profile for projected shape and selection shadows; `shape_shadow_overlay.rs` and `selection_overlay.rs` consume it. `workspace_backdrop.rs` owns the capture/presentation render layers and two minimal quarter-resolution GPU images: a single-mip camera target and one sampled mip pyramid. `workspace_blur.rs` copies the completed capture into level zero and invokes Bevy 0.19's hardware mip-generation pipeline in the capture view's post-process phase. This replaces the deleted two Gaussian cameras, two blur quads, ping-pong images and `workspace_blur.wgsl`. `backdrop_surface.rs` is the shared material owner for Mystic, menus, contextual rails, panels and scene effects; `procedural_sdf.rs` reuses the same pyramid for liquid and assistant surfaces. Every material retains its own logical blur radius and DPR-derived LOD plus sampling footprint. Both shaders use `frag_coord_to_uv(..., view.viewport)` and low-LOD mip-phase compensation, so neither a global radius nor `textureDimensions × downscale` can shift the sampled workspace. Resize atomically retargets the capture camera and every material to one new capture/pyramid generation before releasing the old images. Web mutations request one coalesced redraw and rely on this same-generation RenderApp ordering; the four-timer presentation prime, active-backdrop scan and fake 16-sprite scene blur are deleted. The audio and Bevy web build scripts continue to isolate their WASM outputs, and renderer-version modules prevent stale shader caches. No assistant canvas, visible DOM surface, private renderer, CPU readback, dim overlay or blur fallback exists.
...
- `platforms/ios/atome-auv3/atome.xcodeproj/project.pbxproj` owns the application and AUv3 Atome resource-copy phases. Both use the same filtered `rsync` command and exclude Rust `target` directories, `.git` metadata, and the native-linked `renderers/wgpu-web-external-texture` Rust source tree before copying, so build-only source never enters a signed iOS bundle. `scripts/XCode_testflight_generator` is the single release entry point: it discovers the app scheme, validates its app-extension version parity, validates an installed Apple Distribution identity, then lets the existing automatic-signing configuration resolve archive/export provisioning. It increments the final `MARKETING_VERSION` component and `CURRENT_PROJECT_VERSION` for the archive, then persists both only after the App Store Connect upload succeeds. It derives the Key ID from one externally stored `AuthKey_*.p8` file and reads the Issuer ID from an external local configuration file. It retains every archive/export under the ignored `temp/` directory and never manages TestFlight groups, testers, App Store submission, or Git operations.
...
- `eVe/domains/dashboard/dashboard_bevy_ui_runtime.js` owns Dashboard runtime state, projection orchestration, data-only warmup, cancellable frame scheduling, pointer/wheel routing and the visual active-header state. `dashboard_projection_lifecycle.js` owns record-opacity fade and close teardown. Header activation never redistributes items and has no dedicated transition controller. The neutral `__eve_dashboard_workspace__` projection remains the foreground owner of the shared canvas, while `dashboard_data_controller.js` retains cache, invalidation and asynchronous hydration ownership. Closing cancels pending work, removes every `__eve_dashboard_` record and leaves no Dashboard layout or tree mounted. The runtime must not mount Dashboard DOM, allocate a second visible canvas or start a parallel renderer.
...
- `eVe/domains/dashboard/dashboard_workspace_mode.js` owns `resolveWorkspaceDataProjectId()`, the shared Calendar/Dashboard boundary that rejects the neutral renderer scene as business scope. `dashboard_bevy_ui_runtime.js` loads only the existing account-global critical categories (`projects`, `contacts`) while that resolver has no business project, then includes project-scoped categories after the existing project-bootstrap retarget. `dashboard_data_controller.js` retains the sole Calendar subscription but does not retain Calendar lists in its project cache; `user_workspace_surface_runtime.js` rebinds the visible Dashboard when bootstrap publishes the canonical project.
...
- Media source and identifier helpers own canonical `/api/uploads`, `/api/recordings`, and extract-audio route resolution for product media playback paths. Canonical `file_path` storage wins over timestamped filename heuristics and stale media URLs, so a `Downloads/audio_*` import cannot be projected as a recording. An explicit canonical record owner replaces stale `media_user_id` query ownership. Tauri media decoding uses its injected local Axum base, while browser media remains route-relative for its active server.
...
- `eVe/domains/rendering/project_preview_runtime.js` owns shared renderer-produced project preview capture for Matrix and Dashboard consumers. It filters Dashboard, wallpaper, surface-background, and background records before calling the existing Matrix/WebGPU preview renderer, sends no workspace background descriptor, invalidates the derived cache for every forced capture, reads the active project surface as the canonical source viewport, keeps `640x400` only as the maximum logical output box, prefers current scene records, and merges loader records only to hydrate projects whose scene has not yet published source records. Consumers must use its persisted preview descriptor instead of cloning Matrix DOM or capturing DOM screenshots.
- `eVe/domains/rendering/bevy_project_preview_capture_adapter.js` and `bevy_project_preview_capture_frame.js` own the Dashboard/Matrix project miniature capture surface: one retained hidden `/eve_preview_capture.html` iframe, one reusable Bevy/WebGPU canvas fitted homothetically from the project viewport into the requested target, an explicit transparent preview surface published through `bevy_surface_background_runtime.js`, explicit activation warmup through `warmProjectPreviewCapture()`, cold frame-runtime readiness bounded by an explicit timeout, and WebP export returned as disposable preview data with dimensions. The iframe carries the minimal browser import map for `#squirrel/`, `#shared/`, and `#utils/` before its capture module is injected, so shared rendering imports resolve in the isolated document. Warmup never gates workspace activation; failures enter `reportRuntimeError` as `eve:project-preview:warmup` with project and capture context. Content bounds never define the output ratio, wallpaper/default surface pixels stay absent, nearly empty output is rejected, and stale deferred stable-id resources are discarded before WASM application. The capture surface names itself `preview-capture-webgpu-render-surface`, so the native `bevy.surface.<zone>...` milestone says which owner started this second renderer, and every capture phase publishes `bevy.preview.<stage>` (`start`, `renderer_ready`, `redraw_ready`, `encode.<n>`, `visible.<n>`, `done`, `empty`) so a device freeze names its phase instead of stopping on `bevy.run_ready`. The WebGPU canvas is never read back on the UI thread: `snapshotCaptureCanvas` snapshots it through `createImageBitmap` (off-thread browser readback), draws the bitmap into one reused 2D snapshot canvas, and encodes there; an unavailable snapshot fails loudly (`bevy_project_preview_snapshot_unsupported`) instead of falling back to the synchronous `toDataURL` on the WebGPU surface, which is the only blocking GPU round trip of the rendering domain. This is a capture path, not a live project embedded in Dashboard blocks and not a fallback renderer.
...
- `eVe/domains/media/api/video_api_persist.js` treats a WebSocket/native upload as durable only after an explicit successful completion acknowledgement whose returned filename and canonical `recordings/...` path match the requested target. It returns that acknowledged path to the shared persistence boundary; it must never convert a missing, mismatched, or negative acknowledgement into a recording URL or an Atome commit.
...
- `eVe/domains/media/selected_project_media_playback_runtime.js` owns selected active-project media transport for the cleaned canvas route. It resolves selected media records from the canonical `PROJECT_SCENES` registry in `project_scene_state.js`, preserving the acyclic boundary between media playback and project-view playback, then preserves the canonical record owner while normalizing media routes, unlocks the existing `Squirrel.av.audio`/Kira facade at the start of a real browser play action, and sends audio and extracted video-audio through that same facade. Declared native/Tauri Kira runtimes skip that browser-only capability and retain their native lifecycle. Each fresh play receives a new ephemeral Kira voice id while pause/resume retains that session voice and its offset, preventing a completed native voice from being reused on replay. It releases runtime-only playback state at the persisted, waveform-derived, or Kira-load canonical duration after natural completion so project-level sequential playback can advance. It requires selected project video playback to use the project timeline automation path instead of mutating project-scene records directly. Extracted video-audio must consume the per-Atome project timeline playhead returned by the project playback runtime so Bevy video decode and audio resume from the same position. It must not create visible per-Atome media DOM hosts, a second media engine, a deleted MTraX forced-transport branch, a selected-group `eveMtrackApi.loadGroupTimeline` route, a selected-group DOM video/audio sequence transport, or a selected-Atome DOM `video/audio` transport in `eVeIntuition.js`.
- `eVe/intuition/runtime/eve_intuition/media_reader_tool_runtime.js` owns eVeIntuition registration and orchestration for `ui.media.reader`, `ui.animation.reader`, `ui.play`, `ui.pause`, and `ui.stop`. `boot_runtime.js` imports it once after the shared eVe audio/Kira deferred modules and before Mystic/contextual owners, then injects the existing selection and latch dependencies. It delegates selected media transport to `selected_project_media_playback_runtime.js`, delegates project automation lookup to `project_scene_runtime.js`, and is the only non-decode runtime allowed to call `setBevyVideoDecodePlayback(...)`.
...
- `capture://...` is an internal capture-session identifier, never a project media source or BevyUI URL. Mystic and main-menu recording previews both consume only the recorder-pushed RGBA texture, rendered as the existing texture-backed image record; the persisted recording file resumes the normal media route only after durable stop success.
...
Purpose: Product UI shell, tools, panels, matrix, menu, ribbon, mystic menu, projection, runtime, contracts, and shared closed UI helpers.
...
- `eVe/intuition/runtime/eve_intuition/mystic_context_items_runtime.js` owns Mystic/context item resolution, mixed-selection routing, transport-bound extra input construction, Mystic import/delete dispatch, and the shared context-bound transport tool id set used by footer invocations. `eVeIntuition.js` must not re-inline Mystic item builders or duplicate the transport-selection payload policy.
- `eVe/intuition/runtime/eve_intuition/atome_contextual_rail_selection_runtime.js` owns contextual-rail selection normalization, selection publication, and tool `extraInput` payload construction. Rail row/child/definition runtimes consume these injected helpers instead of rebuilding selection payloads inline.
- `eVe/intuition/runtime/eve_intuition/atome_contextual_rail_runtime.js` is the composition facade: it reuses canonical tool definitions/invocation and delegates the visible project rail to `atome_contextual_edit_runtime.js`. The deleted DOM lifecycle/fullscreen/footer modules are not alternate renderers.
...
- `eVe/intuition/runtime/eve_intuition/context_tool_invocation_runtime.js` owns unified context tool invocation for footer, Mystic, and the main ribbon. It delegates only to the canonical `invokeToolFromUiButton` and the injected Finder handler; it must not create another tool bus.
- `eVe/intuition/runtime/eve_intuition/boot_runtime.js` owns eVeIntuition boot-time menu exposure, auth sync, panel surface registration, Mystic/contextual-edit/vector/draw installation, post-presentation non-critical warmup, UI tool registration, the closed contextual-edit module registry, debug runtime installation, and initial workspace reveal. Its deferred interaction phase first calls the exported `eVe/eVe.js` loader, installs the canonical Media Reader, and calls the exported `intuition/bootstrap.js` interaction-tool loader before Mystic; every import is once-guarded, replaces provisional handlers, and exposes no global loader. `eVeIntuition.js` creates the BevyUI main-menu bridge closed (`open: false`) so disconnected boot cannot paint toolbox buttons before the login accueil. A mounted login choice publishes only authentication readiness. Authenticated boot resolves the canonical project first: a valid saved project is revealed directly, while first use or invalid state opens Dashboard. Each entry route publishes `eve:boot-presentation-ready` and the versioned native bridge message on the presentation evidence it really proves: the project route on WebGPU presentation with Main Toolbar `active/treeMounted`, the Dashboard route on `readWorkspacePresentationEvidence()` (the projected Dashboard tree it already proved for `dashboard.presentation_ready`), because that entry deliberately unmounts the main menu. The watch re-arms until that evidence is satisfied instead of expiring on a bounded window, and it publishes nothing extra to the native watchdog, so a workspace that never presents is still reported as stalled. A transient unavailable surface, mounted tree, or interactive main-menu record still retries; the terminal state stays exposed through `window.__eveWorkspaceBootOpenError`, and a terminal renderer failure is also painted by the closed boot failure surface through `settleTerminalWorkspaceBoot` in `eVe/intuition/runtime/eve_intuition/boot_failure_surface.js`. The boot-open promise is released after success and guarded by a completed flag so late project-publication events cannot trigger duplicate opens. It must not create a second dashboard/menu path. `eVeIntuition.js` must not re-inline boot/warmup/register code.
...
- `eVe/intuition/eVeIntuition.js` is a sub-500-line closed boot coordinator. It delegates BevyUI main-menu and Mystic creation to `eVe/intuition/ribbon/bevy_ui_product_runtime.js`; future changes extend focused owners instead of re-inlining panel actions, text state, menu composition, Atome contextual-edit lifecycle, or boot registration.
- `eVe/intuition/runtime/bevy_panel/` owns the shared BevyUI panel shell. `bevy_panel_runtime.js` owns mounted panel lifecycle plus ephemeral drag/resize geometry; `bevy_panel_layout.js` bottom-anchors every opening geometry to the top of the canonical main-menu reserved band on desktop and mobile, keeps the product-panel mobile fullscreen default, and honors the Panel Lab's explicit development-only floating-mobile capability so drag and resize remain testable on iPhone; `bevy_panel_tree.js` owns its tree shape; `bevy_panel_tokens.js` derives all panel visuals from `EVE_PANEL_SKIN_TOKENS.bevyPanel`. The temporary development/test-only `panel_lab` surface validates this shell from a trailing main-ribbon tool and must be removed with its dedicated registration/tests after component migration completes.
- `eVe/intuition/runtime/bevy_panel/bevy_panel_surfaces.js` statically declares
  only the lightweight common Timeline surface. `tools/user.js`, `tools/calendar.js`,
  `tools/contact.js`, and the internal Panel Lab loader import and register
  their surface on demand. `bevy_panel_runtime.js` owns real
  `eve:surface-state` publication, all-open-surface structural resize, mobile
  geometry projection/restoration, complete unmount cleanup, and fast
  drag/resize routing. `bevy_panel_scroll_runtime.js` owns the frame-coalesced
  latest-wins scroll patch. `bevy_ui_runtime.js` owns targeted source/hit-tree
  translation and text/caret patches without content reconstruction.
- `bevy_panel_media_card.js` owns `identityMediaFrameNode`, the shared
  click/drop identity-photo target used by `bevy_panel_home_view.js` and
  `bevy_panel_contact_view.js`. Home and Contact runtimes own only their
  respective file validation and canonical persistence calls.
- `bevy_panel_home_view.js` lazily composes only open accordion content and
  open Select options. `bevy_panel_home_runtime.js` owns disposable Home draft
  state and section lifecycle; `bevy_panel_home_actions.js` dynamically loads
  profile, Dashboard, Background, Server, and tool owners at the action or
  section boundary. Vault and media-picker modules are likewise loaded only
  when the corresponding authorized action occurs.
...
- `eVe/intuition/tools/core/svg_draw_runtime.js` owns Draw activation and canvas/Visual pointer sessions, resolves existing canonical scene shapes without DOM hosts, delays new Atome creation until the first real stroke, and installs `window.__eveDrawTool`. `svg_draw_commit_runtime.js` owns draw gesture ids, canonical commit payloads, and project patch derivation; `svg_draw_model.js` owns SVG geometry, threshold and markup descriptors. The former `svg_draw_dom_runtime.js` host route is deleted.
- `eVe/domains/rendering/project_view_creation_runtime.js` owns Page latch/gesture creation, immediate structured Text/Draw/Page preparation, Visual subject priority, and shared container navigation. `project_view_create_draft_runtime.js` owns only canonical draft ids plus empty/valid/gesture lifecycle and finalization through commit or BlackHole; it owns no record copy. `project_view_creation_geometry.js` is the pure Page-frame contract. `project_view_visual_interaction_runtime.js` translates optional inline Visual interactions into the existing natural scene intentions and marks edit entry `rail_only`; `project_view_visual_geometry.js` owns preview-to-scene coordinate mapping. `project_view_surface_layout.js` owns structured surface sizing/layout while `project_view_surface_runtime.js` retains the shared navigation stack when Natural is shown.
- Structured Molecule playback projects the complete canonical member composition through `bevy_panel_record_composite_preview.js`, which fits member bounds into Visual while preserving positions and z order and delegates every child to `bevy_panel_record_preview.js`. `project_view_visual_subject.js` resolves the owner plus fresh member records from List/Matrix content; `project_view_playback_delegated_runtime.js` owns simultaneous transport publication and projected-video start/natural-stop cleanup without creating a second media engine.
...
- Native BevyUI viewport diagnostics are resolved by `atome/renderers/bevy-core/src/ui/mod.rs::ui_viewport_size()` from the single `IsDefaultUiCamera`. The quarter-resolution backdrop capture camera is deliberately excluded. `platforms/web/bevy-renderer/src/lib.rs` publishes that same value to the web diagnostics consumed by `bevy_ui_render_scale.js`; it must never use unfiltered ECS camera iteration, because doing so applies the capture downscale to the whole Dashboard and main menu.
- BevyUI overflow responsibilities are split into stable focused owners:
  ...
  `bevy_ui_scroll_runtime.js` owns ephemeral offsets, wheel/drag takeover,
...
- `eVe/intuition/mystic/context.js` owns Mystic gesture entry, `mystic_layout.js` tile geometry, `mystic_menu_items.js` slot and pagination normalization, and `context_target.js` context resolution plus BevyUI point blocking. That blocker resolves the canonical project canvas by point when an iOS Pointer Event targets the project shell instead of the canvas, so a panel hit never arms Mystic long press. `context_selection.js` owns selection compatibility; `context_pointer_lock.js` owns module-local pointer/hold state with no DOM renderer or browser global.
...
- `eVe/intuition/tools/user.js` is the thin Home facade and exposes only `window.open_home_panel` / `window.close_home_panel` / toggle. `user_home_panel_runtime.js` is the session-aware window owner: it keeps unauthenticated login/registration in the existing `user_login_*` application shell, opens the registered Bevy `home` surface for authenticated or Guest workspaces, clears the Home latch after session transitions, and returns logout to the login shell. A successful canonical auth bootstrap ends the login wait immediately and starts Dashboard/project readiness asynchronously; `eve:workspace-open-failed` and sanitized login-stage diagnostics report workspace failures without converting the installed session into an authentication loop. `panel_definitions.js` carries the canonical `home` module key into the Runtime V2 window-owner configuration, and `tool_runtime_bootstrap_panel_handlers.js` resolves that existing lazy module before calling `open_home_panel`; direct authenticated/Guest workspace boot therefore cannot leave the Home owner unavailable on first click. `bevy_panel_home_runtime.js` owns disposable Home orchestration; `bevy_panel_home_view.js` composes six shared first-level accordions and the fixed session action; `bevy_panel_home_editing.js` reuses the single hidden text service, including transient secure password entry; `bevy_panel_home_vault.js` and `bevy_panel_home_vault_runtime.js` own the encrypted credential/Mail/provider-key bridge; `bevy_panel_home_actions.js` binds existing profile, Dashboard, Mail, Background, Server and Adole auth/security owners. Canonical profile state remains in `eVe/domains/user/profile_api.js` / `profile_api_support.js` and commits through `Atome.commit`; model-only visual/accessibility normalization remains in its existing owners. The deleted `eve_user_dialog` and panel-only `user_*` DOM composition/runtime files must not be reintroduced. Login/workspace modules remain application-shell/session owners, not a Home fallback.
...
- `EVE_COMMON_SKIN_TOKENS.bevy.systemSurface` is the one immutable BevyUI system material: background, backdrop and exterior drop shadow. Its black backdrop tint alpha `0.84` bounds a white underlying pixel near `112/255` after the GPU linear-to-sRGB conversion, so live bright media cannot erase white icons while remaining visible through the glass. `EVE_COMMON_SKIN_TOKENS.systemContent` is the single GPU content tuple for system labels, icons, grips and footer controls; CSS/SVG consumers derive their representation from it. `EVE_TOOL_SKIN_TOKENS.bevyMenu` owns only tool geometry, contextual outlines and semantic palette-accent geometry. `bevy_ui_menu_surface.js` is its sole BevyUI adapter and complete standard-tool constructor; main menu, Mystic, contextual editing chrome and panel shells reference the common material while their controls remain transparent layout/content. All trees decorate as presentation content so no menu pixel can feed the shared backdrop capture.
- Size factorization: `eVe/elements/system_ui_tokens.js` owns the one framework base unit (a square tool is `60 px`) and publishes `unitPx`, `halfUnitPx`, `doubleUnitPx`, `scalePx(referencePx)` and `eveCssPx(referencePx)`. Every drawn length of the UI lane is authored through that seam, so one base change resizes geometry, glyph boxes, text sizes, paddings, radii and shadows together: List rows and list columns are 1x (`60`), panel/accordion/contact rows, inputs, selects, segmented controls, numeric fields, scope chips, map result rows, calendar agenda rows and buttons 1/2 (`30`), Dashboard cells and Matrix tiles 2x (`120`). Gaps (`4-12 px`), the calendar time scale, non-row content blocks, gesture tolerances, viewport breakpoints, durations, map zoom levels, opacity/angle ratios and the single `999 px` pill sentinel are declared exceptions that keep their own proportions and still scale. `look/tool_theme.js`, `intuition/ribbon/tokens.js` and `atome/src/squirrel/apis/essentials.js` read that owner instead of their former `50 / 57 / 54 px` copies. Guards: `tests/eve/system_ui_base_unit_contract.test.mjs`, `tests/eve/system_ui_base_resize_contract.test.mjs`, `tests/eve/system_ui_base_wiring_guard.test.mjs`.
- Style factorization follows the existing ownership layers rather than adding a new theme module: `system_ui_tokens.js` owns the shared CSS backdrop and input text/focus depth values; `skin/tokens.js` derives its CSS/GPU blur projection from them; `panel_skin.js` owns one immutable panel-control state palette reused by actions, tables, Select, cards, summaries and inputs; `ribbon/tokens.js` projects the shared backdrop into legacy toolbox runtimes. `look/preset_{controls,chrome,comm_surface,comm_table}.js` compose repeated declarations from file-local definitions into independent preset objects so compatibility callers retain isolated mutable style records. `intuition/tools/visual/tool_visual_tokens.js` uses one selector-list builder for editor/footer tool variants and emits the shared tools declaration once. `tests/eve/bevy_panel_contract.test.mjs` guards these aliases and the explicit circular Mystic exception.
...
- Canonical sync delivery through `wsSyncRuntime.js`; per-principal cloud coffre
  ...
  Vault workers listen to their inherited IPC-channel disconnect and shut down
  even after an abrupt Fastify parent death; the provider bounds graceful stop
  and force-terminates an unresponsive owned worker after the timeout.
...
- `platforms/desktop-tauri/src/server/static_asset_cache.rs` owns the local
  Axum cache policy for bundled runtime assets. It applies conditional
  revalidation (`public, max-age=0`) to the root and eVe/Atome/source/vendor
  HTML, JavaScript and CSS routes while explicitly excluding `/api/`, `/file/`,
  `/text/`, and `/ws/`. `server/mod.rs` composes this middleware and must not
  duplicate its route classification or restore global `no-store` headers.
- `platforms/desktop-tauri/tauri.conf.json` owns the iOS permission plist merge and the resource set limited to `atome/src` plus `eVe`; `platforms/desktop-tauri/tauri.android.conf.json` owns the Android overlay. `platforms/desktop-tauri/gen/apple/` and `gen/android/` are the generated native projects. The Android floor is API 29: `platforms/desktop-tauri/gen/android/app/build.gradle.kts` and `tauri.android.conf.json` must move together, because the NDK sysroot only ships `libamidi.so` (linked by `midir`'s Android backend) from API 29 onward, above the API 26 AAudio floor.
...
- `platforms/ios/atome-auv3/application/AppNativeAudioController.swift`, `AppNativeAudioRecording.swift`, and `AppNativeAudioRecordingScope.swift` own standalone iOS microphone capture over that shared recorder backend. The audio tap only forwards preallocated Float32 PCM, while the main-thread monitor publishes the shared 64-bin min/max plus RMS/peak scope contract through the existing native audio event bridge. After the recorder has closed a valid WAV, `AppNativeAudioRecording.swift` reads it in bounded Float32 chunks to return up to 256 persisted waveform peaks with the stop result; this finalization work never runs in the microphone tap or alters AUv3 capture. Playback and command routing remain split into `AppNativeAudioPlayback.swift` and `AppNativeAudioCommands.swift`; `ViewController.swift` only owns the WebView shell.
...
- `platforms/ios/atome-auv3/Common/WebViewManager.swift` remains the shared manager/composition owner; `WebViewManagerScriptMessages.swift`, `WebViewManagerAudioTransport.swift`, `WebViewManagerNavigation.swift`, and `WebViewManagerIPC.swift` own script dispatch, audio/transport messages, navigation/media-permission delegates, and bounded IPC respectively. `FeatureFlags.panelLabEnabled`, `webInspectorEnabled`, and `textTraceEnabled` are false for normal Xcode launches and require their explicit Debug argument/environment opt-in; Release injects none of these diagnostics. Viewport sizing remains owned by the shared eVe surface lifecycle: the native bootstrap must not write document/body height or schedule delayed rotation corrections. This responsibility split preserves the same native invoke bridge and does not create another capture, transport, or viewport path.
...
- `scripts/commit_and_push.sh` is the user-operated, interactive Git publication helper for the framework root and nested repositories such as `eVe`. It requires an explicit repository, commit message, and literal path list; refuses detached HEAD, merge conflicts, pre-existing staged changes, and targets outside the framework; displays the selected status and remote; requires an exact branch confirmation; and never force-pushes or mutates tags. Coding agents may validate its non-mutating help and syntax paths but remain subject to the repository Git read-only policy and must not execute its publication path.
...
- `scripts/android/apk.sh` owns the Android build lane, dispatched by `./run.sh apk` through the `apk` case of `scripts/setup/service_commands.sh`. It is the single owner of JDK/SDK/NDK/Rust-target resolution (idempotent: present and current means untouched, missing or older means installed, failure means a named error with no substitution), of `tauri android init`, of the staged `platforms/desktop-tauri/gen/android-webroot` frontendDist, of release-keystore creation under `~/.atome/android/`, and of every `tauri android build`/`dev` invocation. `--doctor` reports the chain without building, `--dry-run` prints the exact plan, and `--isolated-home <dir>` keeps the cargo, Gradle and Android user caches under `<dir>` instead of `$HOME` for hermetic and CI builds. Artifact discovery reads the CLI's own `Finished N APK at:` report, because AGP nests outputs under their Gradle flavor (`outputs/apk/<flavor>/debug/`); the filesystem fallbacks use files newer than `temp/apk-build-marker` and the build log kept in the git-ignored `temp/apk-build.log`, and `--install` installs the verified list instead of re-searching for a file. The build log captures both streams, because that report is written to stderr while Gradle diagnostics go to stdout. `remove_stale_artifacts()` drops this build type's previous APK or AAB before the build, because AGP packs into an existing file without truncating it and a repackaged debug APK otherwise carried 489 765 844 dead bytes (994 683 526 bytes instead of 504 916 716). `platforms/desktop-tauri/tauri.android.conf.json` is the Android configuration overlay; `platforms/desktop-tauri/gen/android` is the Gradle project, kept in the repository so signing and manifest customizations survive; `platforms/desktop-tauri/src/android_assets.rs` owns the runtime materialization of the embedded web root. No other script may call `tauri android`.
...
- `scripts/server_update.js` owns reproducible production update orchestration, including dependency reinstall, service restart, and Nginx reload. It logs each phase (`env`, `backup`, `systemd-unit`, `git-update`, `eve-source`, `verify-source`, `npm-ci`, `restart`, `status`, `health`, `nginx-reload`) with command, cwd, duration, and exit status, delegates production secrets and signing identity to `scripts/server_secure_config.js`, initializes and updates the required `eVe` submodule, and runs deployed-source verification immediately after Git/eVe update and before `npm ci`.
- `scripts/server_secure_config.js` owns production JWT/cookie secret preservation, persistent signing-key adoption/generation, stable server identity metadata, fail-closed pair validation, permissions, and pre-update identity backup. Generated keys live outside the Git checkout under `/etc/squirrel/identity`; a code update must never rotate them.
- `scripts/verify_deployed_source.js` owns deployed-source verification markers for production routing, `/eVe/` static serving, required `eVe/eVe.js` and `eVe/version.txt` assets, and lockfile freshness, including the npm 10 top-level `@emnapi/core` and `@emnapi/runtime` peer entries required before production `npm ci`, plus the production secure-config and server-identity provisioning markers.
...
## Mystic BevyUI Ownership

- `eVe/intuition/ribbon/bevy_ui_mystic_model.js` owns the disposable Mystic BevyUI tree shape, radial item placement, and the tokenized shadow/backdrop visual node contract. `atome/renderers/bevy-core/src/backdrop_surface.rs` owns its reusable WebGPU material, rounded SDF clipping, and shared workspace-capture consumption; it is not a DOM or second-canvas renderer.
- `eVe/intuition/ribbon/bevy_ui_mystic_runtime.js` owns the captured pointer session, hit testing, held navigation, preview state, release activation (tool or palette under the finger, never the centre, gated by `MYSTIC_POST_REPLACEMENT_MOVE_PX` after a geometry replacement), the stationary auto-open dwell (`MYSTIC_AUTO_OPEN_DWELL_MS`, sources `bevy_mystic_dwell` / `bevy_mystic_release` / `bevy_mystic_fling`), `activateDirectional` for the cardinal root fling, the four-phase lifecycle, one cancellable animation-frame scheduler, frame-timing diagnostics, and cancellation cleanup. `eVe/intuition/ribbon/bevy_ui_mystic_motion.js` owns deterministic staggered transforms, elastic scale/settle, curved trajectories, reduced-motion sampling, interruption continuity, content reveal, and the temporary liquid procedural-SDF uniform payload. `eVe/domains/rendering/bevy_ui_tree_motion_runtime.js` carries transform/style patches through the existing BevyUI projection; it does not create a renderer.
- `atome/renderers/bevy-core/src/procedural_sdf.rs` and `assets/shaders/procedural_sdf.wgsl` consume the Mystic liquid payload in the shared procedural material. The Mystic branch uses smooth-union circles plus proximity-bounded capsules, samples the compositor blur only inside that temporary silhouette, and becomes empty after the fusion window; the assistant branch remains independent.
- `eVe/intuition/mystic/context.js` remains the gesture/context entry facade and does not create visible Mystic DOM. It owns the radial engagement (`RADIAL_SELECT_MIN_PX` 48 px), forwards the held and the released hover with `dwell: true`, and resolves the cardinal fling (`MYSTIC_FLING_MIN_PX` 250 px, `MYSTIC_FLING_MAX_MS` 500 ms from the moment the hold takes the pointer over, `MYSTIC_FLING_AXIS_TOLERANCE_DEG` 35°) which consumes the terminal event.
- The deleted `eVe/intuition/mystic/menu.js` was a retired DOM renderer and must not be reintroduced.
...
`eVe/domains/dashboard/dashboard_actions.js` owns the closed item-routing table: Projects delegate unchanged to `project_data.js#activateProjectWorkspace`, Calendar delegates by event id to its existing panel, Contacts delegate by stable identity to the existing Contact panel, and unsupported categories return inert without projecting a generic detail surface. `dashboard_item_mystic_menu.js` owns Dashboard Mystic target resolution and the existing project-only delete/duplicate/copy/paste actions. Neither owner may create a second state, panel, renderer, or persistence path.
...
- The shared `accordionNode`, `toggleableRowNode`, text inputs, action buttons,
  ...
  remains normal panel scrolling. The rail and the custom-field `+` reuse the
  canonical 30 px icon-button surface; custom fields precede that `add.svg`
  action. An avatar is projected only when `user_face` exists.
...
# Home panel migration update — 2026-08-02
...
- `bevy_panel_home_runtime.js` owns only disposable loading, accordion, select,
  notice, busy, editable blank rows, and security-form state.
  `bevy_panel_home_view.js` projects the
  six product accordions and fixed session action from shared builders;
  `bevy_panel_home_editing.js` owns the one hidden text entry session;
  `bevy_panel_home_actions.js` normalizes data and delegates every effect to an
  existing profile, preference, Dashboard, Mail, Background, Squirrel vault/AI,
  tool-gateway, or Adole owner.
- `panel_definitions.js` maps `home` to `eve_bevy_panel_home` and marks the
  session-aware window runtime as owner. `user_home_panel_runtime.js` alone
  decides whether to open the existing unauthenticated application shell or the
  Bevy Home surface; successful auth/Guest transitions clear the Home latch.
- `profile_api.js` / `profile_api_support.js` remain the canonical durable
  profile owner and sanitize before `Atome.commit`. Mail runtime persistence
  removes passwords; account passwords never enter Home profile state and are
  transient only in the hidden secure editor. Stable authenticated ids, never
  names, phones, emails, labels, or DOM state, authorize profile/security work.
- The deleted `eve_user_dialog` and 22 panel-only `user_*` runtime modules are
...
- `atome/src/squirrel/ai/provider_client.js` derives user/provider vault ids and
  asynchronously resolves provider credentials from the encrypted security
...
- `eVe/intuition/tools/calendar_api.js` is the sole writable Calendar owner and `atome/src/squirrel/calendar/` is its multi-source facade. `calendar_model.js` owns normalization, todo particles, recurrence, and ICS without importing a runtime store.
- `calendar_model.js` carries Calendar Atome type as top-level commit-envelope metadata; `eVe/core/atome_commit_fetch.js` sanitizes normal particles before inserting that trusted value into reserved event metadata. `database/adole.js#listStateCurrent` restores `atomes.atome_type` for list projection. The built-in Calendar source converts optional ensure exceptions to source results, and the panel runtime excludes the neutral Dashboard scene id from data-project selection.
...
- Project rows reuse `bevy_panel_choice.js` checkboxes and the hierarchical
  list's optional BevyUI `drag_handle`. `bevy_panel_info_runtime.js` owns only
  disposable gesture/selection state and routes accepted releases to existing
  `ui.duplicate`; `tool_runtime_atome_mutation.js` owns UUID allocation,
  typed clone preflight, relationship remapping, relative placement, and the
  single canonical `commitBatch`. The lightweight shared record owner
  `atome_record_utils.js` exposes `isRenderableAtomeIdentity`; Tool Genesis and
  Infos consume that same classifier rather than maintaining parallel rules.
...
# Infos virtual loading and contextual tool placement code map — 2026-08-05

- `bevy_panel_selectable_list.js`: shared replace-in-place virtual window,
  scroll-to-window calculation, spacer projection, exact count label and
  synchronous reusable list-drag session. Finder and future panel lists consume
  this owner instead of cloning lazy-load/drag machinery.
- `bevy_panel_info_runtime.js`: project/global 200-record window state, exact
  totals, revision guards, off-window selection loading and Contact-style rail
...
- `bevy_panel_finder_{runtime,view}.js`: tools-scope hold/drag/release/cancel,
  lifetime of the existing active Tool Registry payload, and project-row
  delegation to the existing `project_data.activateProjectWorkspace` owner.
  Project rows never terminate at generic Atome selection or maintain a second
...
- `project_view_molecule_info.js` projects Molecule/Section/Track properties into the existing Info runtime and enters the existing contextual rail in rail-only mode. Its `playMoleculeContextualOwner` keeps Ensemble on the native Molecule transport and forwards sequential/random rules to `project_view_playback_runtime.js`; its Delete item first lazy-loads the existing Delete tool owner, then routes the selected canonical owner to `ui.delete.selection`. `deleteCanonicalMolecule` commits owner/direct-member deletion and closes the registered timeline API after success so timeline, transport, and scene projections are disposed. `atome_contextual_edit_runtime.js` owns the generic disposable virtual-target seam; neither the rail nor the view owns a delete mutation.
- `eVe/domains/media/shared/media_playback_command.js` owns the immutable internal playback command used at every media boundary: canonical source identity, normalized playback kind, timeline start, natural or explicitly edited duration, source-in, rate, gain, pan, mute and visual order. Persisted `video_recording`/`video` and `audio_recording`/`audio`/`sound` values are normalized only here and are never rewritten in storage.
- `eVe/domains/media/shared/media_playback_audio_executor.js` is the single Kira execution owner shared by standalone, Ensemble and Molecule adapters. It owns unlock, source/extracted-video-audio preparation, asset load, voice start/stop and cleanup; `molecule.native.js` now delegates its PlayRecordCore adapter to this executor instead of maintaining a second implementation. Load payload composition applies the explicit occurrence `assetId` after source or facade payload fields, so normalized empty source ids cannot redirect registration to the media URL. Prepared Molecule assets are reused on resume/seek/scrub; the forced reload and missing-clip voice retry are removed.
- `runtime_transport.js` owns only scoped Molecule scheduling and the ephemeral ordered List sequence projection. It compiles persisted Sections/Tracks into shared media commands, preserves explicit crops/rates/gains/pans, applies no implicit stretch, and lets the session end at the latest command. `runtime.js` coordinates existing Record schedulers at the returned List offsets; `timeline_actions.js` owns the UI action registrations.
- `molecule_session_clips.js` prepares shared Kira assets for audio and extracted video audio only. Molecule visuals remain exclusively on the muted Bevy decoder; no hidden video, metadata-duration override or parallel raster pool exists in the session.
...
- Exhaustive acceptance: `tests/probes/molecule_eve_ui_acceptance_probe.mjs` owns the complete real Web journey and its optional audio/video Record/playback endurance; `molecule_ui_acceptance_support.mjs` owns canvas-to-client hit geometry plus real mouse/touch move, crop, lasso, zoom and split routing. `molecule_ui_drop_acceptance_scenarios.mjs` is the thin three-view entry point, `molecule_ui_drop_core.mjs` owns fixture/drop/reload assertions, `molecule_ui_drop_playback_support.mjs` owns shared hierarchy and playback observation, and the `molecule_ui_drop_{natural,list,matrix}.mjs` modules own only their view-specific gestures. Each drop journey uses real pointer input, reloads canonical projection, checks `parent_id`, playback progression/natural end and the absence of floating members. `scripts/run_molecule_ui_{matrix,repeat}.mjs` compose deterministic visual and repetition campaigns; `tests/performance/molecule_acceptance_performance.mjs` owns the 30-run kernel/virtualization budgets.
...
- Platform acceptance: `platforms/desktop-tauri/src/local_http_navigation.rs` owns bounded local-listener readiness for the sole runtime in `lib.rs`; `main.rs` only calls `squirrel_lib::run()` and must not duplicate bootstrap, server, command, or plugin registration. `build:molecule:tauri` builds the current debug `.app`; `build:molecule:ios` builds the app and AUv3 extension for the Simulator; and `build:molecule:ios:device` signs the generic iPhoneOS app plus embedded AUv3 extension without claiming physical-device interaction.

## iOS waveform and project Record ownership — 2026-08-21

- `atome/src/application/audio_runtime/record_audio_api.js` normalizes native terminal `waveform_peaks` (with `peaks` as a compatibility alias) to finite absolute values in `[0, 1]`, bounded at 256 entries, before media persistence.
- `AppNativeAudioRecording.swift` and `AUv3RecorderAnalysis.swift` are native post-stop waveform owners. AUv3 analyses the closed WAV outside its realtime render callback; `AUv3Recorder.swift` sends both contract names in `record_done`.
- `media_persistence_service.js` persists the canonical peak arrays on the audio-recording Atome. `project_audio_waveform_renderer.js`, `render_atom.js`, and the existing Bevy media resolver project those durable arrays; playback progress remains a separate WebGPU style overlay.
- `project_view_capture_to_timeline.js` requests one Molecule batch for Record replacement. `runtime.js` and `molecule_stores.js` commit timeline snapshot plus owner `playback_mode` together through `Atome.commitBatch`; the surface rereads the owner before treating Stop as successful. `playback_rules.js` rejects a performance marker with no usable clips explicitly, and `clearGroupTimelinePerformance`/`ui.timeline.performance.delete` clear clips and restore sequential mode through the same batch boundary.

## Structured Molecule playback and selection — 2026-08-21

- `tool_runtime_atome_mutation.js` builds the schema-v2 timeline from direct `parent_id` members in the same canonical creation, absorb, and merge batch. Each member occupies one content track at zero; the existing Molecule transport remains the only player. `backfillCanonicalMoleculeTimeline` repairs an older owner exactly once before it is played.
- `project_view_playback_runtime.js` owns queue-preserving manual jumps. List and Matrix only emit intent: a first inactive tap selects, a second tap clears selection, and an active queue jump restarts the chosen item while retaining the already-frozen queue order.
- `project_view_contextual_rail.js` marks its disposable structured rail record. The active `getAtomeContextualEditApi().enter(...)` bridge in `boot_runtime.js` forwards that record to `atome_contextual_rail_runtime.js`; the rail resolver and invocation runtime preserve the marker through the persistent Bevy contextual rail, expose Play for every selected List/Matrix item kind, and delegate it to `project_view_playback_runtime.js`. That runtime owns `isPlayingTarget(...)` for structured item, container, queue, and adopted-Molecule Play/Stop presentation; the rail model and `project_view_surface_context_runtime.js` consume it rather than reading selected-media state directly. The context runtime force-refreshes the current rail target on each playback-state announcement; it reopens the container rail only when no row/cell target exists and never lets `playingIds` own selection. Neither Mystic nor an inline List/Matrix control owns this behavior.
- In that structured rail only, `atome_contextual_rail_model_runtime.js` resolves the parent `record_action` presentation through the existing `record_action_live` behavior. The visible item remains the single Record action toggle with `{ mode: "live" }`; it never expands the key/live/audio/video capture palette. Natural contextual editing keeps its existing type-specific Record presentation.
...
- `project_view_mode_state.js` owns latest user intent without per-project FIFO and applies Natural through the same canonical surface transition as List/Matrix, so an older structured tree cannot survive a project restore. It records explicit intent before asynchronous surface work, checks the intent sequence again after persistence reads, and commits same-mode choices because the local-first in-memory default can differ from durable state. `project_view_records.js` owns canonical paged project reads, preserves `meta.project_id` membership, and excludes canonical deletion tombstones; `project_view_reorder_runtime.js` owns durable `hierarchy_order` writes and uses one canonical batch when available. The canonical shared drag session owns pointer preservation and drag cleanup; List renders a source fade plus insertion gap, while Matrix consumes the same press/drag/release lifecycle. `project_view_surface_runtime.js` retains a pending pointer-node preservation request across queued renders. List and Matrix rebuild their selected projection from canonical selection on load. `project_view_surface_context_runtime.js` owns the contextual rail and `project_view_surface_events.js` owns canonical change/size subscriptions.
- `project_data.js` owns a single global latest-wins workspace-activation generation. It starts the persisted-mode prefetch through `project_view_mode_state.js`, consumes the authoritative `loadProjectAtomes` projection, then calls that same owner's guarded restoration once after loading. The activation load carries internal `viewModePrepared: true`; `tool_genesis_project_load_runtime.js` skips only its redundant restoration in that case. A Natural/Matrix/List gesture after prefetch invalidates the old result and same-mode Natural is still committed, so a late restore cannot overwrite the user. Boot loads await the same single restoration before publishing project readiness; the authoritative refresh never starts a second one.
...
# Canonical List hierarchy and recursive transport — 2026-08-27

- `project_view_visual_panel.js` disables waveform progress through the existing record/composite-preview option. `virtual_scene_contract.js` and `project_scene_invalidation_runtime.js` honor that projection-only visibility flag; row/footer progress and video decoder identity are preserved. `bevy_ui_tree_motion_runtime.js` converts local-position updates to screen-space deltas, preventing a footer head at local y=0 from escaping to the top of the canvas.
- `project_view_navigation.js`, `project_view_molecule_list_model.js`, and `project_view_list_content.js` project only canonical Atome and Molecule records. `parent_id` owns membership and `hierarchy_order` owns sibling order; legacy Timeline Sections, Tracks, and Clips never manufacture List rows.
- `tool_runtime_list_model_migration.js` converts legacy editable Timeline snapshots through one idempotent `Atome.commitBatch`. `project_view_list_migration_runtime.js` invokes it before List projection and reloads canonical state after a successful transaction.
- `project_view_transport_plan.js` compiles nested sequential, simultaneous, random, and performance branches. `project_view_transport_runtime.js` owns the sole bounded clock, explicit transport commands, global/local time mapping, mute gates, and ephemeral progress snapshots.
- Recursive snapshots carry `rootRecord` and `activeLeafRecords`. `project_view_visual_subject.js` consumes those leaves independently of row selection/expansion; hierarchy path ids remain row-state signals, not media subjects. Surface events synchronize Visual before the transport resolves video nodes and discard a stale level snapshot when object playback takes over.
- `bevy_video_decode_source_runtime.js` retains the latest active/paused position in its existing per-window request registry, including requests preceding projection. Continuous playback tolerates sub-250 ms drift instead of seeking every tick; explicit seek/scrub remains precise and `seeked` redraws paused frames. It reuses `bevy_projection_media_contract.js` timeline normalization; no parallel decoder or timer was added.
- `project_view_footer.js` projects the global playhead. `project_view_list_view.js` and the shared `bevy_panel_selectable_list*` modules project fixed 44 px Atome/Molecule rows and patch only visible active local playheads.
- `project_view_list_preview_{model,gesture}.js`, `bevy_panel_record_timeline_preview.js`, and `project_view_occurrence_edit_runtime.js` own derived previews, scrub, crop, move, split, duplicate, join, and Rubber Band stretch orchestration. Media data and `parent_id` remain canonical; generated thumbnails and stretch assets are transient.
- `project_view_drop_intent_runtime.js` owns the five drop zones. `tool_runtime_molecule_combine.js` preserves Molecule envelopes, inserts directly into a matching mode, or creates a canonical wrapper through the existing mutation facade.
- `project_view_surface_runtime.js` remains the sole structured-view orchestrator; tree composition and unmount cleanup are focused internal modules, not parallel routes.

# Public directory implementation — 2026-08-30

- Server projection and invalidation: `server/directoryPublicService.js`, `server/wsAtomeOperations.js`; schema compatibility: `database/adole_schema_migrations.js` and `database/schema.sql`.
- Client population owners: `eVe/intuition/tools/contact_model.js`, `eVe/intuition/runtime/bevy_panel/bevy_panel_finder_data.js`, and `eVe/intuition/tools/communication_users.js`; Dashboard reuses Contact projection through `eVe/domains/dashboard/dashboard_data_adapters.js`.
- Shared missing-photo projection: `atome/src/squirrel/components/media_card_contract.js` resolves canonical `assets/images/icons/user.svg`; Bevy Contact and Dashboard consumers only project it and never persist it.
...
- `platforms/desktop-tauri/src/server/local_atome_sync_worker.rs`: conditional local-profile `atome_id` to remote-principal normalization.
- `platforms/desktop-tauri/src/server/local_atome.rs` and `local_atome_extended.rs`: current-profile bootstrap enqueue after remote sync configuration.
...
- `eVe/domains/user/public_directory_events.js`: shared remote-directory invalidation subscription used by Contact, Dashboard, Finder, and Communication. `eVe/domains/user/contact_change_events.js`: shared local contact population subscription used by the Dashboard Contacts category.
...
Choice palettes (2026-09-24, superseding the 2026-09-18 choice-icon contract): tool_definition_ssot.js owns resolveToolActiveChild alone (resolveToolChoiceChild, resolveToolChoiceIcon and entryForKey are removed); shared/tool_presentation.js resolves the label and the icon of the definition itself (explicit icon, then def.icon, then the key) and takes no children/activeToolIds; toolbox_runtime_model.buildMainEntries(content) and bevy_ui_main_menu_model.buildBevyMainMenuItems(content, ...) consume it, and atome_contextual_edit_model.js passes no activeToolIds. Both renderers still close on a leaf choice and replace a palette slot with its locked child (bevy_ui_main_menu_model.activeToolIdsFromLatchedState, atome_contextual_edit_runtime.activeToolIds); the keep_open_on_child_click / keepPaletteOpenOnChildClick opt-outs are gone. main_menu_content_runtime.js and main_menu_create_content_runtime.js no longer publish selectedChildKey for view/mode/record_action/page_create, bevy_ui_main_menu_runtime.js owns no PROJECT_VIEW_MODE_CHANGED_EVENT listener, and domains/rendering/project_view_contextual_rail.js marks the playback mode in force on the child instead of the slot icon. contextual_selection_levels.test.mjs, bevy_ui_main_menu_contract.test.mjs and create_tools_modes_contract.test.mjs cover slot identity, closure and slot replacement.

Lit tools (2026-09-24): the tools that stay lit once started now have one owner, `eVe/intuition/tools/core/active_tool_registry.js`, which declares each family's residence (`rail` for the record/playback family of the contextual rail, `menu` for text, drawing, code, page, placeholder and template) and the invocation path used both to read and to stop them; `tool_runtime_circle_tool_state.stopOperativeCreationTools` delegates to `stopActiveToolEntries`, and the table it duplicated is deleted. The lit presentation stays on the slot that displays the tool (the rail's Play becomes Stop; the ribbon keeps `shared/tool_presentation.js`), so nothing is duplicated in the registry. In the rail, `buildAtomeContextualEditTree({ activeSlots })` removes the lit rail tools from the ordinary level and pins one case per tool against `layout.bottom`, above the Atome handle, outside the scrolling viewport, pushing the ordinary stack up by exactly the reserved height; the pinned case keeps the level node id `atome_contextual_tool_<key>` (`…_icon`), so handler, tool identity and pulse are shared, and `atome_contextual_edit_runtime.js` keeps the activation order with the catalogue order as the cold-render fallback. A menu-resident lit tool keeps its ribbon slot and never gets a rail case. Both surfaces synchronize the unique `createRecordingIndicatorPulse` (`litToolIconIds()` for the rail, `resolveBevyMainMenuLitToolIconIds` through `litToolPulse` for the ribbon) and clear it at stop; a view mode that paints itself active never pulses. Covered by `tests/eve/contextual_rail_active_zone.test.mjs` plus `contextual_rail_scroll` and `bevy_ui_main_menu_contract`; real-runtime pixel acceptance in Web 3001 / Tauri remains to verify. The options a pinned tool opens on a long press stay anchored to that case and carry its palette accent when open; the case reads its own face from the level definition when the level still publishes it.
...
`bevy_ui_mystic_model.js` owns the Mystic's absolute stacking: the renderer puts every node on one flat scale (`GlobalZIndex(style.z_index ?? 0)`), so the tile grid carries `workspaceSceneLayerOrder('mystic')` down its whole subtree, the way `bevy_panel_tree.js` carries its own. A node left without a `z_index` is painted at 0, underneath every panel. `bevy_ui_mystic_runtime.js` owns dismissal on the native surface, where the document-level owner never sees the press: the presentation root closes on a press outside the tile grid and ignores only the reprojection of the still-held opening pointer (not released, and within `RIBBON_TOKENS.mysticOriginPressTolerancePx` of the opening point). Regression owner: `tests/eve/bevy_ui_mystic_contract.probe.mjs`.
...
### 2026-09-16 — Dashboard plein écran (état courant)

This section supersedes earlier Dashboard descriptions. `dashboard_layout.js` owns `surface_rect`, the five fixed top-to-bottom Calendar/News/Contacts/Monitor/Projects lanes, bottom alignment with Projects flush to the viewport, exact `1U × 1U` cards, the `2U × 1U` weather card, handedness mirroring, and the four-column mobile cap. It reserves neither menu height nor a creation column. `dashboard_records.js` orchestrates only the opaque profile base plus `cover` image, then delegates to `dashboard_header_records.js`, `dashboard_card_records.js`, and `dashboard_weather_card_records.js`. `dashboard_glass_material.js` derives supports from the shared `EVE_COMMON_SKIN_TOKENS.bevy.systemSurface` material. Every content cell, with or without media, first receives that support; `dashboard_skin.js#contentGlass` owns only its transparent fill, tint alpha, and blur radius, while `dashboard_skin.js#labelText` owns the fixed `16 px` ordinary-label size and the display-only eight-grapheme-plus-`.` abbreviation for longer values. `dashboard_card_records.js` then places project/contact media at opacity `1` without inheriting support transparency.

`workspace_main_menu_visibility.js` possède la suspension/réactivation du menu et démonte son tree pendant le Dashboard. `intuition_liquid_menu_renderer.js` empêche sa surface liquide de remonter pendant cette suspension et réconcilie aussi les projections orphelines. `user_workspace_surface_runtime.js` ferme et attend tous les panneaux enregistrés via `panel_globals.js`, puis suspend le menu avant la projection Dashboard ; `project_workspace_activation_runtime.js` le réactive après activation effective. `dashboard_news_modules.js` possède l’état météo jetable et, tant qu’un filtre est ouvert, `items` omet les deux modules — ils reviennent à l’affichage de base avec leurs relevés déjà chargés ; `dashboard_weather_preferences.js` persiste seulement `preferences.dashboard.weather_location` par l’API profil ; le Finder possède le géocodage inverse. Le rail est un outil à deux étages : les deux tiers hauts portent l’icône et le libellé de la rubrique et filtrent la catégorie (cascade des items depuis sa propre rangée, second appui pour revenir à l’affichage de base), le tiers bas porte le libellé `eve.dashboard.header.new` et garde la création ; la découpe appartient à `dashboard_layout.js#dashboardHeaderBands` et la ligne de séparation au token `headerSeparator`, sans moteur de défilement ni transition de focus supplémentaire. Le focus d’en-tête reste visuel seulement. L’ancien contrôleur de transition de focus, les records « + », les bandes, le voile et les ombres Dashboard ont été supprimés.

### 2026-09-20 — Mystic access, execution placeholders and narrow placement

`eVe/intuition/menu/context_menu_resolver.js` reads canonical object/project access at opening and shares command permission checks across root and nested entries. The existing contextual editing entry owns its derived access snapshot; late reads cannot remount a replaced or closed target. `context_menus.json` owns lasso composition. `eVe/domains/rendering/project_scene_intent_runtime.js` dispatches existing scene intents and execution-placeholder text commits; the scene engine remains the facade. `project_work_mode_state.js` guards modes and preservation. `mystic_layout.js` distributes full-size cells and `mystic_menu_items.js` paginates within their capacity.

Evidence and limits: `eVe/documentations/MYSTIC_VALIDATION_2026-09-20.md`.

Execution acceptance: the browser text-placeholder fill survived reload, then Mystic Delete removed it and closed the menu. Non-edit mode return does not read editorial ACL records; project background clicks do not select or create text. See `eVe/documentations/MYSTIC_VALIDATION_2026-09-20.md`.
...
# Lit tools: pinned rail zone and single pulse — 2026-09-24

- `eVe/intuition/tools/core/active_tool_registry.js` (new) owns which tools stay lit and where they live: `ACTIVE_TOOL_SURFACE` (`rail` | `menu`), `ACTIVE_TOOL_ENTRIES`, `isActiveToolId(toolId)` (a lit tool, not an open panel or palette), `isActiveRailToolKey(key)`, `readActiveToolEntries({ keepToolId })` and `stopActiveToolEntries({ keepToolId })`. It reuses the tool's own invocation path (`deactivate`, `close`, `setPageToolActive`, `setActive`) plus the published latch release, so no second command system exists; the lit label and icon stay on the slot that displays the tool.
- `atome_contextual_edit_model.js` owns the rail's pinned zone: `withoutMovedTools` removes the moved roots from the ordinary level, `pinnedHeight = min(n × itemSize, availableRailHeight)` is reserved against `layout.bottom`, the scrolling viewport keeps `availableRailHeight - pinnedHeight`, and `contextualToolNode` builds both a level case and a pinned case from the same definition and the same node id.
- `atome_contextual_edit_runtime.js` owns the activation order (`state.activeSlotOrder`, catalogue-order fallback) and returns the lit rail cases through `activeSlots()`; `litToolIconIds()` feeds the unique `createRecordingIndicatorPulse`.
- `bevy_ui_main_menu_model.resolveBevyMainMenuLitToolIconIds({ items, latchedByToolId, isStayLitTool })` returns the ribbon slots that display a lit tool (an expandable slot reads its locked `activeChild`, never an open panel); `bevy_ui_main_menu_runtime.js` owns the matching `litToolPulse` and clears it on unmount.
- Tests: `tests/eve/contextual_rail_active_zone.test.mjs` (pinned geometry and handedness, non-scrolling zone, non-duplication, no rail case for a menu-resident tool, stable ids, pulse sync and clear) with `contextual_rail_scroll` and `bevy_ui_main_menu_contract`; `npm run check:syntax` stays green. Real-runtime pixel acceptance in Web 3001 / Tauri remains to verify.

# Pinned rail case without a rail, and the stop no selection may block — 2026-09-24

- `active_tool_registry.js` gains `readActiveRailToolEntries()`: the lit facts of the rail family that run with no level (record through `record_action_state.js` or the capture owner's `BOOTSTRAP_RECORD_ACTION_RUNTIME_STATE`; playback through `selected_project_media_playback_state.js` then `project_view_transport_runtime.js`), each with the `extraInput` its stop needs.
- `atome_contextual_edit_runtime.js` reads those slots BEFORE its early returns and owns `mountPinnedZoneOnly()`: `definitions: []`, `activeAtomeId: ''`, a `…_pinned_shadow` node and the same `recordingPulse.sync(litToolIconIds())`. `install()` listens to the record/playback/tool-state events, so the zone repaints with the rail closed.
- `atome_contextual_edit_handlers.js` answers a pinned-only case (`extraDefinitions`) through `runActiveDefinition`, and turns a lit rail palette off when its level publishes no locked child.
- `atome_contextual_rail_definition_invocation_runtime.js` passes the definition's latched state to the selection guard (`active: definition?.active === true`), so a stop is never reported as `no_selection_target`; `atome_contextual_rail_runtime.js` drops its now-duplicate playback listener.
- Tests: `contextual_rail_active_zone.test.mjs` 16/16 covers the rail-less record and playback cases, the live stop payload and the object change; the adjacent rail/menu suites stay green. Real-runtime pixel acceptance remains to verify.

# Selection rail: one anchored placement for Copie and Contact — 2026-09-26

- The selection case of a list was already the canonical component: `bevy_panel_choice.js` composes `toggleableRowNode` down to `buildBevyIconButtonSurfaceNode` on the 30 px `EVE_BUTTON_SKIN_TOKENS.bevyButton` surface, and both consumers — the Copie list (`bevy_panel_selectable_list.js`) and the Contact list (`bevy_panel_contact_view.js`) — go through it. No button material, radius, tone, pressed translation or disabled opacity is re-declared in a panel.
- What did not follow the framework was the placement: each consumer wrote `position: [0, 0]`, which glued the case to the top-left corner of its row. Both ad-hoc writes are removed; a panel never places a control with a pixel of its own.
- `eVe/elements/skin/panel_skin.js` owns the rule as `listRow.selectionInsetPx` (`scalePx(3)`, i.e. the same three pixels of air the footer close disc uses), and `bevy_panel_choice.js` exports `selectionRailPosition(containerHeightPx, sizePx)` — x = inset, y = centered — as the single owner of a rail's position.
- `toggleableRowNode` gains the additive option `containerHeightPx` (default `null`): a control dropped into a taller container says so instead of placing itself, and a caller that declares nothing keeps its historical rendering unchanged. `bevy_panel_selectable_list.js` passes `resolvedRowHeight` and `bevy_panel_contact_view.js` the doubled row height, so the 60 px Copie and Contact rows centre their case with 3 px from the edge; Contact's protected own-card column reserves the same placement (`contact_select_*_protected`).
- `railRanges` now carries `railTop` (the rail's own y inside its row) and `bevy_panel_list_selection.js` reads the finger in the row's coordinates (`range.top + range.railTop + pointerY`) for both `press` and `drag`. Without that term a finger at the top edge of a doubled row answered for the row above; absent `railTop` the term is 0, so every compact consumer keeps its old bands.
- The five other consumers of `toggleableRowNode` — `eVe/intuition/dev/taxonomy_editor/taxonomy_editor_panel.js`, `eVe/intuition/runtime/bevy_panel/bevy_panel_info_view.js`, `eVe/intuition/runtime/bevy_panel/bevy_panel_home_nodes.js`, `eVe/intuition/runtime/bevy_panel/bevy_panel_comm_advanced_view.js` and `eVe/intuition/tools/communication_inbox_view.js` — declare no container height and are byte-for-byte unchanged; `radioGroupNode` keeps its own `[0, index × row]` stacking.
- Tests: `tests/eve/bevy_panel_choice_contract.test.mjs` (anchored placement through `selectionRailPosition`, and the option-less control unchanged), `tests/eve/bevy_panel_paste_contract.test.mjs` and `tests/eve/bevy_panel_contact_pick_contract.test.mjs` (rail position of the doubled rows), `tests/eve/bevy_panel_contact_contract.test.mjs` (protected column placement), `tests/eve/bevy_panel_list_selection_contract.test.mjs` (`railTop` band reading). Real-runtime pixel acceptance in the Web 3001 / Tauri lane remains **To verify**; Tauri and iOS are not claimed.
