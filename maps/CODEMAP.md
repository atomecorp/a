...
- `tests/probes/dashboard_bevy_runtime_probe.test.mjs` owns the real Playwright dashboard acceptance path: guest workspace, real main Atome handle click, canvas header and `+` clicks, panel/project plus actions, project item activation, fullscreen detail projection, toolbox reservation, handedness mirroring, dashboard teardown, and screenshot checks for entêtes, card contrast, and no dashboard paint inside the toolbox band.
