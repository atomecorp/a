...
- `eVe/domains/dashboard/dashboard_actions.js` owns dashboard `+` and item click actions. News, Store, Monitor, and Goals `+` actions are no-ops; Calendar and Contacts route to the existing eVe panel surfaces; Projects enter workspace `transition`, fade out and tear down the dashboard, then create/activate a project through the shared Matrix/Adole helpers, with concurrent project `+` clicks sharing one in-flight creation. In focused category mode, item hit-testing keeps the real item category separate from the visual lane category, and the full-height `+` strip routes to the active category. Project item clicks follow the same transition path and activate/load that project as the active Bevy desktop instead of opening a dashboard fullscreen detail.
...
- `eVe/intuition/ribbon/menu_handle_runtime.js` forwards a tap on the main Atome handle (`button[data-role="eve_intuitionx-handle"]`) to `eVe/intuition/eVeIntuition.js`, which toggles `eVe/domains/dashboard/dashboard_runtime.js` in active workspaces. `main_menu_auth_runtime.js` treats the visible neutral Dashboard scene as an active workspace even when no user project is current and auth state is still catching up, so the handle keeps alternating open/close in the default Dashboard mode. Dashboard runtime `toggle()` must close an active Dashboard and reopen a closed one; it must never be an alias for `open()` only. `tool.main.home` remains a normal `ui.home.panel` route so the user/Home panel is not coupled to the dashboard entry point.
- `eVe/intuition/tools/user_workspace_surface_runtime.js` owns post-session Dashboard surface activation for authenticated and anonymous sessions. It no longer resolves or loads the current user project for the default Dashboard path; it runs data-only Dashboard warmup for `__eve_dashboard_workspace__` when needed, stabilizes the existing main ribbon through `window.new_menu_v2.showFully()`, then ensures the full-viewport neutral `project_view___eve_dashboard_workspace__` host owns the shared `eve_surface_project` canvas before opening the dashboard runtime on that neutral scene. Its readiness contract requires `scene.projection.ok`, visible text-bearing Dashboard header records, and no deferred/skipped header resources in Bevy diagnostics. This owner does not create a second dashboard entry point, duplicate menu state, dashboard DOM, an alternate canvas, or a background project load.
