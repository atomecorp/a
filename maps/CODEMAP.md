...
- `project_data.js` owns a single global latest-wins workspace-activation generation. It prepares the persisted per-project representation before `setCurrent`, applies it through `project_view_mode_state.js` immediately afterward, and then consumes the authoritative `loadProjectAtomes` projection without rendering the scene again. The activation load carries internal `viewModePrepared: true`; `tool_genesis_project_load_runtime.js` skips only its otherwise-late redundant restoration in that case. Boot loads prefetch the same preference and now await its single post-load surface application before publishing project readiness, so iOS cannot release its native launch cover on an intermediate Natural frame before the remembered Matrix is mounted.
