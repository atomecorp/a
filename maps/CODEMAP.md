...
- `eVe/domains/rendering/bevy_web_presentation_runtime.js` owns browser Bevy presentation scheduling for the web runtime. It filters the known non-fatal WASM runner `unreachable` completion from page error reporting, primes bounded WebView presentation redraws through `request_atome_bevy_redraw`, coalesces hidden decode video-frame notifications into Bevy redraw requests, and records WebGPU/canvas context lost/restored events against the existing Bevy canvas without creating a recovery renderer or DOM fallback. `eVe/domains/rendering/bevy_wasm_diagnostics_runtime.js` owns attachment of WASM diagnostics readers to the shared perf diagnostics object so the startup/diff owner stays below the critical size threshold and does not duplicate diagnostics glue. The generated Web Bevy runtime in `platforms/web/bevy-renderer/src/lib.rs` uses an opaque canvas surface with Bevy's reactive desktop-app Winit settings, requests an initial redraw before any user input, preserves wake requests made before the winit event-loop proxy is installed, then wakes the Bevy winit event loop and emits redraw requests whenever queued WASM render operations, explicit redraw requests, or hidden video-frame notifications are applied. Web diagnostics count wake calls, redraw requests/applies, and accepted/applied video-frame redraws so startup, Atome creation, mutation diffs, deferred media resource updates, surface background changes, surface-size patches, and video playback present without an idle continuous loop or waiting for a pointer/keyboard event.
...
- `eVe/domains/media/selected_project_media_playback_runtime.js` owns selected active-project media transport for the cleaned canvas route. It resolves selected media records from `project_scene_runtime`, sends audio and extracted video-audio through the existing `Squirrel.av.audio`/Kira facade, preserves runtime-only Audio Atome pause/resume position across play-tool toggles through Kira voice start offsets, and requires selected project video playback to use the project timeline automation path instead of mutating project-scene records directly. It must not create visible per-Atome media DOM hosts or a second media engine.
...
- `platforms/web/bevy-renderer/` owns the browser/WASM Bevy WebGPU renderer surface. It consumes an explicit `AtomeRenderScene` JSON/wasm-bindgen input, binds Bevy to a caller-owned canvas selector, and exports `run_atome_bevy_renderer(...)`, `request_atome_bevy_redraw()`, explicit diff/resource/surface update exports, renderer diagnostics, `read_atome_bevy_video_backend_capabilities()`, and video copy diagnostics read/reset methods without implementing a DOM renderer, canvas-per-Atome path, or fallback renderer. The video backend diagnostic uses schema `atome.bevy.web.video_backend.v4` to expose the non-final `copy_external_image_to_texture` backend, target `gpu_external_texture_texture_external`, absent video-track API, browser `GPUDevice.importExternalTexture` availability, missing wgpu web create support, missing wgpu source descriptor and external-texture resource binding, and pinned `wgpu_web_external_texture_source_and_resource_binding_unimplemented` blocker. `atome/renderers/bevy-core/src/video_diagnostics.rs` owns copy/skip counters for the current non-final video copy path so probes can separate copied frames from missing sources, missing frame versions, not-ready decode sources, empty source sizes, and already-copied frame versions. Supported browser projections now include Bevy sprite shapes, visible pending media sprites, and Bevy RGBA texture sprites for text/raster images/SVG/persisted video posters/audio waveforms.
- Live project video external-texture work must stay in this same Bevy ownership chain: `eVe/domains/rendering/bevy_web_renderer_runtime.js` and `eVe/domains/rendering/bevy_video_decode_source_runtime.js` bridge browser runtime resources, `platforms/web/bevy-renderer/` owns WASM exports and web renderer integration, and `atome/renderers/bevy-core/` owns shared Bevy ECS/material/render logic. The current pinned `wgpu-27.0.1` web backend leaves `create_external_texture` unimplemented, so direct JavaScript `GPUExternalTexture` probes are diagnostics/reference only and must not become a second project renderer, hidden compositor, fallback, or canvas-per-Atome path.
