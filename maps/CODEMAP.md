...
- `bevy_panel_tree.js` owns the shared Bevy panel text, divider, and button node builders, including the body-content layer above the opaque panel shell. For every new component, its migration record evaluates existing BevyUI widget kinds, actual native/WASM availability, and the canonical Atome/Squirrel contract before choosing direct widget use or a proven-necessary composition. Builders configure or compose the selected primitive and must not reimplement widget drawing, interaction, state, geometry, or styling. Its `textNode` consumes canonical panel typography tokens and `dividerNode` consumes canonical divider paint/margins. Its `toolButtonNode` composes the shared Bevy ribbon tool node/content builders in normal panel-body flow, including the shared 2 px-lowered label metric, final 1 px label translation, and shadow clearance; it owns no separate tool visual contract. `bevy_ui_menu_surface.js` turns the shared `off`/`offPressed`/`on`/`onPressed` tool-skin tokens into the material plane and one non-interactive, tight dark GPU inner shadow without a bright rim. Panel Lab retains approved static text and divider plus the square latching tool-button specimen currently in review. Its close-reset ephemeral state belongs to `bevy_panel_surfaces.js`, never to the DOM or product state.
