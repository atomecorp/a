...
Cette section remplace les descriptions Dashboard antérieures. `dashboard_layout.js` possède `surface_rect`, les cinq lignes fixes News/Calendrier/Projets/Contacts/Moniteur, les cartes exactes `1U × 1U`, la météo `2U × 1U`, le miroir droitier/gaucher et le plafond mobile de quatre colonnes. Il ne réserve ni hauteur de menu ni colonne de création. `dashboard_records.js` orchestre uniquement le fond profil opaque + image `cover`, puis délègue à `dashboard_header_records.js`, `dashboard_card_records.js` et `dashboard_weather_card_records.js`. `dashboard_glass_material.js` dérive les supports du matériau commun `EVE_COMMON_SKIN_TOKENS.bevy.systemSurface`. Chaque cellule de contenu, média ou non, reçoit d’abord ce support ; `dashboard_skin.js#contentGlass` possède uniquement son remplissage transparent, son alpha de teinte et son rayon de flou. `dashboard_card_records.js` superpose ensuite tout média de projet/contact à opacité `1`, sans lui transmettre la transparence du support.
