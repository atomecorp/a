...
- `eVe/domains/dashboard/dashboard_actions.js` owns dashboard item click actions. Focused entêtes no longer expose a `+` creation strip or action route. In focused category mode, item hit-testing keeps the real item category separate from the visual lane category. Project item clicks enter workspace `transition`, fade out and tear down the dashboard, then activate/load that project as the active Bevy desktop instead of opening a dashboard fullscreen detail.
...
- `eVe/intuition/eVeIntuition.js` is now a sub-500-line closed boot coordinator after RF-01. It wires existing focused runtime owners and delegates main-menu creation to `eVe/intuition/ribbon/main_menu_bridge_runtime.js`; future changes must extend the focused runtime owners instead of re-inlining panel actions, text state, main-menu composition, Atome-edit footer lifecycle, or boot registration.
...
- `eVe/intuition/ribbon/menu.js` is the legacy DOM ribbon bootstrap/coordinator and must attach the shared `.eve-intuitionx-projection-tool` visual class through the extracted tool renderer instead of recreating browser-native button visuals. The legacy ribbon is no longer the visible main-menu owner; `main_menu_bridge_runtime.js` creates it as a private manual rescue surface behind the BevyUI legacy item. `menu_model.js` owns ribbon content cloning/patching, tool-entry normalization, handle/icon resolution, tool sizing/radius, color interpolation, transform composition, and pressed/tool visual-state helpers; its clone path recursively clones plain data while preserving function-valued action handlers instead of using browser structured cloning on runtime menu content. Runtime ownership is split across focused modules: `menu_shell_runtime.js` creates the ribbon/layer DOM shell, `menu_delete_visual_runtime.js` owns delete target/backdrop visuals and dragged-node attraction, `menu_drag_delete_runtime.js` owns drag-delete event routing, `menu_layout_runtime.js` owns metrics/overflow/reveal stops, `menu_scroll_runtime.js` owns wheel/native scroll snap behavior, `menu_tool_render_runtime.js` owns tool/palette button rendering and tool press hover routing, `menu_visibility_runtime.js` owns open/close/dismiss visibility state, `menu_placement_runtime.js` owns placement/handedness, `menu_public_api_runtime.js` owns the legacy public facade and latch adapters, `menu_auth_dock_runtime.js` owns auth dock transition animation, `menu_external_open_runtime.js` owns external-open width/geometry resync, and `menu_quick_capture_runtime.js` owns the Home-handle quick-capture overlay, reveal/hover state, fullscreen handoff, and sweep completion. `menu.js` must not reintroduce those helpers inline.
- `eVe/intuition/ribbon/bevy_ui_main_menu_model.js` owns the BevyUI main-menu item order, item-size contract (`resolveDashboardBlockUnitSize() / 2`), handedness-aware bottom layout, Atome edge order, per-block icon+label tree shape, shared icon sizing, single `lightgray` SVG icon tint, label vertical placement, and disposable BevyUI tree shape. Right-handed compact layouts may use a negative menu `x` when the surface is narrower than the fixed 60 px item row so the Atome item stays pinned to the right edge without shrinking or DOM scrolling; left-handed compact layouts stay at `x = 0`. `bevy_ui_main_menu_runtime.js` owns the `window.new_menu_v2` facade implementation, BevyUI mount/update/unmount calls, active surface/window resize observation, tool latched/external-open visual state, Atome dashboard toggle, and manual legacy-menu toggle. `main_menu_bridge_runtime.js` composes the private legacy ribbon and the BevyUI menu facade without allowing automatic DOM ribbon fallback.
- `eVe/domains/rendering/bevy_ui_runtime.js` owns the internal JavaScript BevyUI facade `window.eveBevyUiRuntime`: it normalizes Squirrel-owned UI trees, stores the interaction tree in CSS pixels, versions and queues overlay renders per tree so stale async texture hydration cannot overwrite newer UI records, projects the visible product path through non-selectable WebGPU project overlay records, and must not create DOM nodes per component. Native BevyUI WASM op submission is explicitly opt-in so the overlay-backed product menu does not emit unused Bevy UI events. Focused helpers keep the rendering concerns closed: `bevy_ui_image_runtime.js` hydrates `icon` / `image` sources and label textures into RGBA payloads through the shared Bevy media texture resolver, `bevy_ui_render_scale.js` keeps the submitted UI tree in logical BevyUI units, `bevy_ui_project_overlay_runtime.js` projects the same disposable BevyUI tree as stable non-selectable WebGPU records under the centralized `__eve_bevy_ui_` prefix, `project_scene_record_projection.js` classifies that prefix as ephemeral, `project_preview_runtime.js` excludes it from project preview captures, `project_scene_runtime.updateProjectSceneOverlay(...)` preserves existing scene effects when no explicit `effects` array is supplied, `virtual_scene_contract.js` preserves prehydrated overlay `bevyTexture` payloads and resource signatures through the Virtual Scene, `bevy_media_resource_runtime.js` must not re-resolve nodes that already carry a Bevy texture, and `bevy_ui_value_runtime.js` owns scalar/color/id normalization. `atome/renderers/bevy-core/src/ui/mod.rs` owns the corresponding native BevyUI ECS tree, image-node texture spawning, diagnostics, and event queue, while `ui/types.rs` owns the serializable UI op/node contracts, `ui/tests.rs` owns the focused Rust UI tests, and `platforms/web/bevy-renderer/src/exports.rs` exposes the opt-in native exports `apply_atome_bevy_ui_ops`, `read_atome_bevy_ui_diagnostics`, and `drain_atome_bevy_ui_events`.
