...
- `eVe/domains/rendering/project_scene_runtime.js` also owns project-scene text edit, resize, drag, and generic scene-intent commits. Drag/resize moves emit canonical `gesture_frame` batches for realtime sharing, while text commit and gesture end persist `set` events before disposable scene records rerender.
