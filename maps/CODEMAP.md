...
- `eVe/domains/dashboard/` owns the closed eVe dashboard runtime: JSON default loading, category/item normalization, pure layout, visual tokens, immediate fullscreen detail projection, R&D mockup-style Bevy record projection, data adapters, environment resize/handedness observation, and main Atom handle toggle lifecycle.
- `eVe/domains/dashboard/dashboard_environment.js` owns dashboard shell/environment reads: current project resolution, painted toolbox surface discovery, toolbox reserved height, handedness preference, environment signature, and active-category item projection for render layout. In focused category mode it distributes the active category's deduplicated items across the existing lanes without copying the same item into each lane. It measures visible toolbox controls/surfaces before transparent wrapper containers so the dashboard bottom aligns to the first painted maintoolbox pixel. It does not own durable project state or DOM rendering.
- `eVe/domains/dashboard/dashboard_environment_watcher.js` owns the dashboard-only live environment observer while the dashboard is open. It watches the project surface, surface parent, toolbox candidates, window resize, Intuition handedness state, and profile preference updates, then coalesces environment rerenders only when the environment signature changes. Category-visibility preference changes are handled by `dashboard_runtime.js` through the same profile-preference event so category lists are recomputed before render.
- `eVe/domains/dashboard/dashboard_render_scheduler.js` owns the dashboard-only render diff and animation-frame coalescing helper. It compares disposable Bevy records by id, emits only changed records plus removed ids, and must stay an internal dashboard performance helper rather than a second project renderer or scene owner.
- `eVe/domains/dashboard/dashboard_fade_runtime.js` owns the dashboard-only open/close opacity animation helper. It reads the duration from `dashboard_tokens.js`, updates only runtime `fadeOpacity`, requests Bevy record rerenders through the injected dashboard render function, and must not become a renderer, DOM transition layer, or durable state owner.
...
- `eVe/domains/dashboard/dashboard_actions.js` owns dashboard `+` and item click actions. News, Store, Monitor, and Goals `+` actions are no-ops; Calendar and Contacts route to the existing eVe panel surfaces; Projects closes and tears down the dashboard before creating and activating a new project through the shared Matrix/Adole helpers, with concurrent project `+` clicks sharing one in-flight creation. In focused category mode, item hit-testing keeps the real item category separate from the visual lane category, and the full-height `+` strip routes to the active category. Project item clicks close the dashboard and activate/load that project as the active Bevy desktop instead of opening a dashboard fullscreen detail.
...
- Dashboard visuals are projected as disposable logical-coordinate records into the existing project Bevy surface through `project_scene_runtime.js`; there is no dashboard DOM renderer, no per-item DOM node, no dashboard-side scale transform, and no second visible canvas. Empty categories do not emit placeholder cards. The open dashboard now uses one low-layer project veil record, one violet `reserved_band_fill` record across the toolbox-reserved band below the dashboard, and one vertical `bottom_shadow` texture-gradient record so the foreground dashboard casts depth while the remaining bottom project area is deliberately masked behind the menu. Dashboard Bevy records are decorative/runtime records with `selectable: false`; parked dashboard records are hidden, non-interactive, and opacity-zero, while warmup-only parked records are also moved offscreen before first display. `dashboard_runtime.js` owns the open/close fade as ephemeral `fadeOpacity` over the existing Bevy records, with duration configured by `dashboard_tokens.js` through `transitions.dashboardFadeMs` (default `500`); Atome Bevy core applies that opacity to spawned and patched shape/image/text sprites, non-texture `TextColor`, and generated shape-shadow overlays through `AtomeVisualColor` / `AtomeVisualOpacity`, so the fade is renderer-applied rather than a delayed parking effect. This is not a DOM transition, CSS layer, alternate renderer, or durable state. `dashboard_runtime.js` offsets the Dashboard layer band above the current durable project stack on each active render, and `project_scene_stack_runtime.js` excludes Dashboard/Molecule ephemeral records from durable media/drop stack allocation while clamping new durable records below visible Dashboard records. Dashboard close/refresh/project-change cleanup reconciles all known `__eve_dashboard_` ids from runtime state and Bevy virtual scenes so late RAF renders or orphan scene entries cannot stay visible. Dashboard interaction is owned by `dashboard_runtime.js` layout hit-testing, and the toolbox-reserved band is swallowed as an inert canvas zone while the dashboard is open. Fresh opens clear any focused category, invalidate in-flight warmup through `warmupSerial`, and arm a 180 ms input guard so only the pointer sequence that opened the dashboard is swallowed. Warmup aborts while the runtime is active, opening, closing, or serial-invalidated, so preheated hidden category records cannot overwrite a visible open layout. The dormant `backdrop_blur` effect path is kept for later renderer work but is not emitted by default.
- `eVe/domains/dashboard/dashboard_records.js` owns image-backed Dashboard card projection. Card shape fill is a slight `+3%` same-hue lift from the lane entête color so cards stay visually readable over the lane body while preserving the rubrique color family. Contact cards may emit one Bevy image record from normalized `metadata.user_face` between the card shape and Bevy text, using `media_fit: "cover"` plus the card content `corner_radius`. Project cards may emit one Bevy image record from renderer-owned `metadata.project_preview_source`, using `media_fit: "contain"` plus the same card content `corner_radius` so the full project miniature stays visible, keeps its source ratio, and lets the existing card color show through margins. Image-backed cards also emit one bottom-only-rounded `card_label_backdrop_*` Bevy image record between the media and text; it spans the full card content width, uses one fifth of the card content height, uses the same card fill at `0.6` opacity, and exists only to improve label readability over media. `eVe/domains/rendering/render_atom.js` carries `media_fit` / `object_fit` into disposable media content, while `eVe/domains/rendering/bevy_media_texture_image_fit.js` and `bevy_media_texture_resolver.js` own the product-neutral contain/cover draw rect plus rounded alpha clipping before the texture reaches Bevy. `eVe/domains/rendering/project_preview_runtime.js` resolves project sources from persisted preview fields, the current project scene records, or `loadProjectAtomes(...)` followed by the current Bevy scene state; `bevy_project_preview_capture_adapter.js` keeps the reusable capture iframe/canvas paintable behind the app with a cold-runtime timeout, and `bevy_project_preview_capture_frame.js` waits for deferred Bevy resources, rejects skipped/pending resources, and refuses transparent or nearly empty PNG captures. Dashboard opening invalidates the Projects item cache before first paint and forces async hydration to recapture the current project preview, so a reopening never displays a stale current-project thumbnail. These image records are disposable Dashboard projection fields, not canonical Atome properties, DOM backgrounds, per-item canvases, Matrix DOM clones, or symbolic fallbacks.
- Product-neutral rounded shape and shadow support flows through `corner_radius` and `material.shadow` from render records to the Bevy projection adapter and Atome Bevy core. Rounded shape masks are generated by `atome/renderers/bevy-core/src/texture.rs` through the shared `spawn.rs` texture-handle helper used by both initial scenes and later spawn ops; disposable shape shadows are generated by `atome/renderers/bevy-core/src/shape_shadow_overlay.rs`, keep the shape `AtomeCornerRadius` and owner opacity as runtime component state, and use the same rounded-rectangle signed-distance geometry as a filled shadow mask so `material.shadow` starts at the rounded card contour while radius-less shapes keep rectangular shadows. `components.rs`, `spawn.rs`, and `render_ops.rs` keep base visual color separate from opacity so style opacity patches remain linear and non-cumulative across shape, image, texture-backed text, and `TextColor` records. `shape_shadow_overlay_tests.rs` owns these focused Rust guardrails, while `backdrop_blur_tests.rs` owns the dormant scene-effect blur guardrail split out of the general Bevy core tests. Dashboard cards use this contract for `10px` radius and item shadows, matching each visual lane entête color while the lane body is derived from that entête with `laneShadePercent: -10`; dashboard entêtes use one external `header_side_shadow` image/texture gradient through the existing Bevy image path so the single lateral shadow reaches the tracks without a full-width shadow surface, shape-shadow blur, or interior band over the entête column.
...
- `eVe/intuition/tools/user_workspace_surface_runtime.js` owns post-session workspace surface activation for both authenticated and anonymous sessions. It resolves the current project from the explicit id, `window.__currentProject`, `AdoleAPI.projects.getCurrentId()`, or `eveToolBase.getCurrentProjectId()`, reuses only an attached canonical project surface inside `project_view_<projectId>` before calling the project loader, otherwise asks the existing loader to attach the resolved current project to the shared `eve_surface_project` canvas, then stabilizes the existing main ribbon through `window.new_menu_v2.showFully()` before opening the dashboard runtime with that explicit `projectId`. Its readiness contract requires `scene.projection.ok`, visible text-bearing Dashboard header records, and no deferred/skipped header resources in Bevy diagnostics; a warmed workspace can skip repeat warmup only when the non-visible header projection already exists. This owner does not create a second dashboard entry point, duplicate menu state, dashboard DOM, or an alternate canvas.
...
- `tests/probes/dashboard_bevy_runtime_probe.test.mjs` owns the real Playwright dashboard acceptance path: guest workspace, boot-open-aware dashboard readiness, real main Atome handle click when needed, canvas header and `+` clicks, panel/project plus actions, project item activation, fullscreen detail projection, toolbox reservation, handedness mirroring, dashboard teardown, screenshot checks for entêtes/card contrast/no dashboard paint inside the toolbox band, Dashboard project-preview pixels, project/contact media rounded-corner clipping, image-card label backdrop records/pixels, and browser-error checks covering console, page errors, request failures, and HTTP response failures. Its focused helpers live under `tests/probes/dashboard_bevy_runtime/`: `runtime_support.mjs` owns Playwright waits/click helpers, `snapshot_support.mjs` owns dashboard runtime/record snapshots, `visual_support.mjs` owns shared screenshot color assertions, and `project_preview_support.mjs` owns the canonical preview/contact-photo fixtures plus Dashboard card media/pixel assertions.
...
- `tests/probes/dashboard_workspace_stress_probe.test.mjs` owns the deep real Playwright stress path for dashboard/project/Atome regressions: it creates an isolated 10-project namespace including one Atome-only project plus media projects, writes text/shape Atomes through `window.Atome.commitBatch`, imports image/audio/video through the existing project drop creator, verifies canonical `state_current`, Bevy scene IDs, media node kinds, zero visible dashboard records, one project canvas, zero per-Atome DOM hosts, repeated dashboard open/close, 30 header clicks, project-card switching, dashboard reopen after switching to an Atome-only project, viewport resize after switch, and dynamic Calendar/Contacts invalidation. Its focused helpers live under `tests/probes/dashboard_workspace_stress/` and produce failure screenshots plus JSON reports under `temp/probe_reports/dashboard_workspace_stress/`.
...
- `tests/eve/dashboard_render_performance_contract.test.mjs` owns the non-browser dashboard render budget and contract checks: visible-item bounded layout, exact simple/double snap points, pixel-snapped flush entêtes, square shadowed card interiors, full-height plus strip, handedness mirroring, painted-toolbox reservation over transparent wrappers, live environment signature watching, preference update propagation, compact high-density image icons, one-record text labels through higher-density `text_style`, single external texture-gradient `header_side_shadow` projection with no `material.shadow`, dashboard project veil, violet reserved-band fill, bottom texture shadow, no default `backdrop_blur` emission, cache-first category activation, no-op/panel/project action routing, project `+` close-before-create ordering, concurrent project-creation de-duplication, active project-plus hit routing, and dashboard diff/scheduler coalescing. `tests/eve/dashboard_layout.test.mjs` guards active-category item deduplication, distribution across lanes, item hit-test category ownership, and active-plus routing. `tests/eve/dashboard_focus_color_contract.test.mjs` guards unfocused multi-color rubrique projection, focused active-category color flood across dashboard chrome, entête-matched cards, lane darkening, and active-header toggle restoration.
...
- `eVe/domains/rendering/bevy_projection_adapter.js` owns the strict browser Bevy projection adapter from `AtomeRenderNode` Virtual Scene data into the `run_atome_bevy_renderer(...)` payload shape. It maps explicit id, kind, parent, logical position, logical size, local transform scale/rotation/origin, paint-order layer, opacity, selected flag, fill color, text, shape `material.shadow`, media source, media texture, media `uvRect` / pixel `sourceRect` crop data, video timeline projection, waveform peak fields, and project-audio waveform playback progress, normalizes direct CSS z-index/layer payloads to the Rust `i32` boundary when no Virtual Scene render layer exists, rejects unsupported advanced Bevy blend modes instead of falling back to normal, validates video timeline numbers and speed before payload emission, and rejects incomplete projection nodes instead of inventing silent defaults. `eVe/domains/rendering/bevy_projection_style.js` owns reusable Bevy projection color and shadow normalization consumed by that adapter.
...
- `eVe/domains/rendering/project_scene_runtime.js` `updateProjectSceneOverlay` is the higher-level overlay batch path for owners such as the dashboard that need removals, records, and scene effects committed before one Bevy projection render. `reconcileProjectSceneRecordsByPrefix(...)` is the matching cleanup helper for overlay owners that must park controlled records and remove orphan Bevy ids by prefix without a despawn/spawn fallback. These paths keep effects and records in the same logical projection pass and must not create a second canvas, DOM overlay, or parallel renderer.
- `eVe/domains/dashboard/dashboard_record_parking.js` owns hidden-record parking helpers for dashboard Bevy records. Dashboard open/close and category switching reuse parked records by visibility/style/text updates instead of despawn/spawn churn. `dashboard_render_scheduler.js` owns the cancellable dashboard RAF queue; close must cancel any pending category/header frame before records are parked so a stale visible render cannot run after shutdown.
- `eVe/domains/dashboard/dashboard_runtime.js` owns dashboard projection lifecycle and warmup, including `state.warmedProjectId` so workspace reopen can skip redundant hidden projection when the current project already has dashboard records. It also guards label-editor Enter/Escape ownership so keyboard cancel/commit cannot close the dashboard, while explicit project item/plus actions clear that transient guard before closing. `dashboard_interaction_runtime.js` owns dashboard canvas pointer/wheel routing, item long-press detection, text-field hit resolution, scroll snapping, and suppression of click actions after label-edit long press. `dashboard_data_adapters.js` projects projects from the canonical list plus the current project when needed, and projects contacts from `collectPeopleDirectory()` plus local `eve_contacts_local`; local contacts are ordered before directory contacts, matching directory entries are replaced by the local editable payload, and non-matching directory contacts remain visible after the editable local set. `dashboard_item_text_fields.js` is the shared field contract for dashboard card text: projects/contacts expose a title field, calendar cards expose separate editable title and start-date fields. `dashboard_label_edit_runtime.js` owns hidden-text-service label editing state for dashboard items, including initial full selection, pointer caret placement, pointer drag selection, native keyboard caret movement, and Bevy-projected edit selection; `dashboard_label_persistence.js` delegates project, calendar title/start-date, and local-contact label writes to existing owners, and non-local contacts never arm edit. Calendar panel reopen refresh is owned by `calendar_panel_init.js`/`calendar.js` so dashboard-side `CalendarAPI.updateEvent` commits are visible when the panel is opened after being initialized and closed. `dashboard_category_activation.js` owns header activation reactivity: it flips active state before heavy card projection, cancels stale hydrations, defers cached card content while the category is still active, and prevents stale hydration from repainting a closed or superseded category. `dashboard_data_controller.js` owns cache-per-project, targeted category hydration, and structured cache invalidation; `dashboard_data_invalidation.js` wires CalendarAPI, contacts-directory, project, and generic dashboard Atome mutation events to that controller. Dashboard lifecycle closes only on true project id transitions; same-project `squirrel:project-changed` events during reload/bootstrap invalidate cache without parking the active dashboard records. The runtime projects hidden dashboard records through the existing Bevy overlay path so reusable entities and textures are ready before the visible click path; focused card/title/date ids are stable visual slots so rubrique switches reuse warmed records by visibility/style/transform instead of text rerasterization. It must not mount dashboard DOM, allocate a visible canvas, or start a parallel renderer.
...
- `eVe/domains/rendering/project_preview_runtime.js` owns shared renderer-produced project preview capture for Matrix and Dashboard consumers. It filters Dashboard, wallpaper, surface-background, and background records before calling the existing Matrix/WebGPU preview renderer; it prefers current scene records and merges loader records only to hydrate projects whose scene has not yet published source records. Consumers must use its persisted preview descriptor instead of cloning Matrix DOM or capturing DOM screenshots.
- `eVe/domains/rendering/bevy_project_preview_capture_adapter.js` and `bevy_project_preview_capture_frame.js` own the Dashboard/Matrix project miniature capture surface: one hidden `/eve_preview_capture.html` iframe, one reusable Bevy/WebGPU canvas sized to the requested capture target, transparent `run_atome_bevy_preview_renderer`, cold frame-runtime readiness bounded by an explicit timeout, and PNG export returned as disposable preview data with dimensions. This is a capture path, not a live project embedded in Dashboard blocks and not a fallback renderer.
...
- `eVe/intuition/runtime/eve_intuition/main_menu_content_runtime.js` owns the main Intuition menu content declaration. The visible main sequence is now `home`, `ai`, `find`, `capture`, `time`, `communicate`, `mode`, and `view`; `ai` is the inline IA prompt entry, `capture` is the product capture palette entry used by browser media import, and the unimplemented no-op `help` tool is not exposed. `eVeIntuition.js` injects panel/tool callbacks only and must not re-inline the content object.
...
- `eVe/intuition/runtime/eve_intuition/boot_runtime.js` owns eVeIntuition boot-time menu exposure, auth sync, panel surface registration, Flower/footer/vector/draw installation, deferred non-critical module warmup, UI tool registration, footer API exposure, debug runtime installation, and reload-time workspace reveal for already-active authenticated or anonymous sessions. `eVeIntuition.js` creates the main ribbon closed (`open: false`) so disconnected boot cannot paint toolbox buttons before the login accueil. Workspace/tool warmups start only after an active workspace is known or after `eve:login-choice-mounted`; a visible current project surface on the canonical project canvas also counts as active workspace evidence during reload ordering, so dashboard boot is not blocked when auth state publication lags behind project restoration. `eve:login-choice-visible` remains a post-fade readiness signal and is emitted after the login choice entry fade finishes, so disconnected boot does not reveal toolbox content before the visible login accueil. When a workspace is already active at boot, `boot_runtime.js` waits until a current project exists, retries briefly when the auth signal arrives before project publication, and then calls the canonical `openWorkspaceDashboardAndMainMenu({ source: 'boot_workspace', projectId })` entry once; that source loads the project surface, warms dashboard records/textures, opens the dashboard, and fully shows the existing ribbon after refresh/reboot. Any boot-open failure is recorded on `window.__eveWorkspaceBootOpenError` without creating an unhandled promise rejection. It must not create a second dashboard/menu path. `eVeIntuition.js` must not re-inline boot/warmup/register code.
...
- `eVe/intuition/runtime/eve_intuition/main_menu_content_runtime.js` owns the authenticated main ribbon content declaration. The visible main sequence is `home`, `ai`, `find`, `capture`, `time`, `communicate`, `mode`, and `view`; `ai` opens the inline IA prompt directly in the ribbon, `capture` opens the product capture palette including the browser media import action, `time` exposes `calendar` instead of the removed visible `timeline` child, `mode` is a palette whose children are `perform`, `mode_edit`, and `mode_consume`, and `view` is a palette whose children are `view_list`, `view_table`, and `view_natural`, while `matrix`, `activity`, and `undo` remain catalog/runtime tools but are no longer main-ribbon children.
...
- `platforms/desktop-tauri/Cargo.toml` declares optional Bevy dependency `0.18.1` behind `bevy_backend` and explicit renderer feature groups `bevy_renderer_core` and `bevy_renderer_native`. `bevy_renderer_core` enables Bevy WebGPU/window/winit/render/sprite/camera crates and the Linux `x11`/`wayland` Winit backends required when `bevy_winit` is active; `bevy_renderer_native` extends that core contract with standalone native-window extras such as `bevy/multi_threaded`. Renderer features do not enable `bevy_audio`, so Kira remains the single Atome audio engine.
