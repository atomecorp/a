...
Current workspace-entry contract (2026-07-22; supersedes older statements that make Dashboard opening wait for project readiness): `user_workspace_surface_runtime.js` owns one Dashboard-first orchestration. It mounts the neutral Dashboard and main menu first, then invokes the existing `project_bootstrap.js` readiness owner behind that foreground and refreshes Dashboard data after a project is ready. A project bootstrap failure leaves the mounted Dashboard intact and returns an explicit `project_bootstrap` phase; Dashboard mount/refresh failures return their own explicit phases. This flow is shared by authenticated entry and `boot_runtime.js`; it never creates a second renderer, moves the shared canvas to a user project while Dashboard is active, or changes media/recording paths.
