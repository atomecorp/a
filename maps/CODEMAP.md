...
- `scripts/server_update.js` owns reproducible production update flow, including dependency reinstall, service restart, and Nginx reload. It logs each phase (`env`, `backup`, `systemd-unit`, `git-update`, `verify-source`, `npm-ci`, `restart`, `status`, `health`, `nginx-reload`) with command, cwd, duration, and exit status, and runs deployed-source verification immediately after Git update and before `npm ci`.
- `scripts/verify_deployed_source.js` owns deployed-source verification markers for production routing and lockfile freshness, including the npm 10 top-level `@emnapi/core` and `@emnapi/runtime` peer entries required before production `npm ci`.
