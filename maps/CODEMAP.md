...
- `eVe/intuition/eVeIntuition.js` owns the authenticated main ribbon content declaration. The visible main sequence is `home`, `find`, `time`, `view`, `help`; `view` is a palette whose children are `view_list`, `view_table`, and `view_natural`, while `matrix`, `activity`, `capture`, `perform`, and `undo` remain catalog/runtime tools but are no longer main-ribbon children.
...
- `eVe/intuition/tools/user_login_sequence.js` owns the unauthenticated login orchestrator and preserves the public `createUserLoginSequence(...)` entry point. `eVe/intuition/tools/user_login_choice.js` owns the first pre-authentication choice screen: black left/top `entrez sans compte` routes through the existing anonymous user flow, and white right/bottom `authentifiez vous` opens the credential sequence. `eVe/intuition/tools/user_login_credentials.js` owns the extracted phone/password collection UI, with voice helpers isolated in `user_login_voice.js`. `eVe/intuition/tools/user.js` opens this full-screen sequence instead of showing the legacy compact auth dialog, while `executeLoginFlow` calls the atomic `AdoleAPI.auth.bootstrap` contract so existing-phone attempts verify the password and unknown-phone attempts create through the backend-owned protocol. Anonymous entry calls only `AdoleAPI.security.ensureAnonymousUser({ force: true })`, relies on the canonical session events emitted by Atome auth state, waits for `squirrel:project-changed` / `window.__currentProject`, then closes the choice screen and unlatches the Home tool; `eVe/intuition/tools/project_bootstrap.js` remains the sole project/workspace bootstrap owner. `eVe/intuition/eVeIntuition.js` treats a confirmed anonymous session as an active workspace for the main ribbon and Flowers so the desktop tools remain available, while `user.js` still distinguishes anonymous workspaces from named accounts for profile loading, delete visibility, and the guest-exit action. `eVe/intuition/eVeIntuition.js` routes Home through the canonical `window.open_home_panel` / `window.close_home_panel` functions owned by `user.js`. `eVe/intuition/eVeIntuition.js` and `user.js` auth-check handling open the home panel/login sequence once the initial auth gate reports an unauthenticated user, so first display lands on the choice screen before any phone request.
