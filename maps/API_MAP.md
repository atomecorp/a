...
- `updateUserProfileIdentity(identity, { userId })` is the Contact-facing identity mutation. It preserves all non-identity Home sections and emits the same `eve:user-profile-updated` contract as a Home save.
...
- `preferences.visual.renderStyle` accepts `flat`, `liquid` and `mystic` (default `flat`), and `preferences.visual.mysticRoundness` is clamped to `0..1`; an unknown style falls back to `flat`. `normalizeVisualPreferences()` in `eVe/intuition/tools/user_visual_preferences_model.js` is the only normalizer and `setIntuitionLiquidPreference({ renderStyle, theme, roundness })` the only publisher; the menu surfaces read `isIntuitionMysticActive()`, `readIntuitionMysticRoundness()` and `isIntuitionSurfaceRecordActive()`.
