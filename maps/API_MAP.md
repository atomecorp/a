...
- Workspace main menu: `workspace_main_menu_visibility.js` is an internal eVe runtime helper, not a public API. It may call only the canonical `window.new_menu_v2.showFully()` / `measure()` / `refresh()` surface after auth/workspace entry to reassert menu visibility for Dashboard, project activation, Bevy panel opening, and Dashboard/project restoration. In real BevyUI runtime mode, visibility requires both mounted main-menu overlay diagnostics and matching `__eve_bevy_ui_eve_bevy_ui_main_menu_*` records in the current foreground scene.
