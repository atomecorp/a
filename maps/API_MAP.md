...
- Open/close/render are ephemeral Bevy projection operations over the neutral Dashboard scene by default. Opening includes project-veil, violet reserved-band-fill, and bottom-shadow records below the dashboard foreground, then fades the shared surface in at canvas level. Closing cancels pending dashboard work, fades the shared surface out while records remain present, clears dashboard scene effects, removes every `__eve_dashboard_` record from the scene through prefix reconciliation, and empties runtime `records`/`layout`; a closed Dashboard therefore leaves zero Dashboard Bevy records or nodes in the active surface. Warmup is data-only and never builds Dashboard records or writes hidden/offscreen nodes into the scene. `backdrop_blur` remains an internal scene-effect contract for later renderer work but is disabled by default.
- Dashboard render calls keep an internal record cache and use `dashboard_render_scheduler.js` to emit changed records plus removed ids to `updateProjectSceneOverlay(...)`; record removals, record updates, and scene effects are merged into one project-scene projection per logical dashboard render. Pointermove, wheel, and inertial scroll are coalesced through a cancellable `requestAnimationFrame` scheduler so close cannot be followed by a stale visible dashboard render; open/close and item-detail paths render immediately without multi-frame record fade.
