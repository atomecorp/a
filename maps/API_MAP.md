...
### 2026-09-19 — Mystic menu: canonical set, aligned table, one wave plan

- `MYSTIC_DESKTOP_ITEMS` in `eVe/intuition/mystic/mystic_menu_items.js` is the ONLY list of keys the Mystic root menu shows — `home`, `capture`, `dashboard`, `communicate`, `create`, `calendar`, `info`, `find` — each with the rung it owns. `resolveFlowerContextItems` stays the only item producer: when `isIntuitionMysticActive()` is true and the context is neither a text field nor an explicit surface item, it returns those eight entries through the existing `buildFlowerContextToolItem`, so handlers, submenus and the hit test are the Flower's own and no content key is new. `help` is no longer a rung of this menu.
- `computeMysticLayout({ slots, center, anchor, surface, tileSize, gap, roundness })` returns `center` (the Atom's cell, glued to the edge the menu was opened on), `extents` (`{ left, right, top, bottom }`, the cells really used on each side), `centred` (`{ x, y }`, true on an axis the surface is too small to hold, where the block falls back to centring) and `tiles`. `resolveMysticLayoutMargin({ tileSize })` is `tileSize / 2`: the room a caller reserves before opening is the Atom's own cell, never the whole block.
- `planMysticTiles({ tiles, closing, reducedMotion })` returns `{ tiles, totalMs, minDistance, maxDistance }`, and each tile carries `ring`, `rank`, `delayMs` and `durationMs` (800 ms a turn, 600 ms for the tile nearest the centre, and a rung that starts a QUARTER of the previous turn into that turn — 150 ms after a 600 ms turn, 200 ms after an 800 ms one, so the delay is the running sum of those quarters and never a constant). `totalMs` is `max(delay + duration)` and is what `sampleMysticMotion` compares to say `done`; `resolveMysticTiles` reads that same plan, so opening and closing are one clock. `MYSTIC_BACK_ALPHA` and `MYSTIC_FLIP_STEP_RATIO` are gone; `MYSTIC_HOLE_DOSE` is the dose of the menu plate laid in the cell a turning plate leaves behind.
### 2026-09-19 — Mystic: the seam is a preference, the look is a skin

- `preferences.visual.mysticTileGap` is the one value that decides the room between two Mystic plates. `normalizeMysticTileGap(value)` in `eVe/intuition/mystic/mystic_layout.js` is the only clamp: a value that is not a number falls back to `MYSTIC_TILE_GAP_PX` (`1`), a value under the floor is raised to it, and a value over `MYSTIC_TILE_GAP_MAX_PX` (`12`) is brought back to it — no preference, profile or caller can make two plates touch, because the seam is what makes the menu read as a table of cells instead of one rectangle of colour. `user_visual_preferences_model.js` (default `1`, ceiling `12`), `intuition_liquid_preference.js` (`tileGap` in the normalised preference, `readIntuitionMysticTileGap()`) and `resolveBevyFlowerTreeGeometry({ gap })` read that same normaliser; `computeMysticLayout` publishes `layout.gap` and each tile's `gap`, `resolveMysticTileAt({ gap })` keeps it on the placed tile, and the Home panel offers the presets `1 / 2 / 4` under `home_mystic_gap`. Two neighbouring cells are `size + gap` apart and the flip law is not affected by the seam.
- `EVE_MYSTIC_SKIN_TOKENS` in `eVe/elements/skin/mystic_skin.js` is the only description of what a Mystic element looks like. It is registered as `mystic` in `eVe/elements/skin/index.js` and read through `resolveMysticSkin(overrides)` / `resolveEveSkinTokens('mystic', overrides)`, the same path the panel, button, tool and dashboard skins take. It names the shared `bevy.systemSurface` background (never its glass veil, which is what left the tiles invisible), that surface's own contact shadow, the icon base, `EVE_SEMANTIC_COLOR_FAMILIES` and the Atom mark, and adds only what is the menu's own: `rim`, `hole.dose`, `edge.{softnessPx,maxSeamShare}`, `family.mix`, `center.{plate,tint,icon}` and `radius.roundnessDefault`. `mystic_tokens.js` keeps no literal — every value it exports is a read of that skin.
- `resolveMysticEdgeSoftness(gap, skin)` is `min(skin.edge.softnessPx, gap × skin.edge.maxSeamShare)`, with the skin's own cap for a zero or unknown seam, and `buildMysticSurfaceUniforms` derives `mystic_style[3]` from the seam the image really carries: a wider seam can never become a blurrier plate, and two neighbours can never blend into one another. The rim's hue stays in the shader (it mixes toward the system content colour), so no token declares a value nothing reads. No new public API, MCP tool, renderer or mutation path is introduced.
### 2026-09-19 — Mystic: the opening length is a preference, edited by the standard input box

- `preferences.visual.mysticOpeningMs` is the ONE length a caller may ask for: the whole wave, from the first plate leaving to the last one landing. `normalizeMysticOpeningMs(value)` in `eVe/intuition/tools/user_visual_preferences_model.js` is the only clamp — a value that is not a number falls back to `MYSTIC_OPENING_MS_DEFAULT` (`2000`), a value under `MYSTIC_OPENING_MS_MIN` (`300`) is raised to it and a value over `MYSTIC_OPENING_MS_MAX` (`6000`) is brought back under it — so no preference, profile or caller can ask for a wave longer than six seconds or shorter than a third of a second. `intuition_liquid_preference.js` carries `openingMs` in the normalised preference, exposes `readIntuitionMysticOpeningMs()`, compares it so a re-published profile does not restart the wave, and bridges `eve:profile-preferences-updated` to `visual.mysticOpeningMs`; the profile model's default and ceiling are read from the same constants.
- `planMysticTiles({ tiles, closing, reducedMotion, openingMs })` still owns the shape and only rescales it: the law's own `durationMs` (`600` ms a turn for the tiles nearest the centre, `800` ms for the outer ones) and its quarter stagger are kept, and every delay and duration is multiplied by the SAME factor `askedMs / naturalTotalMs`. A scale cannot break a ratio, so a six-second menu is the same cascade played slower, never a second animation law. `totalMs` stays `max(delay + duration)` and becomes the requested length when one is asked for; a missing, zero, negative or non-numeric request keeps the law's own length, and `reducedMotion` is never rescaled because it is an accessibility answer rather than a duration. `sampleMysticMotion({ ..., openingMs })` forwards it, and `bevy_ui_flower_runtime.js` passes `readIntuitionMysticOpeningMs()` to BOTH of its `state.motion.sample({...})` calls, so opening and closing remain one clock.
- The Home panel edits that value with the product's own numeric field, never a fourth slider: `createNumericFieldRuntime({ id: 'home_mystic_timing', intentPrefix: 'home.mystic_timing', min: 0.3, max: 6, step: 0.1, initialValue: 2, unit: 's' })` in `bevy_panel_home_runtime.js`, whose node `bevy_panel_home_view.js` mounts directly after the `home_mystic_gap` radio. The box speaks seconds and the profile stores milliseconds (`MYSTIC_OPENING_MS_PER_SECOND` is the one conversion); only the END phase of a gesture persists (`home.choice.set`-equivalent write once per gesture, not per key or per press). `syncMysticTiming()` re-reads the stored value on `load()` and on every profile update, so the displayed box cannot drift from the profile, and `home.mystic_timing.` is in the restricted guest intents so a guest can drive it. The fr/en labels are `eve.user.render_style.mystic_timing.{title,label,unit,decrement,increment}`. No new public API, MCP tool, renderer, canvas or mutation path is introduced.
