...
- Browser recording scope is derived from the PCM already written by its recorder worklet. Tauri polls `audio_get_scope` at no more than 30 Hz, and iOS/AUv3 consume `record_scope` frames published from fixed native buffers. None of these routes opens a second microphone or audio context.
