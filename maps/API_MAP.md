...
- Project selection is transient runtime state read by `project_scene_runtime.js`, marked on disposable `RenderAtom.visual.selected`, and carried by `webgpu_compositor.js` active clips so `project_scene_webgpu_adapter.js` can draw the selected outline as an extra WebGPU layer. It must not be serialized into Atome properties or projected as per-Atome DOM.
...
- Recording persistence and dropped/recorded clip imports must forward the active project id into media Atome creation when they create a source Atome, so audio/video recordings appear on the current project through the canonical commit/render path. Direct audio/video recording facades must also project the committed recording Atome through `project_media_atome_renderer.js`; pending source-less recording Atomes are not Bevy-projected until a renderable source exists. Local WAV recording stores canonical waveform peaks on the Atome properties before Bevy texture generation; imported audio can derive waveform peaks by decoding its normalized source.
