...
- JavaScript module exports: `createDashboardBevyUiRuntime`, `getDashboardBevyUiRuntime`, `openDashboardBevyUiRuntime`, `closeDashboardBevyUiRuntime`, `toggleDashboardBevyUiRuntime`, and `warmupDashboardRuntime`; runtime instances also expose `refresh()` and `warmup()` for explicit dashboard-cache refresh and data-only warmup. `dashboard_workspace_mode.js` exposes the internal neutral scene id `DASHBOARD_WORKSPACE_PROJECT_ID`, mode setters/readers for `dashboard | project | transition`, and neutral host/surface creation.
...
- Entry points: the visible BevyUI main-menu Atome tool opens/closes the Dashboard in active workspaces through `toggleWorkspaceDashboardAndMainMenu({ source: "bevy_ui_main_menu_atome" })`; the private legacy ribbon handle keeps the old `main_handle` source only when the legacy item has explicitly shown that ribbon. Successful authenticated and anonymous workspace openings route through `eVe/intuition/tools/user_workspace_surface_runtime.js`, which ensures the canonical `eve_surface_project` canvas is attached to `project_view___eve_dashboard_workspace__`, runs data-only dashboard warmup only when the runtime is not already warm for `__eve_dashboard_workspace__`, stabilizes the current BevyUI menu API with `window.new_menu_v2.showFully()`, opens `window.eveDashboardBevyUiRuntime` for `{ projectId: "__eve_dashboard_workspace__", dataProjectId: "__eve_dashboard_workspace__" }`, and verifies readiness from Bevy UI runtime diagnostics backed by mounted overlay records. The `boot_workspace` source uses the same neutral Dashboard gate and does not wait for, resolve, or load a user project. Concurrent Dashboard opens share one neutral in-flight promise so boot/auth/anonymous routes cannot double-open the dashboard. Direct `tool.main.home` gateway calls remain the `ui.home.panel` user/Home panel route.
...
- `renderProjectScene(..., { preserveEphemeralRecords: true })` is an internal rendering extension for late project reconciliation; it preserves ephemeral Bevy overlay records (`mol:lane:`, `mol:clip:`, `mol:kf:`, `mol:playhead`) from the current runtime map and preserves Dashboard Bevy UI overlay records only while `window.eveDashboardBevyUiRuntime` is active on that same project. Closed or other-project dashboard overlays are never preserved by project refresh, launch, or late reconciliation. Project media/import stack allocation is owned by `project_scene_stack_runtime.js`; it scans durable project records only, ignores ephemeral overlay records, detects visible Dashboard overlays as an active ceiling, and clamps explicit media/drop positions below that ceiling while preserving order-based stacking among durable project records.
