...
- Entry points: the main Atome handle tap (`button[data-role="eve_intuitionx-handle"]`) opens the Dashboard in active workspaces; successful authenticated and anonymous workspace openings route through `eVe/intuition/tools/user_workspace_surface_runtime.js`, which ensures the canonical `eve_surface_project` canvas is attached to `project_view___eve_dashboard_workspace__`, runs data-only dashboard warmup only when the runtime is not already warm for `__eve_dashboard_workspace__`, stabilizes the existing main ribbon with `window.new_menu_v2.showFully()`, opens `window.eveDashboardRuntime.open({ projectId: "__eve_dashboard_workspace__", dataProjectId: "__eve_dashboard_workspace__" })`, and verifies dashboard Bevy projection readiness directly after open. The `boot_workspace` source uses the same neutral Dashboard gate and does not wait for, resolve, or load a user project. Concurrent Dashboard opens share one neutral in-flight promise so boot/auth/anonymous routes cannot double-open the dashboard. Direct `tool.main.home` gateway calls remain the `ui.home.panel` user/Home panel route.
