...
Viewport sizing note: `resolveRenderSurfaceSize({ preferViewport, previousSize })` keeps both fields internal to the shared structural resize scheduler. Initial and observer-only calls remain host-sized. Structural window, visual-viewport, and orientation signals reconcile the visual viewport, layout viewport, and fullscreen host against the preceding surface size; a source that did not change cannot retain Tauri's initial window dimensions, while a visual-only iOS keyboard contraction remains authoritative.
