...
- Entry points: the main Atome handle tap (`button[data-role="eve_intuitionx-handle"]`) toggles the dashboard in active workspaces; successful authenticated and anonymous workspace openings route through `eVe/intuition/tools/user_workspace_surface_runtime.js`, which starts `window.eveToolBase.loadProjectAtomes(projectId, { staleFirst: true, resolveOnFirstPaint: true })`, requires the canonical `eve_surface_project` canvas, stabilizes the existing main ribbon with `window.new_menu_v2.showFully()`, opens `window.eveDashboardRuntime.open({ projectId })` on that resolved project, and verifies dashboard Bevy projection readiness directly after open; direct `tool.main.home` gateway calls remain the `ui.home.panel` user/Home panel route.
