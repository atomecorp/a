...
- Entry points: the main Atome handle tap (`button[data-role="eve_intuitionx-handle"]`) toggles the dashboard in active workspaces; successful authenticated and anonymous workspace openings route through `eVe/intuition/tools/user_workspace_surface_runtime.js`, which reuses an existing canonical `eve_surface_project` canvas when it is already attached to the requested project, otherwise starts `window.eveToolBase.loadProjectAtomes(projectId, { staleFirst: true, resolveOnFirstPaint: true })`. It warms dashboard entities/textures only when the runtime is not already warm for that `projectId`, stabilizes the existing main ribbon with `window.new_menu_v2.showFully()`, opens `window.eveDashboardRuntime.open({ projectId })` on that resolved project, and verifies dashboard Bevy projection readiness directly after open. The `boot_workspace` source uses the same gate for project first paint, dashboard warmup, and menu stabilization; on refresh/reboot of an already active workspace, boot retries until the current project id is published, then reopens the dashboard once through the same canonical path. Concurrent opens for the same project share one in-flight promise so boot/auth/anonymous routes cannot double-open the dashboard. Direct `tool.main.home` gateway calls remain the `ui.home.panel` user/Home panel route.
