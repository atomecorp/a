...
Additional internal helpers: `eVe/domains/rendering/surface_hit_target_runtime.js` owns pure project-canvas hit-target acceptance when the active project layer sits above the shared Bevy canvas; `eVe/intuition/runtime/project_layer_visibility_runtime.js` owns foreground project-layer visibility after project switches; `eVe/intuition/runtime/tool_genesis_account_record_filter.js` owns filtering of account/current-user state records before project-scene projection.
