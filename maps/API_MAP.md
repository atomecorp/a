...
Choice-palette contract (2026-09-18): tool_definition_ssot.resolveToolChoiceChild resolves what a palette PRESENTS — its owner's selectedChildKey (a value or a canonical-owner reader), otherwise the first descendant whose lock is active; resolveToolActiveChild resolves that locked descendant alone, so the same child both shows and switches off. resolveToolChoiceIcon and shared/tool_presentation.resolveToolPresentation({ activeToolIds }) then expose that child's icon AND label, which is what a palette slot displays (an already-resolved label stays authoritative, so state-dependent labels such as Play -> Stop survive). Without activeToolIds the resolution is unchanged, which is what the Mystic consumes. Main-menu and contextual models share it. View binds its selected key to getProjectViewMode, Mode binds its canonical work-mode owner and the record-action owner publishes its own. Locked tool ids stay with their owners and reach both renderers as published state (setToolLatchedState / eve:tool-state-changed); the main menu subscribes to PROJECT_VIEW_MODE_CHANGED_EVENT and to the lock channel, releasing both listeners on destroy. No renderer-owned choice state is introduced.
