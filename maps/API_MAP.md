...
- JavaScript module exports: `createDashboardRuntime`, `getDashboardRuntime`, `openDashboardRuntime`, `closeDashboardRuntime`, `toggleDashboardRuntime`, `warmupDashboardRuntime`; runtime instances also expose `refresh()`, `warmup()`, and `invalidateCache(...)` for explicit dashboard-cache invalidation.
...
- Entry points: the main Atome handle tap (`button[data-role="eve_intuitionx-handle"]`) toggles the dashboard in active workspaces; successful authenticated and anonymous workspace openings route through `eVe/intuition/tools/user_workspace_surface_runtime.js`, which reuses an existing canonical `eve_surface_project` canvas when it is already attached to the requested project, otherwise starts `window.eveToolBase.loadProjectAtomes(projectId, { staleFirst: true, resolveOnFirstPaint: true })`. It resolves the project from the explicit id first, then the current product project APIs (`window.__currentProject`, `AdoleAPI.projects.getCurrentId()`, `eveToolBase.getCurrentProjectId()`), warms dashboard entities/textures only when the runtime is not already warm for that `projectId`, stabilizes the existing main ribbon with `window.new_menu_v2.showFully()`, opens `window.eveDashboardRuntime.open({ projectId })` on that resolved project, and verifies dashboard Bevy projection readiness directly after open. The `boot_workspace` source uses the same gate for project first paint, dashboard warmup, and menu stabilization; on refresh/reboot of an already active workspace, boot treats a visible current project surface as active workspace evidence, retries until the current project id is published, then reopens the dashboard once through the same canonical path. Concurrent opens for the same project share one in-flight promise so boot/auth/anonymous routes cannot double-open the dashboard. Direct `tool.main.home` gateway calls remain the `ui.home.panel` user/Home panel route.
...
- `dashboard_data_adapters.js` exposes read-only `list(category, { projectId, hydratePreviews, forceCurrentProjectPreview })` and `listMany(categories, { projectId, hydratePreviews, forceCurrentProjectPreview })` for grouped category reads. `generic_record` categories share one explicit-project `state_current(projectId)` read and are split by `source_domain: "eve.dashboard"` plus `category_id`. Calendar lists only dated, non-deleted, unique `CalendarAPI.listEvents({ projectId })` items through `calendar_api.js` so dashboard reads do not initialize the Calendar panel DOM or depend on the implicit current project. Projects include the current `window.__currentProject` when the project list is empty or stale; first paint may request `hydratePreviews:false` so project/contact rows appear without waiting for capture, while async hydration derives disposable `metadata.project_preview_source`, `project_preview_width`, and `project_preview_height` only from persisted preview fields or the shared WebGPU project preview runtime. When `forceCurrentProjectPreview` is true for the current project, persisted preview metadata is ignored and the shared renderer capture path is invoked again. Project preview runtime must capture from renderer-owned records, prefer current Bevy scene state, merge loaded records only for hydration, and propagate `bevy_project_preview_capture_empty`, pending-resource, or skipped-resource errors instead of substituting icons, DOM screenshots, or empty preview URLs. Contacts combine `collectPeopleDirectory()` with local `eve_contacts_local` payloads; local payloads are ordered first, replace matching directory entries by phone/email/source id, and are the only editable contacts, while non-matching directory contacts remain visible after them. Contact titles prefer display/name fields before phone or email, and may carry disposable `metadata.user_face` for Bevy card projection.
...
- It treats a visible current project on the canonical project canvas as an active workspace during reload ordering, so dashboard boot is not blocked when auth state publication lags behind project restoration.
