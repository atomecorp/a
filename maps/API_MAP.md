...
- Live project video track mutation exports are now part of the browser/WASM Bevy API. `atome/renderers/bevy-core/src/types.rs` owns the `AtomeVideoTrack` payload contract with canonical id/source/position/size/layer/opacity fields plus optional `uv_rect` and local transform `scale`, `rotation`, and `origin` fields, and `AtomeVideoTransform` / `AtomeVideoTransformPatch` for position, size, scale, rotation, and origin updates. `AtomeRenderNode` and `AtomeTransformPatch` carry the same transform fields with serde defaults, `AtomeRenderNode` carries optional initial `uv_rect`, and `AtomeResourcePatch` carries tri-state `uv_rect` so resource updates can set, preserve, or clear crop rectangles. Browser projection accepts normalized `uvRect` / `uv_rect` and pixel `sourceRect` / `source_rect` / `cropRect` / `crop_rect` values, converting pixel crop rectangles through natural media size before sending Bevy `uv_rect`; it also forwards Virtual Scene `localTransform.scaleX`, `scaleY`, `rotation`, `originX`, and `originY` to Bevy payloads and transform patches. Video timeline fields are currently normalized and validated on the JavaScript projection/browser decode boundary, then applied to the hidden `HTMLVideoElement` source before the Bevy external-texture import; they are not yet a Rust-owned `AtomeVideoTrack` field. `platforms/web/bevy-renderer/src/exports.rs` exposes `apply_atome_bevy_video_track(track)`, `remove_atome_bevy_video_track(id)`, and `update_atome_bevy_video_transform(id, transform)` by decoding those payloads and queuing real `AtomeRenderOp::VideoTrackApply`, `VideoTrackRemove`, and `VideoTrackTransform` operations; these are not facade exports. `AtomeStylePatch` also carries opacity for Virtual Scene style diffs so browser runtime opacity mutations reach the same WASM style export. Browser Bevy projection accepts only normal/source-over blending; unsupported advanced modes such as `add`, `multiply`, and `screen` fail with `bevy_projection_blend_mode_unsupported:<id>:<mode>` instead of silently falling back to normal. `atome/renderers/bevy-core/src/video_external_texture.rs`, `atome/renderers/bevy-core/src/video_external_web.rs`, and `atome/renderers/bevy-core/assets/shaders/video_external.wgsl` own the current product video import/draw/sampling path. The external-texture component carries the current compositor layer, current per-track opacity, and normalized crop `uv_rect`; source-backed video meshes opt out of automatic 2D batching and rebuild UV coordinates from the canonical crop rectangle, while `render_math.rs` and `render_ops.rs` apply local scale/rotation/origin to the Bevy `Transform` without mutating canonical Atome state. The web RenderApp queues external-video phase items by the extracted layer and binds a per-video opacity uniform so z-index, transform, crop, and opacity changes are represented in the same Bevy canvas route. `atome/renderers/wgpu-web-external-texture/` owns the maintained Web `wgpu` backend fork that adds browser media source descriptors, `GPUDevice.importExternalTexture`, and external-texture bind-group resource mapping for the web renderer Cargo graph. The pure JavaScript `GPUExternalTexture` preview/probe path is a diagnostic reference only and must not be promoted into an eVe project-rendering API or side compositor. `read_atome_bevy_video_backend_capabilities()` is diagnostics-only; schema `atome.bevy.web.video_backend.v6` reports target and live backend `gpu_external_texture_texture_external`, `current_backend_final:true`, `video_track_api_exposed:true`, browser `GPUDevice.importExternalTexture` availability, true Web `wgpu` create/source/resource support from the maintained fork, blocker `none`, true product external-texture import and `texture_external` sampling, and false live RGBA and visible DOM overlay capabilities. `read_atome_bevy_video_copy_diagnostics()` and `reset_atome_bevy_video_copy_diagnostics()` are diagnostics-only counters for legacy copy pressure and skip-reason tracking; they do not describe the active source-backed live video route and do not add a second video-track API.
