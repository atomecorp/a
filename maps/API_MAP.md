...
- Entry points: the main Atome handle tap (`button[data-role="eve_intuitionx-handle"]`) toggles the dashboard in active workspaces; successful authenticated and anonymous workspace openings reveal the existing main ribbon and open the dashboard through `eVe/intuition/tools/user_workspace_surface_runtime.js`; direct `tool.main.home` gateway calls remain the `ui.home.panel` user/Home panel route.
