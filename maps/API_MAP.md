...
- JavaScript module exports: `createDashboardBevyUiRuntime`, `getDashboardBevyUiRuntime`, `openDashboardBevyUiRuntime`, `closeDashboardBevyUiRuntime`, `toggleDashboardBevyUiRuntime`, and `warmupDashboardRuntime`; runtime instances also expose `refresh()` and `warmup()` for explicit dashboard-cache refresh and data-only warmup. The internal `open({ refreshCurrentProjectPreview: true })` option is reserved for an explicit project-to-Dashboard transition: first projection waits only for persisted critical Projects/Contacts data, then the existing controller hydrates non-critical categories and forces the current-project capture/persistence path asynchronously. `post_open_hydration_pending` and `post_open_hydration_error` keep that work observable without blocking Dashboard readiness. `dashboard_workspace_mode.js` exposes the internal neutral scene id `DASHBOARD_WORKSPACE_PROJECT_ID`, mode setters/readers for `dashboard | project | transition`, and neutral host/surface creation.
