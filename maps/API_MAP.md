...
- Recording persistence and dropped/recorded clip imports must forward the active project id into media Atome creation when they create a source Atome, so audio/video recordings appear on the current project through the canonical commit/render path.
