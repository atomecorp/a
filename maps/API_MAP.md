...
- `bevy_panel_home_subject.js`: `loadHomeSubject(id)` reads the person from `collectPeopleDirectory()` (answers `self` for the current user, whose card then reads `loadHomeProfile()` and saves with `persistHomeProfile({ eventSource: 'contact_bevy_panel' })`); `persistHomeSubject()` saves only a local address-book contact through `Squirrel.contacts.updateLocalContact` (identity + `custom_fields`); `homeSubjectAllows(subject, intentType)` refuses every writing intent on a read-only card with `home_subject_read_only`.
