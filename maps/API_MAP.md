...
- Entry points: the main Atome handle tap (`button[data-role="eve_intuitionx-handle"]`) toggles the dashboard in active workspaces; successful authenticated and anonymous workspace openings route through `eVe/intuition/tools/user_workspace_surface_runtime.js`, which requires the project loader to attach the canonical `eve_surface_project` canvas before revealing the existing main ribbon and opening the dashboard; direct `tool.main.home` gateway calls remain the `ui.home.panel` user/Home panel route.
