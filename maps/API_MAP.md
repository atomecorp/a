...
- JavaScript module exports: `createDashboardRuntime`, `getDashboardRuntime`, `openDashboardRuntime`, `closeDashboardRuntime`, `toggleDashboardRuntime`, `warmupDashboardRuntime`; runtime instances also expose `refresh()`, `warmup()`, and `invalidateCache(...)` for explicit dashboard-cache invalidation. `dashboard_workspace_mode.js` exposes the internal neutral scene id `DASHBOARD_WORKSPACE_PROJECT_ID`, mode setters/readers for `dashboard | project | transition`, and neutral host/surface creation.
...
- Entry points: the visible BevyUI main-menu Dashboard grid opens/closes the Dashboard in active workspaces through `toggleWorkspaceDashboardAndMainMenu({ source: "bevy_ui_main_menu_grid" })`; the private legacy ribbon handle keeps the old `main_handle` source only when the legacy item has explicitly shown that ribbon. Successful authenticated and anonymous workspace openings route through `eVe/intuition/tools/user_workspace_surface_runtime.js`, which ensures the canonical `eve_surface_project` canvas is attached to `project_view___eve_dashboard_workspace__`, runs data-only dashboard warmup only when the runtime is not already warm for `__eve_dashboard_workspace__`, stabilizes the current BevyUI menu API with `window.new_menu_v2.showFully()`, opens `window.eveDashboardRuntime.open({ projectId: "__eve_dashboard_workspace__", dataProjectId: "__eve_dashboard_workspace__" })`, and verifies dashboard Bevy projection readiness directly after open. The `boot_workspace` source uses the same neutral Dashboard gate and does not wait for, resolve, or load a user project. Concurrent Dashboard opens share one neutral in-flight promise so boot/auth/anonymous routes cannot double-open the dashboard. Direct `tool.main.home` gateway calls remain the `ui.home.panel` user/Home panel route.
...
- Open/close/render are ephemeral Bevy projection operations over the neutral Dashboard scene by default. Opening includes project-veil, violet reserved-band-fill, and bottom-shadow records below the dashboard foreground, starts Dashboard record opacity at `0`, then fades those records to their resolved visual opacity. Rubrique focus animation is internal runtime state owned by `dashboard_focus_transition.js`: it uses `requestAnimationFrame`, projects two temporary Bevy shape records under cards and header text/icon records, supports expand, collapse, and active-to-active switch phases, and removes those records when the focused color settles. Active-to-active switches keep the previous full-dashboard color while the new source lane fades in before expanding; it does not create DOM, CSS animation, a second canvas, or an alternate renderer path. Closing cancels pending dashboard work, fades Dashboard record opacity to `0`, clears dashboard scene effects, removes every `__eve_dashboard_` record from the scene through prefix reconciliation, and empties runtime `records`/`layout`; a closed Dashboard therefore leaves zero Dashboard Bevy records or nodes in the active surface. Intermediate fade frames call direct opacity style projection (`dashboard_fade_projection.js` -> `updateProjectSceneRecordStyles(...)`) against already-present Bevy records, while the final frame goes through the normal record projection. Warmup is data-only and never builds Dashboard records or writes hidden/offscreen nodes into the scene. `backdrop_blur` remains an internal scene-effect contract for later renderer work but is disabled by default.
- Dashboard render calls keep an internal record cache and use `dashboard_render_scheduler.js` to emit changed records plus removed ids to `updateProjectSceneOverlay(...)`; record removals, record updates, and scene effects are merged into one project-scene projection per logical dashboard render. Layout resolution reuses the environment watcher snapshot and item projection uses `createDashboardItemsForRenderMemo()` so pointermove, wheel, fade, and inertial frames do not repeat environment DOM reads or item dedupe work. Pointermove, wheel, and inertial scroll are coalesced through a cancellable `requestAnimationFrame` scheduler so close cannot be followed by a stale visible dashboard render; open/close fade ownership lives in `dashboard_projection_lifecycle.js` and uses record opacity only, never canvas or host CSS opacity.
...
- `renderProjectScene(..., { preserveEphemeralRecords: true })` is an internal rendering extension for late project reconciliation; it preserves ephemeral Bevy overlay records (`mol:lane:`, `mol:clip:`, `mol:kf:`, `mol:playhead`) from the current runtime map and preserves `__eve_dashboard_` records only while `dashboard_runtime.js` is active on that same project. Closed or other-project dashboard records are never preserved by project refresh, launch, or late reconciliation. Prefix reconciliation through `reconcileProjectSceneRecordsByPrefix(...)` collects dashboard ids from runtime records plus Bevy virtual scenes and removes controlled/orphan ids on close, inactive cleanup, refresh, and project changes. Project media/import stack allocation is owned by `project_scene_stack_runtime.js`; it scans durable project records only, ignores ephemeral overlay records, detects visible Dashboard records as an active ceiling, and clamps explicit media/drop positions below that ceiling while preserving order-based stacking among durable project records.
...
- Project `+` creation delegates to shared Matrix/Adole project helpers and project activation; dashboard does not duplicate Matrix project creation or selection logic. The dashboard close/teardown completes before the new project record is created and activated, and concurrent project-creation clicks share the same in-flight operation. On successful creation, `AdoleAPI.projects.create(...)` returns the generated project id on the top-level result (`id`, `atome_id`, and `project_id`) so Matrix activation can use the known canonical id even when backend commit acknowledgements do not echo it.
...
- Public internal methods: `mountTree({ id, surface, tree })`, `updateTree({ id, patches })`, `unmountTree(id)`, and `readDiagnostics()`.
- WASM exports: `apply_atome_bevy_ui_ops(...)`, `read_atome_bevy_ui_diagnostics()`, and `drain_atome_bevy_ui_events()`.

Boundary status: Squirrel UI definitions remain the canonical source of UI structure and handlers. BevyUI receives normalized disposable trees, hydrates `icon` / `image` sources into WebGPU texture payloads through the shared browser texture resolver before renderer submission, stores the interaction tree in logical CSS pixels, projects the visible product path through non-selectable WebGPU project overlay records, and returns UI intentions such as `activate`, `change`, `input`, `focus`, `blur`, `open`, `close`, and `select`; any durable mutation must still route through existing handlers and then `window.Atome.commit` / `commitBatch` when canonical state changes. Native BevyUI WASM op submission is opt-in; the default product path must not call `apply_atome_bevy_ui_ops(...)` while the visible route is the project overlay. Prehydrated overlay textures must remain attached through Virtual Scene projection and must not be replaced by re-resolving the original source. The runtime must not create a DOM node per component; hidden HTML text editing/accessibility bridges are allowed only for active text-entry cases.
...
Effect model: The menu renders on the shared `eve_surface_project` WebGPU canvas through `window.eveBevyUiRuntime`. From the handedness edge, the visible order is Dashboard grid, passive Atome logo, the canonical `toolbox.children` tools, then the manual legacy item on the opposite edge; right-handed placement reverses the row visually so Dashboard remains closest to the right edge. The active runtime observes window/surface resize and remounts the tree so the menu remains pinned to the canvas bottom edge. Each block is a BevyUI column containing a uniformly sized canonical SVG icon hydrated to the same `lightgray` (`#d3d3d3`) texture tint plus its localized label, with that texture carried to Bevy as `bevyTexture` instead of relying on the original SVG color; the Dashboard grid calls `toggleWorkspaceDashboardAndMainMenu({ source: "bevy_ui_main_menu_grid" })`. The legacy item is the only route that may call the private DOM ribbon `showFully()` / `hideCompletely()` methods; there is no automatic fallback to the DOM ribbon.
...
Anonymous workspace entry: `AdoleAPI.security.ensureAnonymousUser({ force: true })` uses the same atomic backend bootstrap contract with private anonymous credentials. It must not split the flow into `auth.login` followed by `auth.register`, because Dashboard boot, main-handle toggle, and Flower access depend on a single completed anonymous session before opening the neutral Dashboard workspace. A successful Fastify anonymous bootstrap must install a real authenticated token/session, clear stale `window.__SQUIRREL_FASTIFY_AUTH_INVALID__` state through the token owner, and reject user-only responses without credentials as `missing_authenticated_session`; existing valid `me` responses also clear the stale invalid flag. If persisted anonymous credentials are explicitly rejected as invalid, the owner may rotate those disposable anonymous credentials and retry the atomic bootstrap once. Tauri local auth must keep its bcrypt cost aligned with Fastify auth (`10`) so local anonymous bootstrap stays inside the workspace-open budget instead of timing out before the Dashboard/Flower runtime can activate.
