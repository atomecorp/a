...
## OpenAI integration — 2026-09-10

2026-09-10 OpenAI credential persistence correction: The designated account has a server-stored key. Both Web and the Tauri relay successfully list models before and after a canonical Fastify restart, with zero paid requests. Real Home clicks, empty-field focus/blur and page reload preserve the key; inspected pixels show a fixed eight-bullet placeholder and the localized server-saved status. The replacement draft remains empty and no secret is fetched into the UI. Status lookup failure is now unknown, never key absence. Server save no longer requires a local encryption key; legacy cleanup is owned by refresh, while removal clears migration material before server deletion to prevent resurrection. Duplicate i18n entries were removed. The final targeted run passes 56 tests in four files after cleanup. The persistent real UI replay passes twice (initial load/reload), with no page errors. Component/AI/WebSocket guardrails pass. On-disk metadata: vault directories 0700, master/encrypted credential files 0600; existing AES-GCM owner retained. Native transport is verified, not native Home pixels. Persistent runner: tests/probes/openai_key_ui_acceptance.mjs.

IN_PROGRESS (2026-09-10 18:50). Real headed Web acceptance passes 13 checks, including all five levels, durable attachment reload, real brush/rectangle/ellipse mask export and original preservation. Narrow left-handed acceptance passes nine checks. Slider hold arbitration and level-range reset are repaired (25 tests). Cold Dashboard local TTS now loads the existing audio facade/Kira through voice bootstrap: three tests and real ONNX/playback completion pass. Realtime resume waits for old tool completion; late-account progress/usage is rejected. The expanded 156-test run had one outdated capture fixture; it is repaired and 47 provider/lifecycle tests pass, and subsequent transcription/modality metering changes pass 160/160 tests in 16 files. Input transcription is recorded separately; Realtime token estimates require complete modality counts and exclude unpriced fees. Updated signed physical iPhone build/install/boot and Tauri Dashboard pixels remain proven. Latest iPhone launch reports boot_stalled at 9.2 s before eventual final render readiness; startup performance remains to verify. These results do not establish native assistant acceptance. No paid OpenAI call has run; last Web/native credential prechecks report unconfigured. To verify: live duplex/photo/service scenarios, native gestures, dedicated Realtime translation, complete cost accounting and exhaustive capability dispatch. See openai_integration.md.

OpenAI 2026-09-10 20:15 repair checkpoint: real Web input (750 chars), SVG square, PNG generation/apply/reload and explicit rectangle retouch/apply now have provider and pixel evidence. Cold profile/AtomeAI bootstrap fixed; runtime MCP rendering projections excluded; expanded icon/status corrected. 68 targeted tests plus separate 16 lifecycle tests passed (overlap with prior suites). Timed recording controlled coverage passes; real voice/timer, undo/redo, project commands, sharing and remaining services pending. Files content returns provider HTTP 400. Full acceptance remains open; see canonical `eVe/documentations/openai_integration.md`.
...
### 2026-09-10 — Agenda alarms and explicit project commands

Implemented in the existing owners: CalendarAPI now exposes readTime/setAlarm/readAlarms/cancelAlarm, registered as calendar.time.read and calendar.alarm.set/read/cancel through the existing AI and Runtime V2 catalogs. An alarm is stored in its calendar_event, never in another database. Set supports an absolute ISO instant or delay_ms, optional existing event/alarm replacement and an optional Runtime V2 command. Triggering calls the existing MCP gateway with the authenticated principal; sensitive tools still require their normal approval. Default MCP actors now receive timeline.read/write under explicit user authorization; explicitly supplied restricted capability lists remain unchanged.

The Time palette includes Alarm, opening the existing Bevy Calendar editor with an at-start reminder. The editor renders the nearest countdown through shared text nodes; its refresh timer is released on close. CalendarAPI timers survive panel closure, clear on account changes, cap long timeouts, persist firedAt before action dispatch and remove deleted event timers on canonical enumeration. Legacy timer cleanup was removed from the panel owner to keep one scheduling lifecycle owner. No new renderer, scheduler service or dependency was added.

Explicit ui.project.open delegates to activateProjectWorkspace after validating the canonical project record. ui.project.transport delegates project-wide play/pause/stop to the existing surface context and transport, loading canonical records independently of selection. It does not change ui.play selection semantics.

Evidence: focused Vitest alarm permissions, countdown, cancellation, long-delay and single-dispatch tests; existing project context/create contracts; calendar_runtime_contract and calendar_persistence_contract probes; component-reuse, mutation and AI guardrails. These are controlled smoke checks, not a new paid-provider/native/physical-device campaign. Logs: temp/alarm-smoke-tests.log, temp/alarm-final-smoke.log, temp/alarm-last-check.log, temp/alarm-calendar-contract.log. Counts overlap.

**To verify / operational limits:** exact-time delivery is available only while the application runtime is executing. Browser background throttling, suspension/closed application, multi-device exclusive execution, real timed microphone recording and native alarm UI remain unvalidated. Persisted firedAt prevents silent replay after a normal reload; it is not a distributed exactly-once transaction or a guarantee of completion after a crash between persistence and dispatch. Missed deadlines are not silently replayed after restart. Hydration follows existing calendar enumeration. Sensitive scheduled actions can be denied at their deadline and require explicit interactive approval; alarms do not mint approval tokens. Local notifications open the existing agenda event; audio/tools can be specified as MCP commands. No deep validation was run, as requested.

2026-09-10 21:14 — Real Web alarm smoke acceptance passed. The authenticated local test account created a three-second alarm through CalendarAPI. firedAt was recorded 2 ms after its deadline, and the existing agenda panel opened on the event; screenshot inspected. The test event was deleted through CalendarAPI and the panel closed. No new media was launched and no OpenAI call was made. Runner: tests/probes/calendar_alarm_live_acceptance.mjs; evidence: temp/calendar-alarm-live.json and temp/calendar-alarm-live.png. This single foreground run is not a latency benchmark, suspended-runtime proof or scheduled-media acceptance.

### 2026-09-10 — Cross-runtime assistant discovery and provider failure

Confirmed by controlled regression: resolveTool uses canonical built-ins but listTools previously returned only persisted registry records. An empty persisted catalog therefore hid ui.draw.edit and ui.ai.image.generate from MCP despite executable built-ins. The common built-in resolver now projects the same definitions into listing, merges custom stored tools and retains explicit disabling; no OpenAI-only catalog was added. Persistence/bootstrap is still awaited by listTools; stalled persistence remains a separate unvalidated failure mode.

Confirmed voice routing defect: provider resolution failure without providerId previously selected the legacy local voice controller, speaking its built-in greeting. bindVoiceAssistantSession now rejects any explicit failed provider resolution. Existing local-voice unit fixtures now select their local provider explicitly instead of depending on that silent failure path. Eleven targeted tests pass (temp/provider-native-regression.log); empty-catalog regression failed before the change (temp/native-tool-discovery-before.log).

Live transport source comparison: both localhost servers 3000/3001 serve source containing the SVG instruction and image definition. This proves server source availability, not native WebView execution or refresh. Safari's actual Atome window was observed at localhost:3001, distinct from the Chromium acceptance origin 127.0.0.1:3001. Browser origin/session isolation is an observation, not proof of a missing key. Safe credential.status/provider diagnostics were attempted through Safari's inspector but no result was obtained; computer-use access ended with procNotFound. No credential was extracted or copied and no cache was cleared.

**To verify:** actual Safari authenticated account, active provider and server credential.status; actual Tauri tool enumeration and native SVG/image creation; iOS execution. Earlier Chromium success is not Safari or Tauri acceptance. Existing explicit image-reference and retouch paths remain in assistant_media_session.js; no new native retouch acceptance is claimed.
...
2026-09-10: Fastify-to-Tauri media repair: inbound authenticated asset download/recovery added to existing native sync owners; RenderAtom media-owner override corrected. 43 native and 35 JS tests pass. Exact reported PNG recovered natively with matching SHA-256 and HTTP 200 on both backends. Native pixels/iOS/live multi-user sharing remain unvalidated. Evidence: temp/inbound-media-live-proof.json, temp/inbound-media-tests.log, temp/media-owner-check.log.
