...
const A = (() => {
    // Store Atome instance global by ID
    const atomeRegistry = {};
    ...
    const baseStyles = {
        ...
        display: 'block',
        position: 'absolute', // position absolute by default
        lineHeight: 'normal',
        fontSize: 'inherit',
        fontWeight: 'inherit',
        color: 'inherit',
    ...
    // Predefined handlers for specific keys
    const handlers = {
        id: (el, v, _, __, instance) => {
            el.id = v;
            // Register the instance in the global registry for future references
            if (v) {
        ...
        markup: (el, v, instance) => {
            // If markup is specified , create new element base on the specified type
            if (v && typeof v === 'string') {
                const newEl = document.createElement(v);
                //Copy all attributes and styles of ancien elements
                Array.from(el.attributes).forEach(attr => {
                    newEl.setAttribute(attr.name, attr.value);
                });
                newEl.style.cssText = el.style.cssText;
                // Replace element in instance
                instance._element = newEl;
                return newEl; // Important : return new element
            }
            return el;
        },
        ...
        center: (el, v) => {
            if (v) {
                // Center horizontally while maintaining absolute positioning
                el.style.left = '50%';
                el.style.transform = 'translateX(-50%)';
                // if we want vertical center
                // el.style.top = '50%';
        ...
        shadow: (el, v) => {
            ...
            const inset = v.invert ? 'inset ' : ''; // Add handling for the inset option
        ...
        // Controls whether default styles should be applied
        reset: (el, v) => {
            // If reset is false, not apply the base styles
            if (v === false) return;
        ...
        //  Allow setting a relative position instead of absolute
        position: (el, v) => {
            el.style.position = v;
        },
        // origin management
        origin: (el, v) => {
            ...
            //Store the origin in the element data for reference
            el.dataset.origin = JSON.stringify(v);

            // Apply position adjustments if necessary
            // Note: this would be better handled with a complete positioning system
        },
        // Overflow management
        overflow: (el, v) => {
            el.style.overflow = v;
        },
        // Management of fastened (attached) objects
        fasten: (el, v, _, __, instance) => {
            if (Array.isArray(v)) {
                el.dataset.fasten = v.join(',');
                // Store child IDs in the instance
                instance._fastened = v;
            }
        },
        // NEW - Child element management
        ...
        category: (el,v) => {
            if (v && typeof v === 'object'){
                ...
                el.style.transform = `all ${items}s ${delay}s`;
                if(items && items.length > 0 ){
                    el.dataset.categoryItems = JSON.stringify(items);
                    // Drop down list 
                    const  DropDownContainer = document.createElement('div');
                    DropDownContainer.className = 'a-category-dropdown';
