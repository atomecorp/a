...
//     var fastifyApiCheckInterval = null;

//     // Fastify ws/api keep-alive (recipient console-message delivery)
//     var fastifyApiReconnectTimer = null;
//     var fastifyApiReconnectAttempts = 0;
//     var fastifyApiAuthFailedToken = null;

//     // ═══════════════════════════════════════════════════════════════════════════
//     // SERVER CONFIG (external)
//     // ═══════════════════════════════════════════════════════════════════════════
//     // Loaded from `/server_config.json` (served from project root by the server).
...
//         var localPort = window.__ATOME_LOCAL_HTTP_PORT__ || 3000;
//         var configUrl = isTauri ? ('http://127.0.0.1:' + localPort + '/server_config.json') : 'server_config.json';

//         serverConfigPromise = fetch(configUrl, { cache: 'no-store' })
//             .then(function (res) {
...
//         var cfg = getFastifyConfig();
//         if (!cfg || !cfg.host || !cfg.port) return null;
//         // Prefer wss when current page is https
...
//         try {
//             // In local dev, prefer the local token (Tauri/local auth flow).
//             // Fallback to cloud/legacy keys for older sessions.
//             if (isLocalDev) {
//                 return (
...
//     function connectFastifyApiWsIfPossible() {
//         if (!serverConfigLoaded) {
//             loadServerConfigOnce().then(connectFastifyApiWsIfPossible);
//             return;
//         }

//         var cfg = getFastifyConfig();
//         var wsBase = getFastifyWsBase();
//         var apiWsPath = cfg && typeof cfg.apiWsPath === 'string' ? cfg.apiWsPath : null;
//         var token = getFastifyToken();

//         if (!wsBase || !apiWsPath) return;

//         // In production we require a token to connect ws/api.
//         // In local dev we allow connecting without auth (useful for debug broadcast).
//         if (!token && !isLocalDev) return;

//         var shouldAttemptAuth = !!token && !(fastifyApiAuthFailedToken && fastifyApiAuthFailedToken === token);

//         // Avoid noisy connection attempts when Fastify is offline.
//         // Use the existing silent HTTP ping to determine availability.
//         if (fastifyAvailable !== true) {
//             checkFastifyViaHttp().then(function (ok) {
//                 if (ok) connectFastifyApiWsIfPossible();
//             });
//             return;
//         }

//         if (fastifyApiWs && (fastifyApiWs.readyState === WebSocket.OPEN || fastifyApiWs.readyState === WebSocket.CONNECTING)) return;

//         try {
//             fastifyApiWs = new WebSocket(wsBase + apiWsPath);

//             fastifyApiWs.onopen = function () {
//                 fastifyApiReconnectAttempts = 0;
//                 if (fastifyApiReconnectTimer) {
//                     try { clearTimeout(fastifyApiReconnectTimer); } catch (_) { }
//                     fastifyApiReconnectTimer = null;
//                 }
//                 if (shouldAttemptAuth) {
//                     try {
//                         fastifyApiWs.send(JSON.stringify({
//                             type: 'auth',
//                             action: 'me',
//                             requestId: 'console_me_' + Date.now(),
//                             token: token
//                         }));
//                     } catch (_) { }
//                 }
...
//             fastifyApiWs.onmessage = function (evt) {
//                 try {
//                     var msg = JSON.parse(evt.data);
//                     if (msg && msg.type === 'console-message') {
//                         console.log('[Fastify Console Message]', msg);
//                         return;
//                     }
//                     if (msg && msg.type === 'server-broadcast') {
//                         try {
//                             console.log(msg.message);
//                         } catch (_) { }
//                         return;
//                     }
//                     if (msg && msg.type === 'auth-response') {
//                         if (msg.success === false) {
//                             try {
//                                 fastifyApiAuthFailedToken = token;
//                                 console.warn('[Fastify ws/api] Auth failed; waiting for a new token', msg.error || 'auth-failed');
//                             } catch (_) { }
//                             // Keep the socket open in local dev so it can still receive debug broadcasts.
//                         } else {
//                             fastifyApiAuthFailedToken = null;
//                         }
//                     }
//                 } catch (_) { }
//             };

//             fastifyApiWs.onerror = function () {
//                 // Avoid throwing noisy errors; onclose will handle reconnect.
//             };

//             fastifyApiWs.onclose = function () {
//                 fastifyApiWs = null;

//                 // Keep ws/api connected when possible so targeted messages can be delivered.
//                 var currentToken = getFastifyToken();
//                 if (!currentToken) {
//                     fastifyApiReconnectAttempts = 0;
//                     return;
//                 }
//                 if (fastifyApiAuthFailedToken && fastifyApiAuthFailedToken === currentToken) return;
//                 if (fastifyApiReconnectTimer) return;

//                 var delay = Math.min(30000, 750 * Math.pow(2, fastifyApiReconnectAttempts));
//                 fastifyApiReconnectAttempts += 1;
//                 fastifyApiReconnectTimer = setTimeout(function () {
//                     fastifyApiReconnectTimer = null;
//                     try { connectFastifyApiWsIfPossible(); } catch (_) { }
//                 }, delay);
//             };
//         } catch (_) {
//             fastifyApiWs = null;
...
//                     // Fastify is online: connect ws/api (for console-message) if token exists.
//                     try { connectFastifyApiWsIfPossible(); } catch (_) { }
//                     resolve(true);
//                 })
//                 .catch(function () {
...
//     // Expose Fastify availability to other modules (e.g. kickstart) so they can avoid noisy probes.
//     try {
//         window._checkFastifyAvailable = function () { return fastifyAvailable; };
//     } catch (_) { }
...
//     // In non-Tauri runtime: proactively connect ws/api once when a token exists.
//     // This is required for targeted recipient-only messages (console-message) to work.
//     if (!isTauriRuntime() && isLocalDev) {
//         var tryConnectFastifyApiWs = function () {
//             try {
//                 if (getFastifyToken()) connectFastifyApiWsIfPossible();
//             } catch (_) { }
//         };
//         setTimeout(tryConnectFastifyApiWs, 0);
//         window.addEventListener('focus', tryConnectFastifyApiWs);
//         window.addEventListener('storage', function (e) {
//             try {
//                 if (!e || (e.key !== 'local_auth_token' && e.key !== 'cloud_auth_token' && e.key !== 'auth_token')) return;
//                 fastifyApiAuthFailedToken = null;
//                 tryConnectFastifyApiWs();
//             } catch (_) { }
//         });
//     }
