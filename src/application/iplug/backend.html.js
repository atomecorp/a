// DEPRECATED — WebAudio fallback backend
// Superseded by backend.kira.js (WASM Kira) for browser playback.
// Retained only as last-resort fallback when WASM Kira module fails to load.
// Not sample-accurate; OK for UI prototyping/offline demonstrations only.
...
(function () {
  ...
  function create_clip(opts) {
    const id = opts.id;
    const clip = {
      id, mode: opts.mode || 'preload', url: opts.path_or_bookmark, buffer: null,
  ...
  const api = {
    dispatch_batch, create_clip, destroy_clip, play, stop, stop_clip, jump, set_param, map_midi,
