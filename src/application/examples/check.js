 ...
 */
async function openUserSelectorForSharing() {
  // Determine what to share
  let shareTarget = null;
  let shareDescription = '';

  if (selectedAtomeId && selectedVisualAtome) {
    // Share selected atome
    shareTarget = { type: 'atome', id: selectedAtomeId };
    shareDescription = 'atome ' + selectedAtomeId.substring(0, 8) + '...';
  } else if (selectedProjectForSharing) {
    // Share selected project
    shareTarget = { type: 'project', id: selectedProjectForSharing };
    const projectName = localStorage.getItem('selected_project_name') || 'project';
    shareDescription = 'project "' + projectName + '"';
  } else {
    puts('❌ No atome or project selected for sharing');
    return;
  }

  puts('🔄 Opening user selector for sharing ' + shareDescription);
  ...
  if (users.length === 0) {
    puts('❌ No users found for sharing');
    return;
  }

  const existingSelector = grab('share_user_selector_overlay');
  if (existingSelector) existingSelector.remove();

  const overlay = $('div', {
    id: 'share_user_selector_overlay',
    parent: intuitionContainer,
    css: {
      position: 'fixed',
      top: '0',
  ...
  $('div', {
    ...
    text: 'Select a user to share with (full permissions)'
  });
  ...
  users.forEach((user, index) => {
    ...
    if (phone === 'No phone') return; // Skip users without phone

    const userDiv = $('div', {
      id: 'share_user_item_' + index,
      parent: modal,
      css: {
        padding: '15px',
        margin: '8px 0',
      ...
      onClick: async () => {
        overlay.remove();
        await shareWithUser(phone, shareTarget, shareDescription);
      }
    });
 ...
 */
async function shareWithUser(phoneNumber, shareTarget, shareDescription) {
  puts('🔄 Sharing ' + shareDescription + ' with user: ' + phoneNumber);
  ...
  // Default to real-time collaboration
  const sharingMode = 'real-time';

  try {
    const result = await share_atome(
      phoneNumber,
      shareTarget.id,
      sharePermissions,
      sharingMode
    ...
    if (result.tauri.success || result.fastify.success) {
      puts('✅ Successfully shared ' + shareDescription + ' with ' + phoneNumber);
      puts('   Permissions: full access (read, alter, delete, create)');
      puts('   Mode: real-time collaboration');
    } else {
      puts('❌ Failed to share ' + shareDescription);
      puts('   Tauri error: ' + (result.tauri.error || 'none'));
      puts('   Fastify error: ' + (result.fastify.error || 'none'));
    }
  } catch (error) {
    puts('❌ Sharing error: ' + error.message);
