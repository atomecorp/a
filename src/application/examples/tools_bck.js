...
const Inntuition_theme = {
  "light": {
	...
	"icon-top": "45%",
  "icon-left": "33%",
  "icon-centered-top": "33%",
  "icon-centered-left": "33%",
	"icon-width": "16%",
	"icon-height": "16%",
 

	"item-shadow": "0px 0px 5px rgba(0,0,0,0.69)",
	// "tool-icon-size": "120px",
  ...
  "particle_value_setter_color": "#656565ff", // default to tool-bg-active
	...
	// Particle mini button positioning (unscaled reference values)
	"particle_value_setter_top": "50%",
	"particle_value_setter_left": "50%",
	"particle_value_setter_transform": "translate(-50%, -50%)"
	,
	// Optional overrides for particle mini input positioning (relative to parent bottom/left)
	// Leave blank to keep default centering logic.
	"particle_input_value_bottom": "",
	"particle_input_value_left": "",
	// Generic element positioning overrides
	"label-top": "0px",
	"label-left": "0px",
	"value-top": "",
	"value-left": "0px",
...
function _updateIntuitionDomScale() {
  ...
  selectorWrap.style.fontSize = theme['global-label-font-size'];
	  ...
	  if (firstChild) {
		try {
		  firstChild.style.fontSize = theme['global-label-font-size'];
	  ...
	  try {
		selectorWrap.querySelectorAll('*').forEach(n => {
		  ...
		  n.style.fontSize = theme['global-label-font-size'];
	...
	if (miniBtn) {
	  ...
	  if (isParticle) {
		// Use themed values directly so they aren't overridden.
		miniBtn.style.width  = theme['particle_value_setter_width']  || '25px';
		miniBtn.style.height = theme['particle_value_setter_height'] || '6px';
		const bottomOverride = theme['particle_input_value_bottom'];
		const leftOverride   = theme['particle_input_value_left'];
		if (bottomOverride) {
		  // Position from bottom override: switch to bottom-based positioning
		  miniBtn.style.bottom = bottomOverride;
		  miniBtn.style.top = '';
		  miniBtn.style.transform = 'translateX(-50%)';
		} else {
		  // Default centered vertical positioning
		  miniBtn.style.top = theme['particle_value_setter_top'] || '50%';
		  miniBtn.style.bottom = '';
		  miniBtn.style.transform = theme['particle_value_setter_transform'] || 'translate(-50%, -50%)';
		}
		miniBtn.style.left = leftOverride || theme['particle_value_setter_left'] || '50%';
	  } else {
  ...
  iconContainer.dataset.intuitionIcon = '1';
	...
	if (el.getAttribute('data-kind') === 'particle') {
	  ...
	  if (mb) {
		...
		const valueEl = el.querySelector('[id$="_value"]');
		// If theme forces bottom positioning for selector, skip auto top-based stacking for selector
		const forceSelectorBottom = Inntuition_theme.light['selector-bottom'];
		if (selectorEl || valueEl) {
		  try {
			...
			if (!forceSelectorBottom && valueEl && selectorEl) {
			  // Side-by-side layout
			...
			} else if (!forceSelectorBottom && valueEl) {
			  const valueH = parseInt(valueEl.style.height) || Math.round(18 * scale);
			  valueEl.style.position = 'absolute';
			  valueEl.style.bottom = '';
			  valueEl.style.top = rowTop + 'px';
			} else if (!forceSelectorBottom && selectorEl) {
	...
	// Final pass: reapply generic positioning overrides (label/value/selector) if explicitly set
	try {
	  const th = Inntuition_theme.light;
	  const lbl = el.querySelector('.intuition-label');
	  if (lbl) {
		if (th['label-top']) lbl.style.top = th['label-top'];
		if (th['label-left']) lbl.style.left = th['label-left'];
	  }
	  const valN = el.querySelector('[id$="_value"]');
	  if (valN && th['value-top']) { valN.style.top = th['value-top']; valN.style.bottom = ''; }
	  if (valN && th['value-left']) { valN.style.left = th['value-left']; }
	  const selN = el.querySelector('[id$="_selector"]');
	  if (selN) {
		if (th['selector-bottom']) { selN.style.bottom = th['selector-bottom']; selN.style.top = ''; }
		else if (th['selector-top']) { selN.style.top = th['selector-top']; selN.style.bottom = ''; }
		if (th['selector-left']) selN.style.left = th['selector-left'];
	  }
	  // Particle input overrides (bottom/left) if specified
	  const particleInput = el.querySelector('.partcle_input_value_setter');
	  if (particleInput && (th['particle_input_value_bottom'] || th['particle_input_value_left'])) {
		if (th['particle_input_value_bottom']) { particleInput.style.bottom = th['particle_input_value_bottom']; particleInput.style.top = ''; particleInput.style.transform = 'translateX(-50%)'; }
		if (th['particle_input_value_left']) { particleInput.style.left = th['particle_input_value_left']; }
	  }
	} catch(e) {}
  });
...
function intuitionCommon(cfg) {
  ...
  fontSize: Inntuition_theme[theme]["global-label-font-size"],
  ...
  try {
	...
	// Wrap label text in its own absolutely positioned container to prevent layout shifts from toggle/button elements
	try {
		if (label) {
			// Capture original text then clear parent textContent
			const original = el.textContent;
			while (el.firstChild) el.removeChild(el.firstChild);
			const lbl = document.createElement('div');
			lbl.className = 'intuition-label';
			lbl.dataset.originalLabel = original;
			...
			lbl.style.lineHeight = '18px';
			lbl.style.pointerEvents = 'none';
			lbl.style.textAlign = 'center';
			lbl.style.fontFamily = 'Roboto';
			lbl.style.fontSize = Inntuition_theme[theme]['global-label-font-size'];
			lbl.style.color = Inntuition_theme[theme]['tool-text'];
			el.appendChild(lbl);
		}
	} catch(e) {}
 ...
 if (icon) {
  ...
  if (Array.isArray(cfg.selector) && cfg.selector.length > 0) {
	...
	const ddWrap = $('div', {
	  ...
	  css: {
		position: 'absolute',
		bottom: (Inntuition_theme[theme]['selector-bottom'] || '0px'),
		left: (Inntuition_theme[theme]['selector-left'] || '0px'),
	width: '100%',
  height: '14px',
  overflow: 'visible'
	  }
  ...
  class: 'partcle_input_value_setter',
	css: {
		position: 'absolute',
		left: (isParticle ? (Inntuition_theme[theme]["particle_input_value_left"] || Inntuition_theme[theme]["particle_value_setter_left"] || '50%') : '50%'),
	// If bottom override provided, use bottom-based positioning; else default centered vertical.
	...(isParticle && Inntuition_theme[theme]["particle_input_value_bottom"] ? {
		bottom: Inntuition_theme[theme]["particle_input_value_bottom"],
		top: undefined,
		transform: 'translateX(-50%)'
	} : {
		top: (isParticle ? (Inntuition_theme[theme]["particle_value_setter_top"] || '50%') : undefined),
		bottom: (isParticle ? undefined : undefined),
		transform: (isParticle ? (Inntuition_theme[theme]["particle_value_setter_transform"] || 'translate(-50%, -50%)') : 'translateX(-50%)')
	}),
  ...
  if (button) {
    ...
    position: 'absolute',
	  css: {
		left: Inntuition_theme[theme]['toggle-btn-left'],
		top: Inntuition_theme[theme]['toggle-btn-top'],
