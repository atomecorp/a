...
```text
node --test \
  tests/database/adole.particle_history_invariants.probe.mjs \
  tests/database/adole.event_projection_invariants.probe.mjs \
  tests/database/adole_history_transactions.probe.mjs \
  tests/database/adole_restart_safe_interactions.probe.mjs \
  tests/eve/adole_commit_boundary.probe.mjs
```
...
|---|---|---|
| GV-T01–GV-T04 | `tests/server/atome_property_security.probe.mjs`; native `property_commit_security_tests` | Green: owner/authorized writes pass; denied and mixed writes leave no event, state, version, or queue side effect |
| GV-T05–GV-T06 | `tests/server/atome_property_security.probe.mjs`; native read-projection tests | Green: current state, capabilities and event/history reads expose only readable keys |
| GV-T07–GV-T08 | `tests/server/atome_property_security.probe.mjs`; `tests/server/granularity_reconnect_projection.probe.mjs`; native remote projection tests | Green: exact/global rows remain distinct and revocation removes the exact scope immediately |
| GV-T09–GV-T10 | `tests/server/atome_property_security.probe.mjs`; native malformed-condition test | Green: malformed/unknown conditions fail closed; a user condition changing true→false is re-evaluated for write, read, history and sync |
| GV-T11–GV-T12 | `tests/server/granularity_protocol_defects.probe.mjs`; `tests/server/atome_property_security.probe.mjs` | Green: recipient-specific live patches omit denied keys and suppress empty projections |
| GV-T13–GV-T14 | `tests/server/granularity_reconnect_projection.probe.mjs`; native projection revocation test | Green: reconnect/backlog reads re-authorize current scope and revoked queued data is absent |
| GV-T15–GV-T18 | `tests/server/granularity_lifecycle_contract.probe.mjs` | Green: versioned property delete, restore, unrelated-property preservation and grouped undo/redo use canonical events |
| GV-T19–GV-T21 | `tests/server/granularity_resilience.probe.mjs` | Green: independent revisions survive, stale same-key edits return `property_version_conflict`, and injected queue failure rolls the whole transaction back |
| GV-T22–GV-T24 | `tests/server/granularity_consumer_projection.probe.mjs` | Green: custom and collection-valued keys use the same ACL/history lifecycle; fabricated nested widening is denied |
| GV-T23 | `tests/server/granularity_consumer_projection.probe.mjs`; `tests/server/conditions_query_authority.probe.mjs` | Green: search, discovery and export contain no denied key, value, inverse metadata, snippet, or count leak |
| GV-T25 | `tests/server/atome_persistence_boundary.probe.mjs` | Green: retained mutation/read routes converge on the canonical event/projection owners and retired HTTP persistence routes remain absent |
| GV-T26 | `tests/server/granularity_lifecycle_contract.probe.mjs`; native remote projection/restart tests; isolated inbound probe | Green at storage/transport level: replay and restart reconstruct the same authorized durable projection |
| GV-T27 | `tests/server/granularity_protocol_defects.probe.mjs`; native recipient-isolation test | Green: sender echo is excluded and distinct owner/session/recipient projections remain isolated |
| GV-T28 | `tests/server/atome_property_security.probe.mjs`; native malformed-condition test | Green: evaluator failure/unknown input denies without protected-data disclosure |
...
- `node --test tests/server/granularity_lan_config.probe.mjs`: **2/2 passed**;
