...
Version: 2026-05-29  
Status: Strict implementation prompt — reinforced execution contract  
...
The implementation agent must treat this prompt as a strict execution contract, not as a suggestion list.

Mandatory execution rules:

1. Do not remove, merge, reorder, rename, weaken, or skip any numbered step from this prompt.
2. Do not replace the requested architecture with a smaller, easier, legacy-compatible, DOM-first, canvas-per-Atome, or fallback-based architecture.
3. Do not implement only the easy media types and leave text, matrix previews, hit-testing, cache invalidation, tests, or map updates for later.
4. Do not declare a step complete unless every acceptance criterion listed for that step is satisfied by inspected code, modified code, and executed tests when tests are available.
5. Do not mark a requirement as “not applicable” unless the codebase evidence proves it is out of scope for the current repository. The evidence must be reported.
6. Do not continue to the next step when a blocking requirement is impossible to satisfy. Stop, report the blocker, cite the exact rule or missing dependency, and propose the smallest compliant resolution.
7. Do not silently downscope the task because of complexity, uncertainty, missing tests, missing documentation, or large refactor size.
8. Do not create placeholder implementations, TODO-only implementations, fake tests, mock-only validation, or comments that pretend the architecture is implemented.
9. Do not preserve the old and new rendering paths in parallel unless this prompt explicitly allows a temporary transition and the removal/validation plan is implemented in the same task scope.
10. Do not modify this prompt, its step list, its acceptance criteria, or its reporting format while executing it.

Allowed refusal is limited to cases where execution would violate `.codex/AGENTS.md`, repository rules, tool safety, or a higher-priority instruction. In that case the agent must:

```text
Execution stopped.
Blocking rule: <exact rule or higher-priority constraint>
Blocked step: <step number and title>
Reason: <precise reason>
Compliant alternative: <smallest possible compliant next action>
Completed percentage: <last validated percentage>
Remaining steps: <full remaining list>
```

A generic refusal, incomplete execution, partial substitution, or simplified alternative is not acceptable.

### Progress calculation rule

Progress must be calculated from validated steps only.

Use this formula:

```text
progress = floor((validated_completed_steps / 18) * 100)
```

A step counts as validated only when:

- its files were inspected;
- its implementation changes, if any, were made through the canonical architecture;
- its acceptance criteria were checked;
- its required tests were run, or the absence of runnable tests was explicitly justified;
- its map/documentation impact was checked;
- its remaining-step report was printed.

Do not count exploration, planning, partial edits, failed tests, or unvalidated code as progress.

### Completion lock

The implementation is not complete until all of the following are true:

- steps 1 through 18 are validated in order;
- the final checklist is satisfied;
- DOM budget tests pass;
- renderer tests pass or missing renderer tests are created and then pass;
- no per-Atome visible DOM or canvas path remains on the main rendering route;
- hidden text DOM is centralized and bounded;
- matrix previews use render targets or shared renderer output, not cloned DOM;
- architecture maps are updated when structure or ownership changed;
- the final report lists every modified file, every test command, every skipped command with reason, and every remaining risk.
