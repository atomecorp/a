...
- `src/application/eVe/core/media_engine/molecule.webgpu.js` owns the WebGPU renderer used by molecule/media layers.
- `src/application/eVe/intuition/tools/core/mtrax_renderer_webgpu_adapter.js` and related MTraX renderer modules provide a WebGPU-backed rendering path for timeline/media preview flows.
- `src/application/eVe/domains/media/preview/webgpu_video_preview_renderer.js` provides WebGPU preview rendering for video preview.
- Some snapshot, poster, and thumbnail paths use temporary 2D canvas extraction.

HTML/DOM-based project areas still present:

- `src/application/eVe/intuition/tools/project_bootstrap.js` creates `project_view_*` as fixed HTML `div` project layers.
- `src/application/eVe/intuition/tools/project_drop.js` creates and moves projected tool/atome hosts as DOM elements with `data-atome-id`.
- `src/application/eVe/core/atome_events/drag_runtime.js` binds pointer drag directly to DOM atome hosts.
- `src/application/eVe/core/atome_events/project_layer_runtime.js` performs lasso selection against DOM nodes.
- `src/application/eVe/intuition/eVeIntuition.js` contains diagnostics and workflows that inspect `[data-atome-id]` DOM hosts directly.
