...
## 10. Outil Couleur dans le rail du fond de projet (25/09, fait)

- Taxonomie : `couleur` ajoutée en fin de `menus.sidebar.objects.project` (dès débutant), dans la même composition **sidebar** `project` que §9 ; aucune entrée Mystic, aucune nouvelle commande — le panneau `couleur` et `tool.main.couleur` existaient déjà pour les atomes.
- Toggle (déjà en place, vérifié tel quel) : `eVe/core/atome_events/project_layer_routing.js#applyBackgroundSelection` remplace la sélection quand le projet n'est pas sélectionné et bascule (`toggle`) quand il l'est. Toucher le fond sélectionne le projet et monte le rail ; le retoucher alors qu'il est la seule sélection le désélectionne et démonte le rail.
- Couleur canonique : `domains/rendering/user_surface_background_texture_runtime.js` écoute `atome:changed`, lit le projet par `window.Atome.getStateCurrent()` (lecture seule, pas de miroir DOM), prend `background` puis `backgroundColor` puis `bg`, convertit par le nouvel export `readSurfaceBackgroundRgba()` (`domains/rendering/surface_background_defaults.js`, alpha `rgba()` désormais lu et borné à 0..1) et republie `signature: project-color-surface-background:<projectId>:<r>:<g>:<b>:<a>` (`mode: color`, sans texture ni `sourceUrl`). Sans couleur lisible, le patch reste `project-default-surface-background` ; la projection est limitée au mode `project` et au projet encore courant après la lecture asynchrone.
- Sonde app réelle : 9/10. Toucher le fond → rail contenant Couleur → panneau avec 12 pastilles → clic réel sur `color_swatch_0_2` → `background` = `rgba(244, 67, 54, 1.00)` sur l'atome projet et signature `project-color-surface-background:…:0.9568627450980393:0.2627450980392157:0.21176470588235294:1`. Seul échec : la lecture pixel, car la voie navigateur ne lance pas le module WASM de l'app ; la preuve pixel appartient à Tauri et n'a pas été exécutée.
