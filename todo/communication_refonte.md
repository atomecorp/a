# Refonte Communication — rebrancher la boîte de réception (ex-outil Aide)

Le 23 sept. 2026, l'outil Aide est redevenu une simple rubrique d'aide. Toute la partie notifications a été **mise de côté sans rien supprimer**. Elle attend la refonte de Communication.

## Modules mis de côté (non branchés)

- `eVe/intuition/tools/communication_inbox_model.js` : règles pures (catégories, non lus, badge, navigation, swipe, présentation automatique, préférences DND/filtres).
- `eVe/intuition/tools/communication_inbox_view.js` : nœuds du ruban (bandeau, badge, barre DND) et du panneau (liste, lecture, confirmation, filtres, actions de secours).
- `eVe/intuition/tools/communication_inbox_rail.js` : définitions du rail (non lu, à traiter, urgent, retirer / archives, filtres, ne pas déranger) et entrée en cible virtuelle.
- `eVe/intuition/tools/communication_inbox_runtime.js` : `createCommunicationInbox(comm, { toolId })`, qui fabrique l'inbox. L'outil hôte vaut par défaut `tool.main.communicate`.
- i18n : `eVe/i18n/languages_{fr,en}_comm_inbox.js` (`eve.comm.inbox.*`).
- Accent du rail : famille `inbox` dans `eVe/elements/skin/tool_skin.js`.

## Pour rebrancher

- [ ] Dans `communication.js`, appeler `createCommunicationInbox({ readNotifications: () => commState.notifications, subscribe: subscribeCommNotifications, markRead: markNotificationRead, patch: patchNotification, remove: removeNotificationStackItem, sendReply: sendNotificationReply, openArchives: () => openCommPanel({ view: 'archives' }), resolveCurrentUserInfo })`. C'est l'ancien appel `installHelpTool`. `removeNotificationStackItem` est déjà déstructuré, et `patchNotification` / `sendNotificationReply` existent toujours.
- [ ] Décider qui porte le bandeau : l'outil Communication lui-même, ou un nouvel outil. Si c'est Communication, retirer `communication_tool_unread_runtime.js` (restauré en attendant) pour éviter deux bandeaux sur le même outil.
- [ ] **Rail** : `atome_contextual_edit_runtime.js` (`definitions()`) ajoute les outils naturels (enregistrer, relire…) et le choix de dépôt à **toute** cible virtuelle. C'est la pollution qu'on voyait dans l'Aide. Ajouter une option `ownToolsOnly` à `enterVirtual` et l'utiliser depuis `communication_inbox_rail.js`.
- [ ] Remettre un point d'entrée « ne pas déranger » (c'était l'appui long sur l'outil Aide).
- [ ] Préférences : `profile.preferences.notifications` (`do_not_disturb`, `hidden_kinds`) n'a été ni effacé ni migré.

## Ancienne spécification (outil Help notifications), pour référence
