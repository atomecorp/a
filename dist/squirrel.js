 ...
 * Generated: 2025-06-18T19:15:47.569Z
 */
...
`;
  ...
  // REMOVED: // REMOVED: // REMOVED: import * as WaveSurferBuilder from './components/waveSurfer_builder.js'; // File not found // File not found // File not found
  ...
  function initSquirrel() {
    ...
    // Button
    if (button_builder && button_builder.create) {
      window.Button = button_builder.create;
      window.ButtonStyles = button_builder;
      pluginManager.loadedPlugins.add('Button');
      console.log('  ✅ Button exposé');
    }
    
    // Slider
    if (slider_builder && slider_builder.create) {
      window.Slider = slider_builder.create;
      window.SliderStyles = slider_builder;
      pluginManager.loadedPlugins.add('Slider');
      console.log('  ✅ Slider exposé');
    }
    
    // Matrix
    if (Matrix && Matrix.create) {
      window.Matrix = Matrix.create;
      window.MatrixStyles = Matrix;
      pluginManager.loadedPlugins.add('Matrix');
      console.log('  ✅ Matrix exposé');
    }
    
    // Table
    if (Table && Table.create) {
      window.Table = Table.create;
      window.TableStyles = Table;
      pluginManager.loadedPlugins.add('Table');
      console.log('  ✅ Table exposé');
    }
    
    // List
    if (List && List.create) {
      window.List = List.create;
      window.ListStyles = List;
      pluginManager.loadedPlugins.add('List');
      console.log('  ✅ List exposé');
    }
    
    // Menu
    if (Menu && Menu.create) {
      window.Menu = Menu.create;
      window.MenuStyles = Menu;
      pluginManager.loadedPlugins.add('Menu');
      console.log('  ✅ Menu exposé');
    }
    ...
    if (draggable) {
      ...
      console.log('  ✅ Draggable exposé');
    }
    
    // Unit
    if (unit_builder && unit_builder.create) {
      window.Unit = unit_builder.create;
      window.UnitStyles = unit_builder;
      pluginManager.loadedPlugins.add('Unit');
      console.log('  ✅ Unit exposé');
    }
    ...
    console.log(`🎯 Squirrel.js v1.0.0 prêt!`);
    console.log(`📊 ${loadedCount} composants disponibles`);
    console.log('🧩 Composants:', Array.from(pluginManager.loadedPlugins));
    
    // === EXÉCUTION DU KICKSTART À LA FIN ===
    console.log('🚀 Exécution du kickstart...');
    ...
    console.log('✅ Kickstart terminé - Vue principale créée');
  ...
  if (typeof window !== 'undefined') {
    ...
    console.log('Squirrel.js chargé en environnement non-navigateur');
  }
