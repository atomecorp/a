...
async function startServer() {
  try {
    ...
    // Rubber Band's WebAssembly build. The bare specifier `rubberband-wasm` and the
    // .wasm are BOTH declared against this prefix (importmap in atome/src/index.html
    // and RUBBERBAND_WASM_URL): node_modules is not under the static root, which is
    // why the two module resolutions used to 404 in the browser.
    await server.register(fastifyStatic, {
      root: path.join(projectRoot, 'node_modules', 'rubberband-wasm', 'dist'),
      prefix: '/vendor/rubberband-wasm/',
      decorateReply: false,
      preCompressed: true
    });
    ...
    server.register(async function (fastify) {
      fastify.get('/ws/api', { websocket: true }, async (connection, request) => {
        ...
        try {
          ...
          connection._wsApiConnectionId = makeId('ws');
        }
        ...
        connection.on('message', async (message) => {
          ...
          // File-transfer family: extracted to server/wsFileOperations.js (P1-2).
          // The 2 465-line handler kept every family inline; this one is the first
          // moved out. Only closure values travel, the body is unchanged.
          if (data.type === 'file') {
            await handleWsFileOperation({
              connection,
              data,
              safeSend,
              projectRoot,
