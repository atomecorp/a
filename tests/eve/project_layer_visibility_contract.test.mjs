...
test('Active project load leaves only the current project layer interactive', () => {
    const dom = new JSDOM('<!doctype html><html><body><div id="project_view_alpha"></div><div id="project_view_beta"><canvas id="eve_surface_project"></canvas></div><div id="project_view_gamma"></div></body></html>');
    const betaCanvas = dom.window.document.getElementById('eve_surface_project');
    const beta = dom.window.document.getElementById('project_view_beta');
    beta.style.visibility = 'hidden';
    betaCanvas.style.visibility = 'hidden';
    betaCanvas.style.pointerEvents = 'none';
    ...
    assert.equal(beta.style.visibility, 'visible');
    assert.equal(beta.style.pointerEvents, 'auto');
    assert.equal(betaCanvas.style.visibility, '');
    assert.equal(betaCanvas.style.pointerEvents, '');
