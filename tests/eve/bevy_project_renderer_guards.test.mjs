...
test('Bevy project renderer guards lock canvas ownership, drag, and video playback routes', () => {
    ...
    const maintainedWgpuBackend = readSource('atome/renderers/wgpu-web-external-texture/wgpu-27.0.1/src/backend/webgpu.rs');
    const maintainedWgpuTypes = readSource('atome/renderers/wgpu-web-external-texture/wgpu-types-27.0.1/src/lib.rs');
    ...
    assert.match(webRendererCargo, /wgpu-web-external-texture\/wgpu-27\.0\.1/);
    assert.match(webRendererCargo, /wgpu-web-external-texture\/wgpu-types-27\.0\.1/);
    assert.match(coreCargo, /wgpu-web-external-texture\/wgpu-27\.0\.1/);
    assert.match(coreCargo, /wgpu-web-external-texture\/wgpu-types-27\.0\.1/);
