...
test('user background download quick action handles failed download results', () => {
    assert.ok(
        userPanelSource.includes("const result = await window.download_random_background_image()"),
        'download quick action must inspect the wallpaper download result'
    );
    assert.ok(
        userPanelSource.includes("result?.ok !== true") && userPanelSource.includes("setUserNotice('eve.user.error'"),
