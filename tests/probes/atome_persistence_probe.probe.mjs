  ...
  try {
    ...
    const createResult = await page.evaluate(async () => {
      const projectId = window.eveToolBase?.currentProjectId
        ...
        || null;
      if (!projectId) {
        return { ok: false, error: 'project_id_unavailable' };
      }
      if (window.eveToolBase?.createAtome) {
        const id = `test8_atome_${Date.now()}_${Math.random().toString(16).slice(2)}`;
        const created = await window.eveToolBase.createAtome({
          id,
          atome_id: id,
          kind: 'shape',
          type: 'shape',
          name: 'Persistence Probe Shape',
          projectId,
          parentId: projectId,
          left: 96,
          top: 96,
      ...
      return { ok: false, error: 'atome_create_tool_unavailable' };
    });
    ...
    report.logout = await page.evaluate(async () => {
      const api = window.AdoleAPI || null;
      if (!api?.auth?.logout) return { ok: false, error: 'auth_logout_unavailable' };
      const result = await api.auth.logout();
      return {
        ok: !!(result?.fastify?.success || result?.tauri?.success || result?.ok || result?.success),
        result
      };
    });
    if (!report.logout?.ok) throw new Error(report.logout?.error || 'logout_failed');

    await page.reload({ waitUntil: 'networkidle' });
    const debugReadyAfterLogoutReload = await waitFor(page, () => !!window.__DEBUG__?.getPersistenceState, 25000);
    report.debugReadyAfterLogoutReload = debugReadyAfterLogoutReload;
    if (!debugReadyAfterLogoutReload) throw new Error('debug_surface_unavailable_after_logout_reload');

    report.relogin = await prepareSession(page);
    if (!report.relogin?.ok) throw new Error(report.relogin?.error || 'relogin_failed');
    await page.waitForTimeout(1000);

    const afterRelogin = await readPersistence(page, 'after_relogin');
    report.afterRelogin = afterRelogin;

    const reloginDomIds = idsFromSnapshot(afterRelogin);
    const reloginStateIds = stateIdsFromSnapshot(afterRelogin);
    report.reloginCandidateStatus = candidateIds.map((id) => ({
      id,
      inDomAfterRelogin: reloginDomIds.has(id),
      inStateAfterRelogin: reloginStateIds.has(id)
    }));
    const persistedAfterRelogin = report.reloginCandidateStatus.some((entry) => entry.inDomAfterRelogin && entry.inStateAfterRelogin);
    if (!persistedAfterRelogin) throw new Error('created_atome_missing_after_relogin');

    report.projectSwitch = await page.evaluate(async ({ baseProject, secondName }) => {
      const api = window.AdoleAPI || null;
      if (!api?.projects?.create || !api?.projects?.setCurrent) return { ok: false, error: 'project_api_unavailable' };
      const readProjectId = (entry) => String(entry?.id || entry?.atome_id || '').trim();
      const readProjectName = (entry) => {
        const props = entry?.properties || entry?.particles || entry?.data || {};
        return String(entry?.name || props.name || '').trim();
      ...
      await api.projects.create(secondName);
      let projectsResult = await api.projects.list();
      let projects = projectList(projectsResult);
      const secondProject = projects.find((entry) => readProjectName(entry) === secondName) || null;
      const secondId = readProjectId(secondProject);
      if (!secondId) return { ok: false, error: 'second_project_missing', projectsResult };
      await api.projects.setCurrent(secondId, secondName, null, true);
      await new Promise((resolve) => setTimeout(resolve, 300));
      await api.projects.setCurrent(baseProject.id, baseProject.name, baseProject.owner_id || null, true);
      if (window.eveToolBase?.loadProjectAtomes) {
        await window.eveToolBase.loadProjectAtomes(baseProject.id, { force: true }).catch(() => { });
      }
      return { ok: true, secondProject: { id: secondId, name: secondName }, baseProject };
    }, {
      baseProject: report.session.project,
      secondName: `${projectName} Switch ${Date.now()}`
    });
    if (!report.projectSwitch?.ok) throw new Error(report.projectSwitch?.error || 'project_switch_failed');

    const afterProjectSwitch = await readPersistence(page, 'after_project_switch_return');
    report.afterProjectSwitch = afterProjectSwitch;
    const switchDomIds = idsFromSnapshot(afterProjectSwitch);
    const switchStateIds = stateIdsFromSnapshot(afterProjectSwitch);
    report.projectSwitchCandidateStatus = candidateIds.map((id) => ({
      id,
      inDomAfterProjectSwitch: switchDomIds.has(id),
      inStateAfterProjectSwitch: switchStateIds.has(id)
    }));
    const persistedAfterProjectSwitch = report.projectSwitchCandidateStatus.some((entry) => entry.inDomAfterProjectSwitch && entry.inStateAfterProjectSwitch);
    if (!persistedAfterProjectSwitch) throw new Error('created_atome_missing_after_project_switch');
