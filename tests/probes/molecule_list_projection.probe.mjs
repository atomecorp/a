...
test('Molecule List derives names-only Molecule, Section and Track rows from schema v2', async () => {
    ...
    assert.deepEqual(content.readState().entries.map((entry) => entry.label), ['Molécule 1', 'Section 1', 'Not a Molecule']);
    ...
    assert.deepEqual(content.readState().entries.map((entry) => entry.label), [
        'Molécule 1', 'Section 1', 'Piste 1', 'Bus 1', 'Piste 2', 'Not a Molecule'
    ]);
