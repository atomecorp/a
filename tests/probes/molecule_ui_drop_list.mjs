...
export const validateListMoleculeDrop = async ({ page, project, fixture, report, outDir }) => {
    ...
    await drag({ page, source: image, destination: overlapDestination, steps: 1, holdMs: 50 });
    await wait(650);
    ...
    await selectListRow(page, molecule.sourceParent);
    await waitFor(page, async (ownerId) => {
        ...
        const root = tree?.tree?.root || tree?.root;
        const find = (node) => node?.id === 'project_view_visual' ? node : (node?.children || []).map(find).find(Boolean);
        const visual = find(root);
        return { ok: projectViewVisualPanel.subjectId() === ownerId && (visual?.children?.length || 0) > 0 };
    }, molecule.sourceParent);
    ...
    for (const mode of ['sequential', 'random', 'layer']) {
        ...
        assert(progressed.playing && progressed.selectionIds.includes(moleculeId),
            `molecule_playback_not_progressing:${mode}:${JSON.stringify(progressed)}`);
        assert(progressed.activeLeafIds.length === (mode === 'layer' ? 2 : 1),
            `molecule_active_leaves_incorrect:${mode}:${JSON.stringify(progressed)}`);
        ...
        assert(endedSelection.primaryId === moleculeId,
        `molecule_playback_final_selection:${mode}:${JSON.stringify(endedSelection)}`);
        playbackModes.push({ mode, rule, started, progressed, ended: ended.state, endedSelection });
    }
    const children = [];
